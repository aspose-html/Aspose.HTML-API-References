...
description: "com.aspose.html.dom.css.ICSS2Properties インターフェイス。CSS2Properties インターフェイスは CSSStyleDeclaration 内のプロパティを取得・設定するための便利なメカニズムを提供します。このインターフェイスの属性は CSS2 で定義されたすべてのプロパティに対応しています。このインターフェイスの属性を取得することは、CSSStyleDeclaration インターフェイスの getPropertyValue メソッドを呼び出すことと同等です。このインターフェイスの属性を設定することは、CSSStyleDeclaration インターフェイスの setProperty メソッドを呼び出すことと同等です。"
type: docs
...
CSS2Properties インターフェイスは、[`CSSStyleDeclaration`](../icssstyledeclaration/) 内のプロパティを取得・設定するための便利なメカニズムを表します。このインターフェイスの属性は CSS2 で指定されたすべてのプロパティに対応しています。このインターフェイスの属性を取得することは、[`CSSStyleDeclaration`](../icssstyledeclaration/) インターフェイスの getPropertyValue メソッドを呼び出すことと同等です。このインターフェイスの属性を設定することは、[`CSSStyleDeclaration`](../icssstyledeclaration/) インターフェイスの setProperty メソッドを呼び出すことと同等です。
