...
description: "IViewCSS メソッド。IViewCSS.getComputedStyle メソッドは、アクティブなスタイルシートを適用し、値が含む可能性のある基本的な計算を解決した後の要素のすべての CSS プロパティ値を含むオブジェクトを返します"
type: docs
...
IViewCSS.getComputedStyle() メソッドは、アクティブなスタイルシートを適用し、これらの値に含まれる基本的な計算を解決した後、要素のすべての CSS プロパティ値を含むオブジェクトを返します。
...
| パラメータ | 型 | 説明 |
| --- | --- | --- |
| element | Element | 計算済みスタイルを取得する対象の [`Element`](../../../com.aspose.html.dom/element/)。このパラメータは null にできません |
...
| 例外 | 条件 |
| --- | --- |
| TypeError | 渡されたオブジェクトが Element でない場合、または pseudoElt が有効な疑似要素セレクタでない場合。 |
