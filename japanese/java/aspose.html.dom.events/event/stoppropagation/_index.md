...
このメソッドが任意の [`IEventListener`](../../ieventlistener/) によって呼び出された場合、イベントはツリー内での伝播を停止します。イベントは、イベントフローが停止する前に現在の [`IEventTarget`](../../ieventtarget/) 上のすべてのリスナーへのディスパッチを完了します。このメソッドは、イベントフローの任意の段階で使用できます。
