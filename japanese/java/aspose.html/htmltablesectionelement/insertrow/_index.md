...
このセクションに行を挿入します。新しい行はこのセクションの現在の `index` 番目の行の直前に挿入されます。`index` が -1 であるか、このセクションの行数と等しい場合、新しい行は末尾に追加されます。
...
| パラメータ | 型 | 説明 |
| --- | --- | --- |
| index | Int32 | 新しい行を挿入する行番号。このインデックスは 0 から始まり、このセクション内に含まれる行に対してのみ相対的であり、テーブル全体の行に対してではありません。 |
