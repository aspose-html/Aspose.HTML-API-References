...
description: "com.aspose.html.HTMLElement クラス。すべての HTML 要素インターフェイスはこのクラスから派生します。HTML のコア属性のみを公開する要素は、基本 HTMLElement インターフェイスで表されます。これらの要素は以下のとおりです：特別な SUB SUP SPAN BDO font TT I B U S STRIKE BIG SMALL phrase EM STRONG DFN CODE SAMP KBD VAR CITE ACRONYM ABBR list DD DT NOFRAMES NOSCRIPT ADDRESS CENTER。HTML 要素の style 属性は、CSS モジュール DOM Level 2 Style Sheets and CSS で定義されている ElementCSSInlineStyle インターフェイスを通じてアクセスできます。"
type: docs
...
すべての HTML 要素インターフェイスはこのクラスから派生します。HTML のコア属性のみを公開する要素は、基本の `HTMLElement` インターフェイスで表されます。これらの要素は以下の通りです：special: SUB, SUP, SPAN, BDO; font: TT, I, B, U, S, STRIKE, BIG, SMALL; phrase: EM, STRONG, DFN, CODE, SAMP, KBD, VAR, CITE, ACRONYM, ABBR; list: DD, DT, NOFRAMES, NOSCRIPT, ADDRESS, CENTER。HTML 要素の `style` 属性は、CSS モジュールで定義されている `ElementCSSInlineStyle` インターフェイスを介してアクセスできます [[DOM Level 2 Style Sheets and CSS](http://www.w3.org/TR/2000/REC-DOM-Level-2-Style-20001113)]。
