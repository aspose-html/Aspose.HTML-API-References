...
description: "com.aspose.html.HTMLInputElement クラス。フォームコントロール。ページが表示される環境に応じて、ファイルアップロード入力タイプの場合、value プロパティは読み取り専用になることがあります。パスワード入力タイプの場合、実際に返される値は不正使用を防ぐためにマスクされることがあります。HTML 4.01 の INPUT 要素定義を参照してください。さらに、Document Object Model DOM Level 2 HTML Specification も参照してください。"
type: docs
...
| [getFiles](../../com.aspose.html/htmlinputelement/files/) files IDL 属性により、スクリプトは要素の選択されたファイルにアクセスできます。取得時に IDL 属性が適用される場合、現在選択されているファイルを表す FileList オブジェクトを返す必要があります。選択されたファイルのリストが変わるまで同じオブジェクトが返されます。IDL 属性が適用されない場合は、代わりに null を返す必要があります。 [FILEAPI] |
