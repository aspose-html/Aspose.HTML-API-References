...
description: "com.aspose.html.HTMLTableElement クラス。テーブル上の作成および削除メソッドにより、作者はテーブルを構築および変更できます。HTML 4.01 では、テーブル内に CAPTION、THEAD、TFOOT 要素はそれぞれ最大1つしか存在できないと規定されています。そのため、既に存在し、createTHead または createTFoot メソッドが呼び出された場合、メソッドは既存の THead または TFoot 要素を返します。HTML 4.01 の TABLE 要素定義をご参照ください。"
type: docs
...
テーブル上の create* と delete* メソッドは、作者がテーブルを構築および変更できるようにします。[[HTML 4.01](http://www.w3.org/TR/1999/REC-html401-19991224)] は、テーブル内に `CAPTION`、`THEAD`、`TFOOT` 要素がそれぞれ最大1つしか存在できないと規定しています。したがって、既に存在し、createTHead() または createTFoot() メソッドが呼び出された場合、メソッドは既存の THead または TFoot 要素を返します。HTML 4.01 の TABLE 要素定義を参照してください。
...
| [getTBodies](../../com.aspose.html/htmltableelement/tbodies/) テーブル本体のコレクションを返します（暗黙的なものを含む）。 |
...
| [createTFoot](../../com.aspose.html/htmltableelement/createtfoot/)() | テーブルフッター行を作成するか、既存のものを返します。 |
| [createTHead](../../com.aspose.html/htmltableelement/createthead/)() | テーブルヘッダー行を作成するか、既存のものを返します。 |
| [deleteCaption](../../com.aspose.html/htmltableelement/deletecaption/)() | テーブルキャプションが存在する場合、削除します。 |
| [deleteRow](../../com.aspose.html/htmltableelement/deleterow/)(int) | テーブル行を削除します。 |
| [deleteTFoot](../../com.aspose.html/htmltableelement/deletetfoot/)() | テーブルにフッターが存在する場合、削除します。 |
| [deleteTHead](../../com.aspose.html/htmltableelement/deletethead/)() | テーブルにヘッダーが存在する場合、削除します。 |
...
| [insertRow](../../com.aspose.html/htmltableelement/insertrow/)(int) | テーブルに新しい空行を挿入します。新しい行は、テーブル内の現在の `index` 番目の行の直前、同じセクションに挿入されます。`index` が -1 もしくは行数と同じ場合、新しい行は末尾に追加されます。さらに、テーブルが空の場合、行は作成されテーブルに挿入される `TBODY` に挿入されます。テーブル行は [[HTML 4.01](http://www.w3.org/TR/1999/REC-html401-19991224)] に従い空にできません。 |
