...
description: "HTMLTableRowElement メソッド。この行に空の TD セルを挿入します。index が -1 またはセル数と等しい場合、新しいセルは末尾に追加されます"
type: docs
...
`TD` の空セルをこの行に挿入します。`index` が -1 またはセル数と等しい場合、新しいセルは末尾に追加されます。
...
| Parameter | Type | 説明 |
| --- | --- | --- |
| index | Int32 | セルを挿入する位置です。0 から始まります。 |
