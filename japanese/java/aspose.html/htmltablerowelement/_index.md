---
title: "HTMLTableRowElement クラス"
second_title: "Aspose.HTML for Java API リファレンス"
description: "com.aspose.html.HTMLTableRowElement クラス。テーブル内の行です。HTML 4.01 の TR 要素の定義をご覧ください。"
type: docs

url: /ja/java/com.aspose.html/htmltablerowelement/
---
## HTMLTableRowElement class

テーブルの行です。HTML 4.01 の TR 要素定義を参照してください。
...
| [getCells](../../com.aspose.html/htmltablerowelement/cells/) この行のセルのコレクションです。 @version DOM Level 2 |
[getCh]
...
| [getRowIndex](../../com.aspose.html/htmltablerowelement/rowindex/) これは論理順序であり、文書順序ではありません。`rowIndex` はテーブル内のセクション（`THEAD`、`TFOOT`、または `TBODY`）を考慮し、インデックスではまず `THEAD` 行を配置し、次に `TBODY` 行、最後に `TFOOT` 行を配置します。 @version DOM Level 2 |
| [getSectionRowIndex](../../com.aspose.html/htmltablerowelement/sectionrowindex/) この行のインデックスで、現在のセクション（`THEAD`、`TFOOT`、または `TBODY`）に対して相対的に、0 から開始します。 @version DOM Level 2 |
...
| [deleteCell](../../com.aspose.html/htmltablerowelement/deletecell/)(int) | 現在の行からセルを削除します。 |
...
| [insertCell](../../com.aspose.html/htmltablerowelement/insertcell/)(int) | この行に空の `TD` セルを挿入します。`index` が -1 もしくはセル数と同じ場合、新しいセルは末尾に追加されます。 |
