...
description: "com.aspose.html.HTMLObjectElement クラス。汎用埋め込みオブジェクト。原則としてオブジェクト要素のすべてのプロパティは読み書き可能ですが、環境によっては基になるオブジェクトがインスタンス化された後に一部のプロパティが読み取り専用になることがあります。HTML 4.01 の OBJECT 要素定義を参照してください。"
type: docs
...
汎用埋め込みオブジェクト。原則として、object 要素のすべてのプロパティは読み書き可能ですが、環境によっては基になるオブジェクトがインスタンス化された後に読み取り専用になるプロパティがあります。[[HTML 4.01](http://www.w3.org/TR/1999/REC-html401-19991224)] の OBJECT 要素定義を参照してください。
