...
description: "com.aspose.html.HTMLLinkElement クラス。LINK 要素は外部リソースへのリンクを指定し、このドキュメントとそのリソースとの関係（またはその逆）を定義します。HTML 4.01 の LINK 要素定義を参照してください。また、StyleSheet モジュールの LinkStyle インターフェイス（DOM Level 2 スタイルシートと CSS）も参照してください。"
type: docs
...
`LINK` 要素は外部リソースへのリンクを指定し、そのリソースとの文書の関係（またはその逆）を定義します。HTML 4.01 の LINK 要素の定義を参照してください（スタイルシートモジュールの `LinkStyle` インターフェイスも参照してください [[DOM Level 2 Style Sheets and CSS](http://www.w3.org/TR/2000/REC-DOM-Level-2-Style-20001113)]）。
