---
title: "PlatformException"
second_title: "Aspose.HTML for Java API リファレンス"
description: "PlatformException コンストラクタ。PlatformException クラスの新しいインスタンスを初期化します。"
type: docs

url: /ja/java/com.aspose.html/platformexception/platformexception/
---
## PlatformException() {#constructor}

[`PlatformException`](../) クラスの新しいインスタンスを初期化します。

```java
public PlatformException()
```

### 関連項目

* class [PlatformException](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## PlatformException(String) {#constructor_1}

[`PlatformException`](../) クラスの新しいインスタンスを、指定されたエラーメッセージで初期化します。

```java
public PlatformException(String message)
```

| Parameter | Type | 説明 |
| --- | --- | --- |
| message | 文字列 | エラーを説明するメッセージ。 |

### 関連項目

* class [PlatformException](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## PlatformException(String, Exception) {#constructor_2}

[`PlatformException`](../) クラスの新しいインスタンスを、指定されたエラーメッセージと、この例外の原因となる内部例外への参照で初期化します。

```java
public PlatformException(String message, Exception innerException)
```

| Parameter | Type | 説明 |
| --- | --- | --- |
| message | 文字列 | 例外の原因を説明するエラーメッセージ。 |
| innerException | 例外 | 現在の例外の原因となっている例外、または内部例外が指定されていない場合は null 参照です。 |

### 関連項目

* class [PlatformException](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)
