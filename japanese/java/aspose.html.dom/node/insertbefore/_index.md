...
指定されたノードがすでにドキュメント内に存在する場合、insertBefore() はそのノードを現在の位置から新しい位置へ移動します。（つまり、指定された新しい親ノードに追加される前に、既存の親から自動的に削除されます。）

これは、ノードが同時に文書内の二つの場所に存在できないことを意味します。
...
追加された子ノードを返します（newNode が [`DocumentFragment`](../../documentfragment/) の場合は除き、その場合は空の [`DocumentFragment`](../../documentfragment/) が返されます）。
