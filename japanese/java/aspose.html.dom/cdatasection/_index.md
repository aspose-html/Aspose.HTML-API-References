---
title: "CDATASection クラス"
second_title: "Aspose.HTML for Java API リファレンス"
description: "com.aspose.html.dom.CDATASection クラス。CDATA セクションは、マークアップと見なされる可能性のある文字を含むテキストブロックをエスケープするために使用されます。"
type: docs

url: /ja/java/com.aspose.html.dom/cdatasection/
---
## CDATASection class

CDATA セクションは、マークアップと見なされる文字を含むテキストブロックをエスケープするために使用されます。
...
| [getIsElementContentWhitespace](../../com.aspose.html.dom/text/iselementcontentwhitespace/) このテキストノードが要素コンテンツの空白（しばしば「無視できる空白」と呼ばれる）を含むかどうかを返します。 |
...
| [getNodeName](../../com.aspose.html.dom/cdatasection/nodename/) このノードの名前で、タイプに応じて決まります。 |
| [getNodeType](../../com.aspose.html.dom/cdatasection/nodetype/) 基底オブジェクトのタイプを表すコードです。 |
| [nodeValue](../../com.aspose.html.dom/text/nodevalue/) { get; set; } | このノードの値は、タイプに応じて異なります。 |
...
| [textContent](../../com.aspose.html.dom/text/textcontent/) { get; set; } | この属性は、このノードとその子孫のテキストコンテンツを返します。null に設定されている場合、設定しても効果はありません。設定すると、このノードが持つ可能性のあるすべての子が削除され、新しい文字列が空でなく null でもない場合は、その文字列を含む単一の Text ノードに置き換えられます。 |
| [getWholeText](../../com.aspose.html.dom/text/wholetext/) このノードに論理的に隣接する Text ノードすべてのテキストを、文書順に連結して返します。 |
...
| [insertData](../../com.aspose.html.dom/characterdata/insertdata/)(int, String) | 指定された 16 ビット単位オフセットに文字列を挿入します。 |
...
| [normalize](../../com.aspose.html.dom/node/normalize/)() | このノードの下位サブツリー全体の深さにあるすべての[`Text`](../text/)ノード（属性ノードを含む）を、構造（例：[`elements`](../element/)、[`comments`](../comment/)、[`processing instructions`](../processinginstruction/)、`CDATA sections`、[`entity references`](../entityreference/)）だけが[`Text`](../text/)ノードを分離する「標準」形式に変換します。つまり、隣接する Text ノードや空の Text ノードは存在しません。これは、ドキュメントの DOM 表示が保存して再読み込みした場合と同じになることを保証するために使用でき、特定のドキュメントツリー構造に依存する操作（例：XPointer [XPointer] ルックアップ）を使用する際に有用です。[`DOMConfiguration`](../../com.aspose.html/configuration/) オブジェクトに付随する [`Node.ownerDocument`](../node/ownerdocument/) のパラメータ "normalize-characters" が true の場合、このメソッドは Text ノードの文字も完全に正規化します。 |
...
| [replaceData](../../com.aspose.html.dom/characterdata/replacedata/)(int, int, String) | 指定された 16 ビット単位オフセットから始まる文字を、指定された文字列で置き換えます。 |
| [replaceWholeText](../../com.aspose.html.dom/text/replacewholetext/)(String) | 現在のノードと論理的に隣接するすべてのテキストノードのテキストを指定されたテキストに置き換えます。論理的に隣接するテキストノードはすべて削除され、現在のノードも置換テキストの受取側でない限り削除されます。 |
| [splitText](../../com.aspose.html.dom/text/splittext/)(int) | 指定されたオフセットでこのノードを2つに分割し、両方をツリー上で兄弟ノードとして保持します。 |
