...
description: "com.aspose.html.dom.CDATASection クラス。CDATA セクションは、マークアップとみなされる可能性のある文字を含むテキストブロックをエスケープするために使用されます。"
type: docs
...
| [getNodeName](../../com.aspose.html.dom/cdatasection/nodename/) このノードの名前は、そのタイプに依存します。 |
| [getNodeType](../../com.aspose.html.dom/cdatasection/nodetype/) 基礎オブジェクトのタイプを表すコードです。 |
| [nodeValue](../../com.aspose.html.dom/text/nodevalue/) { get; set; } | このノードの値を、タイプに応じて返します。 |
...
| [textContent](../../com.aspose.html.dom/text/textcontent/) { get; set; } | この属性は、このノードとその子孫のテキストコンテンツを返します。null に設定されている場合、設定しても効果はありません。設定すると、このノードが持つ可能性のあるすべての子ノードが削除され、かつ新しい文字列が空でなく null でもない場合は、その文字列を含む単一の Text ノードに置き換えられます。 |
| [getWholeText](../../com.aspose.html.dom/text/wholetext/) このノードに論理的に隣接する Text ノードすべてのテキストを文書順に連結して返します。 |
...
| [replaceWholeText](../../com.aspose.html.dom/text/replacewholetext/)(String) | 現在のノードと論理的に隣接するすべてのテキストノードのテキストを指定されたテキストに置き換えます。論理的に隣接するテキストノードはすべて削除され、置換テキストの受取側でなかった現在のノードも含まれます。 |
| [splitText](../../com.aspose.html.dom/text/splittext/)(int) | 指定されたオフセットでこのノードを2つのノードに分割し、両方をツリー内の兄弟ノードとして保持します。 |
