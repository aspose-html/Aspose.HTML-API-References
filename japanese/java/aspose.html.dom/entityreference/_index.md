---
title: "EntityReference クラス"
second_title: "Aspose.HTML for Java API リファレンス"
description: "com.aspose.html.dom.EntityReference クラス。EntityReference ノードはツリー内のエンティティ参照を表すために使用できます。"
type: docs

url: /ja/java/com.aspose.html.dom/entityreference/
---
## EntityReference class

EntityReference ノードは、ツリー内のエンティティ参照を表すために使用できます。
...
| [getNodeName](../../com.aspose.html.dom/entityreference/nodename/) このノードの名前で、タイプに応じて決まります。 |
| [getNodeType](../../com.aspose.html.dom/entityreference/nodetype/) 基底オブジェクトのタイプを表すコードです。 |
...
| [normalize](../../com.aspose.html.dom/node/normalize/)() | すべての [`Text`](../text/) ノードを、このノード以下のサブツリー全体の深さにわたって配置し、属性ノードを含めて「正規」形にします。この形では構造（例：[`elements`](../element/)、[`comments`](../comment/)、[`processing instructions`](../processinginstruction/)、[`CDATA sections`](../cdatasection/)、および `entity references`）だけが [`Text`](../text/) ノードを分離し、隣接した Text ノードや空の Text ノードが存在しません。これにより、ドキュメントの DOM ビューが保存して再読み込みした場合と同じになることを保証でき、特定のドキュメントツリー構造に依存する操作（例：XPointer [XPointer] ルックアップ）で使用する際に有用です。[`DOMConfiguration`](../../com.aspose.html/configuration/) オブジェクトの "normalize-characters" パラメータが、[`Node.ownerDocument`](../node/ownerdocument/) に添付されている場合は、このメソッドは Text ノードの文字も完全に正規化します。 |
