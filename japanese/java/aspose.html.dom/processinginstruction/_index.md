...
description: "com.aspose.html.dom.ProcessingInstruction クラス。ProcessingInstruction は、XML でプロセッサ固有の情報を文書のテキストに保持するために使用される処理指示を表します。"
type: docs
...
| [getNodeName](../../com.aspose.html.dom/processinginstruction/nodename/) このノードの名前（タイプに応じて）です。 |
| [getNodeType](../../com.aspose.html.dom/processinginstruction/nodetype/) 基底オブジェクトのタイプを表すコードです。 |
| [nodeValue](../../com.aspose.html.dom/processinginstruction/nodevalue/) { get; set; } | このノードの値を、タイプに応じて返します。 |
...
| [getTarget](../../com.aspose.html.dom/processinginstruction/target/) この処理命令のターゲットです。 |
| [textContent](../../com.aspose.html.dom/processinginstruction/textcontent/) { get; set; } | この属性は、このノードとその子孫のテキストコンテンツを返します。null に設定されている場合、設定しても効果はありません。設定すると、このノードが持つ可能性のあるすべての子ノードが削除され、かつ新しい文字列が空でなく null でもない場合は、その文字列を含む単一の Text ノードに置き換えられます。 |
...
| [normalize](../../com.aspose.html.dom/node/normalize/)() | このノードの下位ツリー全体の深さにあるすべての[`Text`](../text/)ノードを、属性ノードを含めて「正規」形に変換します。正規形では、構造（例：[`elements`](../element/)、[`comments`](../comment/)、`processing instructions`、[`CDATA sections`](../cdatasection/)、および[`entity references`](../entityreference/)）だけが[`Text`](../text/)ノードを分離し、隣接するテキストノードや空のテキストノードが存在しません。これは、ドキュメントの DOM ビューが保存して再読み込みした場合と同じになることを保証するために使用でき、特定のドキュメントツリー構造に依存する操作（例：XPointer [XPointer] ルックアップ）を使用する際に有用です。[`DOMConfiguration`](../../com.aspose.html/configuration/) オブジェクトに添付された `Node.ownerDocument` のパラメータ "normalize-characters" が true の場合、このメソッドは Text ノードの文字も完全に正規化します。 |
