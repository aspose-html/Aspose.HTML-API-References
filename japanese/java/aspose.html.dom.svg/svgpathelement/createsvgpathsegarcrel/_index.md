---
title: "SVGPathElement.CreateSVGPathSegArcRel"
second_title: "Aspose.HTML for Java API リファレンス"
description: "SVGPathElement メソッド。スタンドアロンで親のない SVGPathSegArcRel オブジェクトを返します"
type: docs

url: /ja/java/com.aspose.html.dom.svg/svgpathelement/createsvgpathsegarcrel/
---
## SVGPathElement.CreateSVGPathSegArcRel method

単独で親を持たない SVGPathSegArcRel オブジェクトを返します。

```java
public SVGPathSegArcRel CreateSVGPathSegArcRel(float x, float y, float r1, float r2, float angle, 
    bool largeArcFlag, bool sweepFlag)
```

| パラメータ | 型 | 説明 |
| --- | --- | --- |
| x | Single | このパスセグメントの終点の相対 X 座標。 |
| y | Single | このパスセグメントの終点の相対 Y 座標。 |
| r1 | Single | 楕円の x 軸半径（すなわち r1）です。 |
| r2 | Single | 楕円の y 軸半径（すなわち r2）。 |
| angle | Single | ユーザー座標系の x 軸に対する楕円の x 軸の回転角度（度）。 |
| largeArcFlag | Boolean | large-arc-flag パラメータの値。 |
| sweepFlag | Boolean | large-arc-flag パラメータの値。 |

### 戻り値

スタンドアロンで親のない SVGPathSegArcRel オブジェクト。

### 関連項目

* class [SVGPathSegArcRel](../../../com.aspose.html.dom.svg.paths/svgpathsegarcrel/)
* class [SVGPathElement](../)
* package [com.aspose.html.dom.svg](../../../com.aspose.html.dom.svg/)
* package [Aspose.HTML](../../../)
