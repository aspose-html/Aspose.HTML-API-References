...
description: "com.aspose.html.dom.svg.SVGSVGElement クラス。主要なインターフェイス定義は SVGSVGElement インターフェイスで、svg 要素に対応するインターフェイスです。このインターフェイスには、行列操作などのさまざまな一般的に使用されるユーティリティメソッドや、ビジュアルレンダリングデバイスでの再描画時間を制御する機能が含まれています。"
type: docs

url: /ja/java/com.aspose.html.dom.svg/svgsvgelement/
---
## SVGSVGElement class
...
| [getCurrentTranslate](../../com.aspose.html.dom.svg/svgsvgelement/currenttranslate/) 最外層の svg 要素では、ユーザーの"拡大" を考慮した対応する平行移動係数を表します。最外層でない ‘svg’ 要素でアクセスされた場合、この属性の動作は未定義です。 |
...
| [getHeight](../../com.aspose.html.dom.svg/svgsvgelement/height/) 指定された ‘svg’ 要素の属性 ‘height’ に対応します。 |
[getId]
...
| [getPreserveAspectRatio](../../com.aspose.html.dom.svg/svgsvgelement/preserveaspectratio/) 指定された要素の属性 ‘preserveAspectRatio’ に対応します。 |
...
| [getViewBox](../../com.aspose.html.dom.svg/svgsvgelement/viewbox/) 指定された要素の属性 ‘viewBox’ に対応します。 |
...
| [getWidth](../../com.aspose.html.dom.svg/svgsvgelement/width/) 指定された ‘svg’ 要素の属性 ‘width’ に対応します。 |
| [X](../../com.aspose.html.dom.svg/svgsvgelement/x/) { get; } | 指定された ‘svg’ 要素の属性 ‘x’ に対応します。 |
| [Y](../../com.aspose.html.dom.svg/svgsvgelement/y/) { get; } | 指定された ‘svg’ 要素の属性 ‘y’ に対応します。 |
[getZoomAndPan]
...
| [animationsPaused](../../com.aspose.html.dom.svg/svgsvgelement/animationspaused/)() | この SVG ドキュメントフラグメントが一時停止状態にある場合、true を返します。 |
...
| [createEvent](../../com.aspose.html.dom.svg/svgsvgelement/createevent/)(String) | 実装がサポートするタイプの [`Event`](../../com.aspose.html.dom.events/event/) を作成します。 |
| [createSVGAngle](../../com.aspose.html.dom.svg/svgsvgelement/createsvgangle/)() | ドキュメントツリー外で SVGAngle オブジェクトを作成します。オブジェクトは 0 度（単位なし）の値で初期化されます。 |
| [createSVGLength](../../com.aspose.html.dom.svg/svgsvgelement/createsvglength/)() | ドキュメントツリー外で SVGLength オブジェクトを作成します。オブジェクトは 0 ユーザー単位の値で初期化されます。 |
| [createSVGMatrix](../../com.aspose.html.dom.svg/svgsvgelement/createsvgmatrix/)() | ドキュメントツリー外で SVGMatrix オブジェクトを作成します。オブジェクトは単位行列で初期化されます。 |
| [createSVGNumber](../../com.aspose.html.dom.svg/svgsvgelement/createsvgnumber/)() | ドキュメントツリー外で SVGNumber オブジェクトを作成します。オブジェクトはゼロの値で初期化されます。 |
| [createSVGPoint](../../com.aspose.html.dom.svg/svgsvgelement/createsvgpoint/)() | ドキュメントツリー外で SVGPoint オブジェクトを作成します。オブジェクトはユーザー座標系の点 (0,0) で初期化されます。 |
| [createSVGRect](../../com.aspose.html.dom.svg/svgsvgelement/createsvgrect/)() | ドキュメントツリー外で SVGRect オブジェクトを作成します。オブジェクトはすべての値が 0 ユーザー単位に設定された状態で初期化されます。 |
| [createSVGTransform](../../com.aspose.html.dom.svg/svgsvgelement/createsvgtransform/)() | ドキュメントツリー外で SVGTransform オブジェクトを作成します。オブジェクトは単位行列変換 (SVG_TRANSFORM_MATRIX) で初期化されます。 |
| [createSVGTransformFromMatrix](../../com.aspose.html.dom.svg/svgsvgelement/createsvgtransformfrommatrix/)(SVGMatrix) | ドキュメントツリー外で SVGTransform オブジェクトを作成します。オブジェクトは指定された行列変換（すなわち SVG_TRANSFORM_MATRIX）で初期化されます。パラメータ行列の値はコピーされ、matrix パラメータは SVGTransform::matrix として採用されません。 |
...
| [getCurrentTime](../../com.aspose.html.dom.svg/svgsvgelement/getcurrenttime/)() | 現在の SVG ドキュメントフラグメントの開始時刻からの経過秒数を返します。ドキュメントのタイムラインが開始される前に getCurrentTime が呼び出された場合（例として、ドキュメントの SVGLoad イベントが発火する前に ‘script’ 要素内のスクリプトが実行された場合）、0 が返されます。 |
| [getElementById](../../com.aspose.html.dom.svg/svgsvgelement/getelementbyid/)(String) | この SVG ドキュメントフラグメント（検索はドキュメントツリーのサブセットに限定されます）内で、elementId で指定された id を持つ要素を検索します。要素が見つかればその要素を返します。該当する要素が存在しない場合は null を返します。同じ id を持つ要素が複数ある場合の動作は未定義です。 |
...
| [pauseAnimations](../../com.aspose.html.dom.svg/svgsvgelement/pauseanimations/)() | この ‘svg’ 要素に対応する SVG ドキュメントフラグメント内で定義された、現在実行中のすべてのアニメーションを一時停止（＝pause）し、フラグメントに対応するアニメーションクロックが再開されるまで停止させます。 |
...
| [setCurrentTime](../../com.aspose.html.dom.svg/svgsvgelement/setcurrenttime/)(float) | この SVG ドキュメント フラグメントのクロックを調整し、新しい現在時刻を設定します。setCurrentTime がドキュメントのタイムラインが開始される前に呼び出された場合（たとえば、ドキュメントの SVGLoad イベントがディスパッチされる前に ‘script’ 要素内で実行されるスクリプトによる場合）、メソッドの最後の呼び出しでの秒数の値は、ドキュメントのタイムラインが開始されたときにドキュメントがシークする時刻を示します。 |
...
| [unpauseAnimations](../../com.aspose.html.dom.svg/svgsvgelement/unpauseanimations/)() | SVG ドキュメント フラグメント内で定義された現在実行中のアニメーションのサスペンド（つまり、一時停止）を解除し、アニメーションクロックがサスペンドされた時点から続行するようにします。 |
