...
主要なインターフェイス定義は SVGSVGElement インターフェイスで、‘svg’ 要素に対応するインターフェイスです。このインターフェイスには、行列演算や視覚レンダリングデバイスでの再描画時間を制御する機能など、さまざまな一般的に使用されるユーティリティメソッドが含まれています。
...
| [getCurrentTranslate](../../com.aspose.html.dom.svg/svgsvgelement/currenttranslate/) 最外層の svg 要素では、ユーザーの「拡大」を考慮した対応する平行移動係数を示します。最外層でない ‘svg’ 要素でアクセスした場合、この属性の動作は未定義です。 |
...
| [createSVGAngle](../../com.aspose.html.dom.svg/svgsvgelement/createsvgangle/)() | SVGAngle オブジェクトをドキュメントツリー外で作成します。オブジェクトは 0 度（単位なし）の値で初期化されます。 |
| [createSVGLength](../../com.aspose.html.dom.svg/svgsvgelement/createsvglength/)() | SVGLength オブジェクトをドキュメントツリー外で作成します。オブジェクトは 0 ユーザー単位の値で初期化されます。 |
| [createSVGMatrix](../../com.aspose.html.dom.svg/svgsvgelement/createsvgmatrix/)() | SVGMatrix オブジェクトをドキュメントツリー外で作成します。オブジェクトは単位行列で初期化されます。 |
| [createSVGNumber](../../com.aspose.html.dom.svg/svgsvgelement/createsvgnumber/)() | SVGNumber オブジェクトをドキュメントツリー外で作成します。オブジェクトはゼロの値で初期化されます。 |
| [createSVGPoint](../../com.aspose.html.dom.svg/svgsvgelement/createsvgpoint/)() | SVGPoint オブジェクトをドキュメントツリー外で作成します。オブジェクトはユーザー座標系の点 (0,0) で初期化されます。 |
| [createSVGRect](../../com.aspose.html.dom.svg/svgsvgelement/createsvgrect/)() | SVGRect オブジェクトをドキュメントツリー外で作成します。オブジェクトはすべての値が 0 ユーザー単位に設定された状態で初期化されます。 |
| [createSVGTransform](../../com.aspose.html.dom.svg/svgsvgelement/createsvgtransform/)() | SVGTransform オブジェクトをドキュメントツリー外で作成します。オブジェクトは単位行列変換 (SVG_TRANSFORM_MATRIX) で初期化されます。 |
| [createSVGTransformFromMatrix](../../com.aspose.html.dom.svg/svgsvgelement/createsvgtransformfrommatrix/)(SVGMatrix) | SVGTransform オブジェクトをドキュメントツリー外で作成します。オブジェクトは指定された行列変換 (SVG_TRANSFORM_MATRIX) で初期化されます。パラメータ行列の値はコピーされ、matrix パラメータは SVGTransform::matrix として採用されません。 |
...
| [getCurrentTime](../../com.aspose.html.dom.svg/svgsvgelement/getcurrenttime/)() | 現在の SVG ドキュメントフラグメントの開始時刻からの経過秒数を返します。getCurrentTime がドキュメントのタイムラインが開始される前（例: ‘script’ 要素内のスクリプトが SVGLoad イベントがディスパッチされる前に実行される場合）に呼び出された場合、0 が返されます。 |
| [getElementById](../../com.aspose.html.dom.svg/svgsvgelement/getelementbyid/)(String) | この SVG ドキュメントフラグメント（すなわち、ドキュメントツリーのサブセットに限定された検索）内で、elementId で指定された id を持つ Element を検索します。要素が見つかればその Element を返します。該当する要素が存在しない場合は null を返します。複数の要素が同じ id を持つ場合の動作は未定義です。 |
...
| [pauseAnimations](../../com.aspose.html.dom.svg/svgsvgelement/pauseanimations/)() | この ‘svg’ 要素に対応する SVG ドキュメントフラグメント内で定義された、現在実行中のすべてのアニメーションを一時停止（＝pause）します。これにより、ドキュメントフラグメントに対応するアニメーションクロックが停止し、再開されるまで停止したままになります。 |
...
| [setCurrentTime](../../com.aspose.html.dom.svg/svgsvgelement/setcurrenttime/)(float) | この SVG ドキュメントフラグメントの時計を調整し、新しい現在時刻を設定します。setCurrentTime がドキュメントのタイムラインが開始される前（例として、ドキュメントの SVGLoad イベントがディスパッチされる前に ‘script’ 要素内で実行されるスクリプトによって）に呼び出された場合、メソッドの最後の呼び出しでの seconds の値は、ドキュメントのタイムラインが開始されたときにドキュメントがシークする時刻を示します。 |
...
| [unpauseAnimations](../../com.aspose.html.dom.svg/svgsvgelement/unpauseanimations/)() | SVG ドキュメントフラグメント内で定義された現在実行中のアニメーションを再開（つまり、一時停止解除）し、アニメーション時計が一時停止された時点から続行されます。 |
