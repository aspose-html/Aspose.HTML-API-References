...
| 名前 | 説明 |
| --- | --- |
| [arc](../../com.aspose.html.dom.canvas/icanvaspathmethods/arc/#arc)(double, double, double, double, double) | パスに弧を追加します。弧は (x, y) を中心とし、半径 r で、startAngle から始まり endAngle で終わり、指定された方向（デフォルトは時計回りで、反時計回りを指定可能）に描かれます。 |
| [arc](../../com.aspose.html.dom.canvas/icanvaspathmethods/arc/#arc_1)(double, double, double, double, double, bool) | パスに弧を追加します。弧は (x, y) を中心とし、半径 r で、startAngle から始まり endAngle で終わり、指定された方向（デフォルトは時計回りで、反時計回りを指定可能）に描かれます。 |
| [arcTo](../../com.aspose.html.dom.canvas/icanvaspathmethods/arcto/)(double, double, double, double, double) | 指定された制御点と半径を使用してパスに弧を追加し、前の点と直線で接続します。 |
| [bezierCurveTo](../../com.aspose.html.dom.canvas/icanvaspathmethods/beziercurveto/)(double, double, double, double, double, double) | パスに三次ベジェ曲線を追加します。3 つの点が必要です。最初の 2 点は制御点、3 番目の点が終点です。開始点は現在のパスの最後の点で、ベジェ曲線を作成する前に moveTo() で変更できます。 |
