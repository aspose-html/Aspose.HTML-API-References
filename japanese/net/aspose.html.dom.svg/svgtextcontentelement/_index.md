...
description: Aspose.Html.Dom.Svg.SVGTextContentElement クラス. SVGTextContentElement はSVGTextElementSVGTSpanElementSVGTRefElementSVGAltGlyphElementSVGTextPathElement などさまざまなテキスト関連のインターフェイスによって継承されます 文字または文字数のインデックスを参照するこのインターフェイスのメソッドについてはこれらの参照はそれぞれUTF16 コード単位または UTF16 コード単位の数へのインデックスとして解釈されますこれはCharacterData インターフェイスのメソッドが文字データ内のインデックスとカウントとして UTF16 コード単位を使用する DOM レベル 2 コアとの一貫性のためです U10000 などの BMP 文字の場合その要素で getNumberOfChars を呼び出すと 2 が返されますこれはその 1 文字を表すために使用される UTF16 コード単位 サロゲート ペア が 2 つあるためです
type: docs
weight: 2340
url: /ja/net/aspose.html.dom.svg/svgtextcontentelement/
---
...
| 名前 | 説明 |
| --- | --- |
| const [LENGTHADJUST_SPACING](../../aspose.html.dom.svg/svgtextcontentelement/lengthadjust_spacing/) | 値「間隔」に対応します。 |
| const [LENGTHADJUST_SPACINGANDGLYPHS](../../aspose.html.dom.svg/svgtextcontentelement/lengthadjust_spacingandglyphs/) | 値「spacingAndGlyphs」に対応します。 |
| const [LENGTHADJUST_UNKNOWN](../../aspose.html.dom.svg/svgtextcontentelement/lengthadjust_unknown/) | 列挙型が事前定義されたタイプではない値に設定されました。この型の新しい値を定義しようとしたり、既存の値をこの型に切り替えようとしたりすることは無効です. |
