...
description: Aspose.Html.Dom.Events.ErrorEvent クラス. ErrorEvent実行時に発生したエラーに関するコンテキスト情報を提供します.
type: docs
weight: 760
url: /ja/net/aspose.html.dom.events/errorevent/
---
## ErrorEvent class

`ErrorEvent`実行時に発生したエラーに関するコンテキスト情報を提供します.
...
| 名前 | 説明 |
| --- | --- |
| [ErrorEvent](errorevent/#constructor_1)(Exception) | の新しいインスタンスを初期化します`ErrorEvent` class. |
| [ErrorEvent](errorevent/#constructor)(IDictionary&lt;string, object&gt;) | の新しいインスタンスを初期化します`ErrorEvent` class. |
...
| [ColNo](../../aspose.html.dom.events/errorevent/colno/) { get; } | colno 属性は、初期化された値を返す必要があります。オブジェクトの作成時に、この属性をゼロに初期化する必要があります。スクリプト内でエラーが発生した列番号を表します。 |
...
| [Error](../../aspose.html.dom.events/errorevent/error/) { get; } | error 属性は、初期化された値を返す必要があります。オブジェクトの作成時に、この属性を null に初期化する必要があります。必要に応じて、エラーを表すオブジェクトに設定されます (例: キャッチされていない DOM 例外の場合の例外オブジェクト). |
| [EventPhase](../../aspose.html.dom.events/event/eventphase/) { get; } | イベントフローのどのフェーズが現在評価されているかを示すために使用されます. |
| [FileName](../../aspose.html.dom.events/errorevent/filename/) { get; } | filename 属性は、初期化された値を返す必要があります。オブジェクトの作成時に、この属性を空の文字列に初期化する必要があります。エラーが最初に発生したスクリプトの絶対 URL を表します。 |
| [IsTrusted](../../aspose.html.dom.events/event/istrusted/) { get; } | isTrusted 属性は、初期化された値を返す必要があります。イベントが作成されたら、属性を false. に初期化する必要があります。 |
| [LineNo](../../aspose.html.dom.events/errorevent/lineno/) { get; } | lineno 属性は、初期化された値を返す必要があります。オブジェクトの作成時に、この属性をゼロに初期化する必要があります。スクリプトでエラーが発生した行番号を表します. |
| [Message](../../aspose.html.dom.events/errorevent/message/) { get; } | メッセージ属性は、初期化された値を返す必要があります。オブジェクトの作成時に、この属性を空の文字列に初期化する必要があります。エラーメッセージを表します. |
