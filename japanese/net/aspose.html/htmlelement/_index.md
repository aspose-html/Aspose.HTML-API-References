...
description: Aspose.Html.HTMLElement クラス. すべての HTML 要素インターフェイスはこのクラスから派生します のみが HTML コア属性を公開する要素はベース で表されますHTML要素インターフェースこれらの要素は次のとおりです スペシャル SUBSUPSPANBDOフォント TTIBUSSTRIKEBIGSMALL フレーズ EMSTRONGDFNCODESAMPKBDVARCITE頭字語略語リスト DDDTNOFRAMESNOSCRIPTADDRESSCENTERTheスタイル HTML 要素の属性 は を通じてアクセスできます要素CSSInlineStyle CSS モジュールで定義されているインターフェイス DOM レベル 2 スタイル シートと CSS 
type: docs
weight: 3200
url: /ja/net/aspose.html/htmlelement/
---
## HTMLElement class

すべての HTML 要素インターフェイスは、このクラスから派生します。 のみが HTML コア属性を公開する要素は、ベース で表されます`HTML要素`インターフェース。これらの要素は次のとおりです: スペシャル: SUB、SUP、SPAN、BDOフォント: TT、I、B、U、S、STRIKE、BIG、SMALL フレーズ: EM、STRONG、DFN、CODE、SAMP、KBD、VAR、CITE、頭字語、略語リスト: DD、DTNOFRAMES、NOSCRIPTADDRESS、CENTERThe`スタイル` HTML 要素の属性 は、 を通じてアクセスできます。`要素CSSInlineStyle` CSS モジュールで定義されているインターフェイス [[DOM レベル 2 スタイル シートと CSS](http://www.w3.org/TR/2000/REC-DOM-Level-2-Style-20001113) ]。
