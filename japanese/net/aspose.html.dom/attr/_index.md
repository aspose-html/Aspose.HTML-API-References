...
description: Aspose.Html.Dom.Attr クラス. Attr インターフェイスはElement オブジェクトの属性を表します通常属性の許容値はドキュメントに関連付けられたスキーマで定義されます.
type: docs
weight: 110
url: /ja/net/aspose.html.dom/attr/
---
## Attr class
...
| [IsId](../../aspose.html.dom/attr/isid/) { get; } | この属性が ID 型 (つまり、所有者要素の識別子を含む) であることがわかっているかどうかを返します。 |
| [LastChild](../../aspose.html.dom/node/lastchild/) { get; } | このノードの最後の子。そのようなノードがない場合、これは null. を返します。 |
| override [LocalName](../../aspose.html.dom/attr/localname/) { get; } | このノードの修飾名のローカル部分を返します。 ELEMENT_NODE および ATTRIBUTE_NODE 以外のタイプのノード、および Document.createElement() などの DOM レベル 1 メソッドで作成されたノードの場合、これは常に null です。 |
| [Name](../../aspose.html.dom/attr/name/) { get; } | この属性の名前を返します。 |
| override [NamespaceURI](../../aspose.html.dom/attr/namespaceuri/) { get; } | このノードの名前空間 URI、または指定されていない場合は null. |
| [NextSibling](../../aspose.html.dom/node/nextsibling/) { get; } | このノードの直後のノード。そのようなノードがない場合、これは null. を返します。 |
| override [NodeName](../../aspose.html.dom/attr/nodename/) { get; } | タイプに応じたこのノードの名前. |
| override [NodeType](../../aspose.html.dom/attr/nodetype/) { get; } | 基礎となるオブジェクトのタイプを表すコード. |
| override [NodeValue](../../aspose.html.dom/attr/nodevalue/) { get; set; } | タイプに応じたこのノードの値. |
...
| [OwnerElement](../../aspose.html.dom/attr/ownerelement/) { get; } | このアトリビュートがアタッチされている Element ノード、またはこのアトリビュートが使用されていない場合は null。 |
...
| override [Prefix](../../aspose.html.dom/attr/prefix/) { get; } | このノードの名前空間プレフィックス、または指定されていない場合は null。 null と定義されている場合、設定しても効果はありません |
| [PreviousSibling](../../aspose.html.dom/node/previoussibling/) { get; } | このノードの直前のノード。そのようなノードがない場合、これは null. を返します。 |
| [SchemaTypeInfo](../../aspose.html.dom/attr/schematypeinfo/) { get; } | この属性に関連付けられたタイプ情報。 |
| [Specified](../../aspose.html.dom/attr/specified/) { get; } | この属性がインスタンス ドキュメントで明示的に値を与えられた場合は true、それ以外の場合は false. |
| override [TextContent](../../aspose.html.dom/attr/textcontent/) { get; set; } | この属性は、このノードとその子孫のテキスト コンテンツを返します。 null として定義されている場合、設定しても効果はありません。設定時に、このノードが持つ可能性のある子はすべて削除され、新しい文字列が空または null でない場合は、この属性が設定されている文字列を含む単一の Text ノードに置き換えられます。 |
| [Value](../../aspose.html.dom/attr/value/) { get; set; } | 取得時に、属性の値が文字列として返されます。 |
