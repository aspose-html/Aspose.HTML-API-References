...
description: "Metode IXPathExpression. Mengevaluasi ekspresi XPath ini dan mengembalikan hasil."
type: docs
...
| type | XPathResultType | Jika `type` tertentu ditentukan, maka hasil akan dipaksa untuk mengembalikan tipe yang ditentukan dengan mengandalkan konversi XPath dan gagal jika koersi yang diinginkan tidak memungkinkan. Ini harus menjadi salah satu nilai dari [`XPathResultType`](../../xpathresulttype/). |
| result | Object | `result` menentukan objek hasil tertentu yang dapat digunakan kembali dan dikembalikan oleh metode ini. Jika ini ditentukan sebagai `null` atau implementasi tidak menggunakan kembali hasil yang ditentukan, objek hasil baru akan dibuat dan dikembalikan. Untuk hasil XPath 1.0, objek ini akan berjenis [`IXPathResult`](../../ixpathresult/). |
