...
Filter adalah objek yang tahu cara "menyaring" node. Jika sebuah NodeIterator atau TreeWalker diberikan NodeFilter, ia menerapkan filter tersebut sebelum mengembalikan node berikutnya. Jika filter menyatakan untuk menerima node, logika penelusuran mengembalikannya; jika tidak, penelusuran mencari node berikutnya dan menganggap node yang ditolak tidak ada.

DOM tidak menyediakan filter apa pun. NodeFilter hanyalah antarmuka yang dapat diimplementasikan pengguna untuk menyediakan filter mereka sendiri.

NodeFilter tidak perlu mengetahui cara menelusuri dari node ke node, juga tidak perlu mengetahui apa pun tentang struktur data yang sedang ditelusuri. Hal ini membuat penulisan filter sangat mudah, karena satu-satunya hal yang harus mereka ketahui adalah cara mengevaluasi satu node. Satu filter dapat digunakan dengan sejumlah jenis penelusuran yang berbeda, mendorong penggunaan kembali kode.
