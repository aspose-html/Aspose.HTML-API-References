---
title: "MarkdownSyntaxFactory.Text"
second_title: "Referencia de la API de Aspose.HTML para Java"
description: "Método MarkdownSyntaxFactory. Crea TextSyntax"
type: docs

url: /es/java/com.aspose.html.toolkit.markdown.syntax/markdownsyntaxfactory/text/
---
## Text(String) {#text_2}

Crea TextSyntax

```java
public TextSyntaxNode Text(String text)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| texto | String | El valor de la cadena. |

### Valor de retorno

El TextSyntax.

### Ver también

* class [TextSyntaxNode](../../textsyntaxnode/)
* class [MarkdownSyntaxFactory](../)
* package [com.aspose.html.toolkit.markdown.syntax](../../../com.aspose.html.toolkit.markdown.syntax/)
* package [Aspose.HTML](../../../)

---

## Text(SourceText) {#text}

Crea el TextSyntax a partir de SourceText.

```java
public TextSyntaxNode Text(SourceText source)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| origen | SourceText | El SourceText. |

### Valor de retorno

El TextSyntax.

### Ver también

* class [TextSyntaxNode](../../textsyntaxnode/)
* class [SourceText](../../../com.aspose.html.toolkit.markdown.syntax.text/sourcetext/)
* class [MarkdownSyntaxFactory](../)
* package [com.aspose.html.toolkit.markdown.syntax](../../../com.aspose.html.toolkit.markdown.syntax/)
* package [Aspose.HTML](../../../)

---

## Text(SourceText, TextSpan) {#text_1}

Crea el TextSyntax a partir de SourceText y TextSpan.

```java
public TextSyntaxNode Text(SourceText source, TextSpan span)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| origen | SourceText | El SourceText. |
| span | TextSpan | El TextSpan. |

### Valor de retorno

El TextSyntax.

### Ver también

* class [TextSyntaxNode](../../textsyntaxnode/)
* class [SourceText](../../../com.aspose.html.toolkit.markdown.syntax.text/sourcetext/)
* struct [TextSpan](../../../com.aspose.html.toolkit.markdown.syntax.text/textspan/)
* class [MarkdownSyntaxFactory](../)
* package [com.aspose.html.toolkit.markdown.syntax](../../../com.aspose.html.toolkit.markdown.syntax/)
* package [Aspose.HTML](../../../)
