---
title: "Int32Array"
second_title: "Referencia de la API de Aspose.HTML para Java"
description: "Int32Array constructor. Inicializa una nueva instancia de la clase Int32Array"
type: docs

url: /es/java/com.aspose.html/int32array/int32array/
---
## Int32Array(int) {#constructor_3}

Inicializa una nueva instancia de la clase [`Int32Array`](../).

```java
public Int32Array(int length)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| longitud | Int32 | La longitud (número de elementos). |

### Ver también

* class [Int32Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Int32Array(int[]) {#constructor_4}

Inicializa una nueva instancia de la clase [`Int32Array`](../).

```java
public Int32Array(int[] data)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| datos | Int32[] | Los datos del arreglo. |

### Ver también

* class [Int32Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Int32Array(ArrayBuffer) {#constructor}

Inicializa una nueva instancia de la clase [`Int32Array`](../).

```java
public Int32Array(ArrayBuffer buffer)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| buffer | ArrayBuffer | El búfer de datos. |

### Ver también

* class [ArrayBuffer](../../arraybuffer/)
* class [Int32Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Int32Array(ArrayBuffer, int) {#constructor_1}

Inicializa una nueva instancia de la clase [`Int32Array`](../).

```java
public Int32Array(ArrayBuffer buffer, int byteOffset)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| buffer | ArrayBuffer | El búfer de datos. |
| byteOffset | Int32 | El desplazamiento de bytes. |

### Ver también

* class [ArrayBuffer](../../arraybuffer/)
* class [Int32Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Int32Array(ArrayBuffer, int, int) {#constructor_2}

Inicializa una nueva instancia de la clase [`Int32Array`](../).

```java
public Int32Array(ArrayBuffer buffer, int byteOffset, int length)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| buffer | ArrayBuffer | El búfer de datos. |
| byteOffset | Int32 | El desplazamiento de bytes. |
| longitud | Int32 | La longitud. |

### Ver también

* class [ArrayBuffer](../../arraybuffer/)
* class [Int32Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)
