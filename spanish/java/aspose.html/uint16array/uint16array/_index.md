---
title: "Uint16Array"
second_title: "Referencia de la API de Aspose.HTML para Java"
description: "Constructor Uint16Array. Inicializa una nueva instancia de la clase Uint16Array"
type: docs

url: /es/java/com.aspose.html/uint16array/uint16array/
---
## Uint16Array(int) {#constructor_3}

Inicializa una nueva instancia de la clase [`Uint16Array`](../).

```java
public Uint16Array(int length)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| longitud | Int32 | La longitud (número de elementos). |

### Ver también

* class [Uint16Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Uint16Array(ushort[]) {#constructor_4}

Inicializa una nueva instancia de la clase [`Uint16Array`](../).

```java
public Uint16Array(ushort[] data)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| datos | UInt16[] | Los datos del arreglo. |

### Ver también

* class [Uint16Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Uint16Array(ArrayBuffer) {#constructor}

Inicializa una nueva instancia de la clase [`Uint16Array`](../).

```java
public Uint16Array(ArrayBuffer buffer)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| buffer | ArrayBuffer | El buffer de datos. |

### Ver también

* class [ArrayBuffer](../../arraybuffer/)
* class [Uint16Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Uint16Array(ArrayBuffer, int) {#constructor_1}

Inicializa una nueva instancia de la clase [`Uint16Array`](../).

```java
public Uint16Array(ArrayBuffer buffer, int byteOffset)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| buffer | ArrayBuffer | El buffer de datos. |
| byteOffset | Int32 | El desplazamiento de bytes. |

### Ver también

* class [ArrayBuffer](../../arraybuffer/)
* class [Uint16Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Uint16Array(ArrayBuffer, int, int) {#constructor_2}

Inicializa una nueva instancia de la clase [`Uint16Array`](../).

```java
public Uint16Array(ArrayBuffer buffer, int byteOffset, int length)
```

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| buffer | ArrayBuffer | El buffer de datos. |
| byteOffset | Int32 | El desplazamiento de bytes. |
| longitud | Int32 | La longitud. |

### Ver también

* class [ArrayBuffer](../../arraybuffer/)
* class [Uint16Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)
