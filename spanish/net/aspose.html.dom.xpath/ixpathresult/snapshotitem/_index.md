...
description: Devuelve elíndice º elemento en la colección de instantáneas. Siíndicees mayor que o igual al número de nodos en la lista este método devuelvenulo . A diferencia del resultado del iterador  la instantánea no se vuelve inválida pero es posible que no se corresponda con el documento actual si está mutado.
type: docs
weight: 90
...
| excepción | condición |
| --- | --- |
| [DOMException](../../../aspose.html.dom/domexception) | TYPE_ERR: planteado si`tipo de resultado` no es `Instantánea de nodo desordenado` tipo o`Instantánea de nodo ordenado` escribe. |
