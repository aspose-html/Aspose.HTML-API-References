---
title: "Numeric.GetValue"
second_title: "Справочник API Aspose.HTML для Java"
description: "Метод Numeric. Возвращает значение единицы."
type: docs

url: /ru/java/com.aspose.html.drawing/numeric/getvalue/
---
## GetValue() {#getvalue}

Получает значение единицы.

```java
public double GetValue()
```

### Возвращаемое значение

Значение объекта.

### См. также

* class [Numeric](../)
* package [com.aspose.html.drawing](../../../com.aspose.html.drawing/)
* package [Aspose.HTML](../../../)

---

## GetValue(UnitType) {#getvalue_1}

Возвращает значение, преобразованное к указанному [`UnitType`](../../unittype/).

```java
public double GetValue(UnitType unitType)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| unitType | UnitType | Тип единицы. |

### Возвращаемое значение

Возвращает значение, преобразованное к указанному типу.

### Исключения

| исключение | условие |
| --- | --- |
| ArgumentNullException | unitType |

### См. также

* class [UnitType](../../unittype/)
* class [Numeric](../)
* package [com.aspose.html.drawing](../../../com.aspose.html.drawing/)
* package [Aspose.HTML](../../../)
