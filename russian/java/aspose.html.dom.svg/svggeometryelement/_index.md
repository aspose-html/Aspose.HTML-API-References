...
description: "com.aspose.html.dom.svg.SVGGeometryElement class. Интерфейс SVGGeometryElement представляет SVG‑элементы, рендеринг которых определяется геометрией с эквивалентным путём и которые могут быть заполнены и обведены. Это включает пути и базовые формы."
type: docs
