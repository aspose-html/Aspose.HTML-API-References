...
description: "com.aspose.html.dom.svg.SVGDocument class. SVGDocument является корнем иерархии SVG и содержит всё содержимое. Помимо предоставления доступа к иерархии, он также предлагает удобные методы для получения определённых наборов информации из документа. Когда элемент svg встроен в документ из другого пакета, например как встроенный элемент в документ XHTML, объект SVGDocument не будет существовать; вместо него корневым объектом в иерархии объектов документа будет объект Document другого типа, например HTMLDocument. Тем не менее объект SVGDocument будет существовать, когда корневым элементом иерархии XML‑документа является элемент svg, как при просмотре отдельного SVG‑файла (т.е. файла с MIME‑типом image/svg+xml). В этом случае объект SVGDocument будет корневым объектом иерархии модели объектов документа."
type: docs
...
Объект `SVGDocument` является корнем иерархии SVG и содержит всё содержимое. Помимо предоставления доступа к иерархии, он также предлагает некоторые удобные методы для доступа к определённым наборам информации из документа. Когда элемент ‘svg’ встроен непосредственно как компонент документа из другого пакета, например, когда элемент ‘svg’ встроен в документ XHTML [XHTML], объект SVGDocument не будет существовать; вместо этого корневым объектом в иерархии объектов документа будет объект Document другого типа, например объект HTMLDocument. Тем не менее объект SVGDocument действительно будет существовать, когда корневым элементом иерархии XML‑документа является элемент ‘svg’, например при просмотре отдельного SVG‑файла (т.е. файла с MIME‑типом \"image/svg+xml\"). В этом случае объект SVGDocument будет корневым объектом иерархии модели объектов документа.
...
| [getDomain](../../com.aspose.html.dom.svg/svgdocument/domain/) Имя домена сервера, который предоставил документ, или строка null, если сервер нельзя идентифицировать по имени домена. |
...
| [getRootElement](../../com.aspose.html.dom.svg/svgdocument/rootelement/) Корневой элемент ‘svg’ в иерархии документа. |
...
| [getTitle](../../com.aspose.html.dom.svg/svgdocument/title/) Заголовок документа, указанный в дочернем элементе ‘title’ корневого элемента ‘svg’ (например, Here is the title...) |
...
| [getOverrideStyle](../../com.aspose.html.dom.svg/svgdocument/getoverridestyle/)(Element, String) | Этот метод используется для получения объявления переопределяющего стиля для указанного элемента и указанного псевдоэлемента. |
...
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save)(ResourceHandler) | Сохраняет содержимое документа и ресурсы с использованием [`ResourceHandler`](../../com.aspose.html.saving.resourcehandlers/resourcehandler/). |
...
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save_1)(ResourceHandler, SVGSaveFormat) | Сохраняет содержимое документа и ресурсы с использованием [`ResourceHandler`](../../com.aspose.html.saving.resourcehandlers/resourcehandler/). |
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save_2)(ResourceHandler, SVGSaveOptions) | Сохраняет содержимое документа и ресурсы с использованием [`ResourceHandler`](../../com.aspose.html.saving.resourcehandlers/resourcehandler/). |
