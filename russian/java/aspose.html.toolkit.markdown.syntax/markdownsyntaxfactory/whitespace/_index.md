---
title: "MarkdownSyntaxFactory.Whitespace"
second_title: "Справочник API Aspose.HTML для Java"
description: "Метод MarkdownSyntaxFactory. Создает WhitespaceSyntax"
type: docs

url: /ru/java/com.aspose.html.toolkit.markdown.syntax/markdownsyntaxfactory/whitespace/
---
## Whitespace() {#whitespace}

Создайте WhitespaceSyntax.

```java
public WhitespaceSyntaxNode Whitespace()
```

### Возвращаемое значение

WhitespaceSyntax.

### См. также

* class [WhitespaceSyntaxNode](../../whitespacesyntaxnode/)
* class [MarkdownSyntaxFactory](../)
* package [com.aspose.html.toolkit.markdown.syntax](../../../com.aspose.html.toolkit.markdown.syntax/)
* package [Aspose.HTML](../../../)

---

## Whitespace(int) {#whitespace_3}

Создайте WhitespaceSyntax.

```java
public WhitespaceSyntaxNode Whitespace(int count)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| количество | Int32 | Количество пробельных символов. |

### Возвращаемое значение

WhitespaceSyntax.

### См. также

* class [WhitespaceSyntaxNode](../../whitespacesyntaxnode/)
* class [MarkdownSyntaxFactory](../)
* package [com.aspose.html.toolkit.markdown.syntax](../../../com.aspose.html.toolkit.markdown.syntax/)
* package [Aspose.HTML](../../../)

---

## Whitespace(SourceText) {#whitespace_1}

Создайте WhitespaceSyntax.

```java
public WhitespaceSyntaxNode Whitespace(SourceText source)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| источник | SourceText | Текст SourceText. |

### Возвращаемое значение

WhitespaceSyntax.

### См. также

* class [WhitespaceSyntaxNode](../../whitespacesyntaxnode/)
* class [SourceText](../../../com.aspose.html.toolkit.markdown.syntax.text/sourcetext/)
* class [MarkdownSyntaxFactory](../)
* package [com.aspose.html.toolkit.markdown.syntax](../../../com.aspose.html.toolkit.markdown.syntax/)
* package [Aspose.HTML](../../../)

---

## Whitespace(SourceText, TextSpan) {#whitespace_2}

Создаёт [`WhitespaceSyntaxNode`](../../whitespacesyntaxnode/).

```java
public WhitespaceSyntaxNode Whitespace(SourceText source, TextSpan span)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| источник | SourceText | Текст SourceText. |
| span | TextSpan | Текст TextSpan. |

### Возвращаемое значение

WhitespaceSyntax.

### См. также

* class [WhitespaceSyntaxNode](../../whitespacesyntaxnode/)
* class [SourceText](../../../com.aspose.html.toolkit.markdown.syntax.text/sourcetext/)
* struct [TextSpan](../../../com.aspose.html.toolkit.markdown.syntax.text/textspan/)
* class [MarkdownSyntaxFactory](../)
* package [com.aspose.html.toolkit.markdown.syntax](../../../com.aspose.html.toolkit.markdown.syntax/)
* package [Aspose.HTML](../../../)
