...
description: "Класс com.aspose.html.HTMLElement. Все интерфейсы HTML‑элементов наследуются от этого класса. Элементы, которые раскрывают только базовые атрибуты HTML, представлены базовым интерфейсом HTMLElement. К этим элементам относятся специальные SUB SUP SPAN BDO font TT I B U S STRIKE BIG SMALL phrase EM STRONG DFN CODE SAMP KBD VAR CITE ACRONYM ABBR list DD DT NOFRAMES NOSCRIPT ADDRESS CENTER. Атрибут style HTML‑элемента доступен через интерфейс ElementCSSInlineStyle, который определён в модуле CSS DOM Level 2 Style Sheets and CSS"
type: docs
