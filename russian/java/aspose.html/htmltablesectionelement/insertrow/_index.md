...
Вставить строку в этот раздел. Новая строка вставляется непосредственно перед текущей строкой с индексом `index` в этом разделе. Если `index` равен -1 или равен количеству строк в этом разделе, новая строка добавляется в конец.
...
| --- | --- | --- |
| index | Int32 | Номер строки, в которую нужно вставить новую строку. Этот индекс начинается с 0 и относится только к строкам, содержащимся в этом разделе, а не ко всем строкам в таблице. |
