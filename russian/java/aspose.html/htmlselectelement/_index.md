...
description: "Класс com.aspose.html.HTMLSelectElement. Элемент select позволяет выбирать вариант. Содержимые варианты могут быть напрямую доступны через элемент select как коллекция. См. определение элемента SELECT в HTML 4.01"
type: docs
...
Элемент select позволяет выбирать вариант. Содержимые варианты можно напрямую получить через элемент select как коллекцию. См. определение элемента SELECT в HTML 4.01.
...
| [getType](../../com.aspose.html/htmlselectelement/type/) Тип этого элемента управления формой. Это строка \"select-multiple\", когда атрибут multiple имеет значение `true`, и строка \"select-one\", когда `false`. |
[getValue]
