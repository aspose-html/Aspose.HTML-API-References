...
description: "Класс com.aspose.html.dom.DOMException. Интерфейс DOMException представляет аномальное событие, называемое исключением, которое возникает в результате вызова метода или доступа к свойству веб‑API. По сути, так описываются условия ошибок в веб‑API."
type: docs
...
| const [INUSE_ATTRIBUTE_ERR](../../com.aspose.html.dom/domexception/inuse_attribute_err/) | Если попытка добавить атрибут, уже используемый в другом месте. |
...
| const [INVALID_CHARACTER_ERR](../../com.aspose.html.dom/domexception/invalid_character_err/) | Если указано недопустимое или незаконное символ, например в имени XML. |
...
| const [INVALID_NODE_TYPE_ERR](../../com.aspose.html.dom/domexception/invalid_node_type_err/) | Переданный узел некорректен или имеет неправильного предка для этой операции. |
| const [INVALID_STATE_ERR](../../com.aspose.html.dom/domexception/invalid_state_err/) | Если попытка использовать объект, который не является, или более не является, пригодным. |
| const [NAMESPACE_ERR](../../com.aspose.html.dom/domexception/package_err/) | Если попытка создать или изменить объект способом, который некорректен с точки зрения пакетов. |
| const [NETWORK_ERR](../../com.aspose.html.dom/domexception/network_err/) | Произошла сетевая ошибка. |
| const [NOT_FOUND_ERR](../../com.aspose.html.dom/domexception/not_found_err/) | Если попытка сослаться на узел (Node) в контексте, где он не существует. |
...
| const [NO_DATA_ALLOWED_ERR](../../com.aspose.html.dom/domexception/no_data_allowed_err/) | Если для узла (Node) указаны данные, но он не поддерживает данные. |
| const [NO_MODIFICATION_ALLOWED_ERR](../../com.aspose.html.dom/domexception/no_modification_allowed_err/) | Если попытка изменить объект, где изменения не разрешены. |
| const [QUOTA_EXCEEDED_ERR](../../com.aspose.html.dom/domexception/quota_exceeded_err/) | Превышена квота. |
...
| const [TYPE_ERR](../../com.aspose.html.dom/domexception/type_err/) | Выражение нельзя преобразовать к возвращаемому указанному типу. |
