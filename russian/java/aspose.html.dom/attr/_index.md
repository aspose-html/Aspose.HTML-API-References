...
| [getLocalName](../../com.aspose.html.dom/attr/localname/) Возвращает локальную часть квалифицированного имени этого узла. Для узлов любого типа, кроме ELEMENT_NODE и ATTRIBUTE_NODE, и узлов, созданных методом уровня DOM 1, например Document.createElement(), всегда возвращается null. |
...
| [getOwnerElement](../../com.aspose.html.dom/attr/ownerelement/) Узел Element, к которому прикреплён этот атрибут, или null, если атрибут не используется. |
...
| [getPrefix](../../com.aspose.html.dom/attr/prefix/) Префикс пакета этого узла, или null, если не указан. Когда он определён как null, установка значения не имеет эффекта. |
...
| [textContent](../../com.aspose.html.dom/attr/textcontent/) { get; set; } | Этот атрибут возвращает текстовое содержимое этого узла и его потомков. Если он задан как null, попытка установить его не оказывает эффекта. При установке все возможные дочерние узлы удаляются, и если новая строка не пуста и не null, она заменяется одним текстовым узлом, содержащим эту строку. |
[getValue]
