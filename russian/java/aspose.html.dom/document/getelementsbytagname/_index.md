...
Выполняется поиск по всему документу, включая корневой узел. Возвращаемый [`HTMLCollection`](../../../com.aspose.html.collections/htmlcollection/) является живым, то есть автоматически обновляется, чтобы оставаться синхронным с деревом DOM без необходимости повторно вызывать document.getElementsByTagName().
