...
description: "Document метод. Создает новый NodeIterator над поддеревом, корнем которого является указанный узел"
type: docs
...
Создайте новый NodeIterator над поддеревом, корнем которого является указанный узел.
...
| --- | --- | --- |
| корень | Node | Узел, который будет итерироваться вместе со своими дочерними элементами. Итератор изначально позиционируется сразу перед этим узлом. Флаги whatToShow и фильтр, если они заданы, не учитываются при установке этой позиции. Корень не должен быть null. |
