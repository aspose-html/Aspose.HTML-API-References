...
description: "Класс com.aspose.html.dom.events.Event. Он используется для предоставления контекстной информации о событии обработчику, который обрабатывает событие."
type: docs
...
| const [CapturingPhase](../../com.aspose.html.dom.events/event/capturingphase/) | Событие в данный момент оценивается у целевого [`IEventTarget`](../ieventtarget/). |
...
Объект, реализующий этот интерфейс, обычно передаётся в качестве первого параметра обработчику события. Более конкретная контекстная информация передаётся обработчикам событий путём наследования дополнительных интерфейсов, которые содержат информацию, непосредственно относящуюся к типу сопровождающего их события. Эти производные интерфейсы также реализуются объектом, передаваемым слушателю события.
