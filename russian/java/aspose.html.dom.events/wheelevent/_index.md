...
description: "Класс com.aspose.html.dom.events.WheelEvent. Интерфейс WheelEvent предоставляет специфическую контекстную информацию, связанную с событиями колеса (wheel events). Чтобы создать экземпляр интерфейса WheelEvent, используйте конструктор WheelEvent, передавая необязательный словарь WheelEventInit."
type: docs
...
| [getDeltaX](../../com.aspose.html.dom.events/wheelevent/deltax/) В пользовательских агентах, где действие события колеса по умолчанию — прокрутка, значение ДОЛЖНО представлять измерение вдоль оси x (в пикселях, строках или страницах), которое будет прокручено, если событие не отменено. В противном случае это измерение, специфичное для реализации (в пикселях, строках или страницах) движения устройства колеса вокруг оси x. |
| [getDeltaY](../../com.aspose.html.dom.events/wheelevent/deltay/) В пользовательских агентах, где действие события колеса по умолчанию — прокрутка, значение ДОЛЖНО представлять измерение вдоль оси y (в пикселях, строках или страницах), которое будет прокручено, если событие не отменено. В противном случае это измерение, специфичное для реализации (в пикселях, строках или страницах) движения устройства колеса вокруг оси y. |
| [getDeltaZ](../../com.aspose.html.dom.events/wheelevent/deltaz/) В пользовательских агентах, где действие события колеса по умолчанию — прокрутка, значение ДОЛЖНО представлять измерение вдоль оси z (в пикселях, строках или страницах), которое будет прокручено, если событие не отменено. В противном случае это измерение, специфичное для реализации (в пикселях, строках или страницах) движения устройства колеса вокруг оси z. |
