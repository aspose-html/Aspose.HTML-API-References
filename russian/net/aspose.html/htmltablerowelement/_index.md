...
description: Строка в таблице. См. определение элемента TR в HTML 4.01.
type: docs
weight: 3660
...
| [Cells](../../aspose.html/htmltablerowelement/cells) { get; } | Набор ячеек в этой строке. @version DOM Level 2 |
...
| [RowIndex](../../aspose.html/htmltablerowelement/rowindex) { get; } | Это в логическом порядке, а не в порядке документа. ` rowIndex` действительно учитывает разделы ( ` THEAD` ,` TFOOT` , или` TBODY` ) внутри таблица, размещение` THEAD` строк первыми в индексе, за которыми следуют` TBODY` строк, за которыми следуют` TFOOT` строки. @version DOM Level 2 |
| [SchemaTypeInfo](../../aspose.html.dom/element/schematypeinfo) { get; } | Информация о типе, связанная с этим элементом. |
| [SectionRowIndex](../../aspose.html/htmltablerowelement/sectionrowindex) { get; } | Индекс этой строки относительно текущего раздела ( ` THEAD` ,` TFOOT` , или` TBODY` ), начиная с 0 . @version DOM Level 2 |
...
| [VAlign](../../aspose.html/htmltablerowelement/valign) { get; set; } | Вертикальное выравнивание данных внутри ячеек этой строки. См. определение атрибута valign в HTML 4.01. |
...
| [InsertCell](../../aspose.html/htmltablerowelement/insertcell)(int) | Вставьте пустую ячейку` TD` в эту строку. Если ` index` равно -1 или равно количеству ячеек, добавляется новая ячейка . |
