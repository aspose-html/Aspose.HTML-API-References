---
title: Class Page
second_title: Справочник по Aspose.HTML для .NET API
description: Aspose.Html.Drawing.Page сорт. Представляет объект страницы используемый для страницы вывода конфигурации. Значение пробела для размера страницы A4 210x297 мм
type: docs
weight: 2810
url: /ru/net/aspose.html.drawing/page/
---
## Page class

Представляет объект страницы, используемый для страницы вывода конфигурации. Значение пробела для размера страницы: A4 (210x297 мм)

```csharp
public class Page
```

## Конструкторы

| Имя | Описание |
| --- | --- |
| [Page](page/#constructor)() | Инициализирует новый экземпляр`Page` класс. |
| [Page](page/#constructor_1)(Margin) | Инициализирует новый экземпляр`Page` класс. |
| [Page](page/#constructor_2)(Size) | Инициализирует новый экземпляр`Page` класс. |
| [Page](page/#constructor_3)(Size, Margin) | Инициализирует новый экземпляр`Page` класс. |

## Характеристики

| Имя | Описание |
| --- | --- |
| [Margin](../../aspose.html.drawing/page/margin/) { get; set; } | Получает или задает поля страницы. |
| [Size](../../aspose.html.drawing/page/size/) { get; set; } | Получает или задает размер страницы. |

### Смотрите также

* пространство имен [Aspose.Html.Drawing](../../aspose.html.drawing/)
* сборка [Aspose.HTML](../../)


