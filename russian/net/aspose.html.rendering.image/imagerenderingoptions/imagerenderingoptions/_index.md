---
title: ImageRenderingOptions
second_title: Справочник по Aspose.HTML для .NET API
description: Инициализирует новый экземпляр классаImageRenderingOptionsaspose.html.rendering.image/imagerenderingoptionsPngбудет использоваться как формат изображения по умолчанию.
type: docs
weight: 10
url: /ru/net/aspose.html.rendering.image/imagerenderingoptions/imagerenderingoptions/
---
## ImageRenderingOptions() {#constructor}

Инициализирует новый экземпляр класса[`ImageRenderingOptions`](../../imagerenderingoptions);Pngбудет использоваться как формат изображения по умолчанию.

```csharp
public ImageRenderingOptions()
```

### Смотрите также

* class [ImageRenderingOptions](../../imagerenderingoptions)
* пространство имен [Aspose.Html.Rendering.Image](../../imagerenderingoptions)
* сборка [Aspose.HTML](../../../)

---

## ImageRenderingOptions(ImageFormat) {#constructor_1}

Инициализирует новый экземпляр класса[`ImageRenderingOptions`](../../imagerenderingoptions)с указанным форматом изображения.

```csharp
public ImageRenderingOptions(ImageFormat format)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| format | ImageFormat | Выходной формат изображения. |

### Смотрите также

* enum [ImageFormat](../../imageformat)
* class [ImageRenderingOptions](../../imagerenderingoptions)
* пространство имен [Aspose.Html.Rendering.Image](../../imagerenderingoptions)
* сборка [Aspose.HTML](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.HTML.dll -->
