...
description: "Node-Methode. Die appendChild-Methode des Node-Interfaces fügt einen Knoten am Ende der Kindliste eines angegebenen Elternknotens hinzu. Wenn das angegebene Kind eine Referenz zu einem bereits im Dokument vorhandenen Knoten ist, verschiebt appendChild ihn von seiner aktuellen Position zur neuen Position; es ist nicht erforderlich, den Knoten vor dem Anhängen an einen anderen Knoten aus seinem Elternknoten zu entfernen."
type: docs
...
Das bedeutet, dass ein Knoten nicht gleichzeitig an zwei Stellen im Dokument sein kann. Hat der Knoten bereits einen Elternknoten, wird er zuerst entfernt und dann an der neuen Position angehängt. Die [`Node.cloneNode()`](../clonenode/)-Methode kann verwendet werden, um vor dem Anhängen unter dem neuen Elternknoten eine Kopie des Knotens zu erstellen. Mit [`cloneNode`](../clonenode/) erstellte Kopien werden nicht automatisch synchron gehalten.
