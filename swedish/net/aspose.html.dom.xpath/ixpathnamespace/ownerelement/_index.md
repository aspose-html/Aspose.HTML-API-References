---
title: IXPathNamespace.OwnerElement
second_title: Aspose.HTML för .NET API Referens
description: IXPathNamespace fast egendom. DenElementdär namnområdet var i omfattning när det begärdes. Detta ändras inte på en returnerad namnområdesnod även om dokumentet ändras så att namnområdet går utanför räckvidden på det elementet och denna nod inte längre hittas där av XPath.
type: docs
weight: 10
url: /sv/net/aspose.html.dom.xpath/ixpathnamespace/ownerelement/
---
## IXPathNamespace.OwnerElement property

Den[`Element`](../../../aspose.html.dom/element/)där namnområdet var i omfattning när det begärdes. Detta ändras inte på en returnerad namnområdesnod även om dokumentet ändras så att namnområdet går utanför räckvidden på det elementet och denna nod inte längre hittas där av XPath.

```csharp
public Element OwnerElement { get; }
```

### Se även

* class [Element](../../../aspose.html.dom/element/)
* interface [IXPathNamespace](../)
* namnutrymme [Aspose.Html.Dom.XPath](../../ixpathnamespace/)
* hopsättning [Aspose.HTML](../../../)


