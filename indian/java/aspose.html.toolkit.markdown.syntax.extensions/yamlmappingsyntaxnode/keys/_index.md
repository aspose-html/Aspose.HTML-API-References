---
title: "YamlMappingSyntaxNode.Keys"
second_title: "Aspose.HTML Java के लिए API संदर्भ"
description: "YamlMappingSyntaxNode प्रॉपर्टी। सभी कुंजियों को प्राप्त करें"
type: docs

url: /hi/java/com.aspose.html.toolkit.markdown.syntax.extensions/yamlmappingsyntaxnode/keys/
---
## YamlMappingSyntaxNode.Keys property

सभी कुंजियों को प्राप्त करें।

```java
public IEnumerable<ChildFrontMatterSyntaxNode> Keys { get; }
```

### संबंधित देखें

* class [ChildFrontMatterSyntaxNode](../../childfrontmattersyntaxnode/)
* class [YamlMappingSyntaxNode](../)
* package [com.aspose.html.toolkit.markdown.syntax.extensions](../../../com.aspose.html.toolkit.markdown.syntax.extensions/)
* package [Aspose.HTML](../../../)
