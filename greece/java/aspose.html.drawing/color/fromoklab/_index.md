---
title: "Color.FromOklab"
second_title: "Αναφορά API του Aspose.HTML για Java"
description: "Μέθοδος Color. Επιστρέφει ένα νέο Color με τις ζητούμενες τιμές φωτεινότητας A B για το μοντέλο OKLAB."
type: docs

url: /el/java/com.aspose.html.drawing/color/fromoklab/
---
## Color.FromOklab method

Επιστρέφει ένα νέο Color με τις ζητούμενες τιμές lightness, A, B για το μοντέλο OKLAB.

```java
public static Color FromOklab(float lightness, float a, float b)
```

| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| φωτεινότητα | Single | Ένα float που αντιπροσωπεύει το συστατικό φωτεινότητας του χρώματος. |
| a | Single | Ένα float που αντιπροσωπεύει το συστατικό A του χρώματος. |
| b | Single | Ένα float που αντιπροσωπεύει το συστατικό B του χρώματος. |

### Τιμή Επιστροφής

Μια νέα παρουσία της κλάσης [`Color`](../) class

### Δείτε επίσης

* class [Color](../)
* package [com.aspose.html.drawing](../../../com.aspose.html.drawing/)
* package [Aspose.HTML](../../../)
