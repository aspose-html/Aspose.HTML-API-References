---
title: "ImageDevice"
second_title: "Αναφορά API του Aspose.HTML για Java"
description: "Κατασκευαστής ImageDevice. Αρχικοποιεί μια νέα παρουσία της κλάσης ImageDevice"
type: docs

url: /el/java/com.aspose.html.rendering.image/imagedevice/imagedevice/
---
## ImageDevice(ICreateStreamProvider) {#constructor}

Αρχικοποιεί μια νέα παρουσία της κλάσης [`ImageDevice`](../).

```java
public ImageDevice(ICreateStreamProvider streamProvider)
```

| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| streamProvider | ICreateStreamProvider | Ο πάροχος ροής. |

### Δείτε επίσης

* interface [ICreateStreamProvider](../../../com.aspose.html.io/icreatestreamprovider/)
* class [ImageDevice](../)
* package [com.aspose.html.rendering.image](../../../com.aspose.html.rendering.image/)
* package [Aspose.HTML](../../../)

---

## ImageDevice(ImageRenderingOptions, ICreateStreamProvider) {#constructor_1}

Αρχικοποιεί μια νέα παρουσία της κλάσης [`ImageDevice`](../) με επιλογές απόδοσης και πάροχο ροής.

```java
public ImageDevice(ImageRenderingOptions options, ICreateStreamProvider streamProvider)
```

| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| options | ImageRenderingOptions | Επιλογές απόδοσης. |
| streamProvider | ICreateStreamProvider | Αντικείμενο που υλοποιεί τη διεπαφή [`ICreateStreamProvider`](../../../com.aspose.html.io/icreatestreamprovider/) |

### Δείτε επίσης

* class [ImageRenderingOptions](../../imagerenderingoptions/)
* interface [ICreateStreamProvider](../../../com.aspose.html.io/icreatestreamprovider/)
* class [ImageDevice](../)
* package [com.aspose.html.rendering.image](../../../com.aspose.html.rendering.image/)
* package [Aspose.HTML](../../../)

---

## ImageDevice(String) {#constructor_5}

Αρχικοποιεί μια νέα παρουσία της κλάσης [`ImageDevice`](../).

```java
public ImageDevice(String file)
```

| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| αρχείο | String | Το όνομα του αρχείου εξόδου. |

### Δείτε επίσης

* class [ImageDevice](../)
* package [com.aspose.html.rendering.image](../../../com.aspose.html.rendering.image/)
* package [Aspose.HTML](../../../)

---

## ImageDevice(ImageRenderingOptions, String) {#constructor_3}

Αρχικοποιεί μια νέα παρουσία της κλάσης [`ImageDevice`](../) με επιλογές απόδοσης και όνομα αρχείου εξόδου.

```java
public ImageDevice(ImageRenderingOptions options, String file)
```

| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| options | ImageRenderingOptions | Επιλογές απόδοσης. |
| αρχείο | String | Το όνομα του αρχείου εξόδου. |

### Δείτε επίσης

* class [ImageRenderingOptions](../../imagerenderingoptions/)
* class [ImageDevice](../)
* package [com.aspose.html.rendering.image](../../../com.aspose.html.rendering.image/)
* package [Aspose.HTML](../../../)

---

## ImageDevice(Stream) {#constructor_4}

Αρχικοποιεί μια νέα παρουσία της κλάσης [`ImageDevice`](../).

```java
public ImageDevice(Stream stream)
```

| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ροή | Ροή | Η ροή. |

### Δείτε επίσης

* class [ImageDevice](../)
* package [com.aspose.html.rendering.image](../../../com.aspose.html.rendering.image/)
* package [Aspose.HTML](../../../)

---

## ImageDevice(ImageRenderingOptions, Stream) {#constructor_2}

Αρχικοποιεί μια νέα παρουσία της κλάσης [`ImageDevice`](../) με επιλογές απόδοσης και ροή εξόδου.

```java
public ImageDevice(ImageRenderingOptions options, Stream stream)
```

| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| options | ImageRenderingOptions | Επιλογές απόδοσης. |
| ροή | Ροή | Η ροή εξόδου. |

### Δείτε επίσης

* class [ImageRenderingOptions](../../imagerenderingoptions/)
* class [ImageDevice](../)
* package [com.aspose.html.rendering.image](../../../com.aspose.html.rendering.image/)
* package [Aspose.HTML](../../../)
