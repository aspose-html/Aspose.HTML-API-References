...
description: "الفئة com.aspose.html.dom.ProcessingInstruction. تمثل ProcessingInstruction تعليمًا للمعالجة يُستخدم في XML كطريقة للحفاظ على معلومات خاصة بالمُعالج في نص المستند."
type: docs
...
تمثل ProcessingInstruction "تعليمات المعالجة"، وتُستخدم في XML كطريقة للحفاظ على معلومات خاصة بالمعالج داخل نص المستند.
...
| [getTarget](../../com.aspose.html.dom/processinginstruction/target/) الهدف من هذه التعليمة المعالجة. |
...
| [normalize](../../com.aspose.html.dom/node/normalize/)() | يضع جميع عقد [`Text`](../text/) في العمق الكامل للشجرة الفرعية تحت هذه العقدة، بما في ذلك عقد السمات، في شكل \"عادي\" حيث يفصل فقط الهيكل (مثلًا، [`elements`](../element/)، [`comments`](../comment/)، `processing instructions`، [`CDATA sections`](../cdatasection/)، و[`entity references`](../entityreference/)) عقد [`Text`](../text/)، أي لا توجد عقد نصية متجاورة ولا عقد نصية فارغة. يمكن استخدام ذلك لضمان أن عرض DOM للوثيقة هو نفسه كما لو تم حفظها وإعادة تحميلها، وهو مفيد عندما تُستخدم عمليات (مثل عمليات البحث XPointer [XPointer]) التي تعتمد على بنية شجرة وثيقة معينة. |
