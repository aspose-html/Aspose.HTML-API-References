...
| الاسم | الوصف |
| --- | --- |
| [DocDevice](docdevice/#constructor)(ICreateStreamProvider) | ينشئ مثيلاً جديدًا من الفئة `DocDevice`. |
| [DocDevice](docdevice/#constructor_4)(Stream) | ينشئ مثيلاً جديدًا من الفئة `DocDevice` باستخدام تدفق الإخراج. |
| [DocDevice](docdevice/#constructor_5)(String) | ينشئ مثيلاً جديدًا من الفئة `DocDevice` باستخدام اسم ملف الإخراج. |
| [DocDevice](docdevice/#constructor_1)(DocRenderingOptions, ICreateStreamProvider) | ينشئ مثيلاً جديدًا من الفئة `DocDevice` باستخدام خيارات التصيير ومزود التدفق. |
| [DocDevice](docdevice/#constructor_2)(DocRenderingOptions, Stream) | ينشئ مثيلاً جديدًا من الفئة `DocDevice` باستخدام خيارات التصيير وتدفق الإخراج. |
| [DocDevice](docdevice/#constructor_3)(DocRenderingOptions, String) | ينشئ مثيلاً جديدًا من الفئة `DocDevice` باستخدام خيارات التصيير واسم ملف الإخراج. |
...
| [clip](../../com.aspose.html.rendering.doc/docdevice/clip/)(FillRule) | يعدل مسار القص الحالي عن طريق تقاطعه مع المسار الحالي، باستخدام قاعدة FillMode لتحديد المنطقة التي يجب ملؤها. هذه الطريقة تنهي المسار الحالي. |
| [closePath](../../com.aspose.html.rendering.doc/docdevice/closepath/)() | يغلق المسار الفرعي الحالي بإضافة قطعة خط مستقيم من النقطة الحالية إلى نقطة بدء المسار الفرعي. إذا كان المسار الفرعي الحالي مغلقًا بالفعل، فإن "ClosePath" لا يفعل شيئًا. هذا المشغل ينهي المسار الفرعي الحالي. إضافة قطعة أخرى إلى المسار الحالي يبدأ مسارًا فرعيًا جديدًا، حتى وإن بدأت القطعة الجديدة عند نقطة النهاية التي وصل إليها أسلوب "ClosePath". |
...
| [fill](../../com.aspose.html.rendering.doc/docdevice/fill/)(FillRule) | يملى المنطقة الكاملة المحاطة بالمسار الحالي. إذا كان المسار يتكون من عدة مسارات فرعية غير متصلة، فإنه يملأ داخل جميع المسارات الفرعية معًا. هذه الطريقة تنهي المسار الحالي. |
| [fillText](../../com.aspose.html.rendering.doc/docdevice/filltext/)(String, PointF) | يملى سلسلة النص المحددة في الموقع المحدد. |
...
| [moveTo](../../com.aspose.html.rendering.doc/docdevice/moveto/)(PointF) | يبدأ مسارًا فرعيًا جديدًا بنقل النقطة الحالية إلى إحداثيات المعامل pt، متجنبًا أي قطعة خطية ربطية. إذا كانت طريقة بناء المسار السابقة في المسار الحالي هي أيضًا "MoveTo"، فإن "MoveTo" الجديد يتجاوزها؛ لا يبقى أي أثر لعملية "MoveTo" السابقة في المسار. |
...
| [stroke](../../com.aspose.html.rendering.doc/docdevice/stroke/)() | يرسم خطًا على طول المسار الحالي. يتبع الخط المرسوم كل قطعة مستقيمة أو منحنية في المسار، مركزيًا على القطعة مع جوانب موازية لها. يتم معالجة كل مسار فرعي للمسار بشكل منفصل. هذه الطريقة تنهي المسار الحالي. |
...
| الاسم | الوصف |
| --- | --- |
| class [DocGraphicContext](../../com.aspose.html.rendering.doc/docdevice.docgraphiccontext) | يحتفظ بمعلمات التحكم الرسومية الحالية لجهاز DocDevice. تحدد هذه المعلمات الإطار العام الذي تُنفّذ فيه عمليات الرسوميات. |
