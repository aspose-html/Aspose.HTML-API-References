---
title: "SVGTextContentElement 클래스"
second_title: "Aspose.HTML for Java API 참조"
description: "com.aspose.html.dom.svg.SVGTextContentElement 클래스. SVGTextContentElement는 SVGTextElement, SVGTSpanElement, SVGTRefElement, SVGAltGlyphElement 및 SVGTextPathElement와 같은 다양한 텍스트 관련 인터페이스에서 상속됩니다. 이 인터페이스의 메서드가 문자 인덱스 또는 문자 수를 참조할 경우, 해당 참조는 각각 UTF-16 코드 단위 인덱스 또는 UTF-16 코드 단위 수로 해석됩니다. 이는 DOM Level 2 Core와 일치하도록, CharacterData 인터페이스의 메서드가 문자 데이터 내에서 인덱스와 개수에 UTF-16 코드 단위를 사용하기 때문입니다. 예를 들어 텍스트 요소의 내용이 U10000과 같은 단일 비BMP 문자일 경우, 해당 요소에서 getNumberOfChars를 호출하면 2가 반환됩니다. 이는 하나의 문자를 표현하기 위해 사용되는 두 개의 UTF-16 코드 단위(서로게이트 쌍) 때문입니다."
type: docs

url: /ko/java/com.aspose.html.dom.svg/svgtextcontentelement/
---
...
| [getLengthAdjust](../../com.aspose.html.dom.svg/svgtextcontentelement/lengthadjust/) 주어진 요소의 ‘lengthAdjust’ 속성에 해당합니다. 값은 이 인터페이스에 정의된 length adjust 상수 중 하나여야 합니다. |
...
| [getTextLength](../../com.aspose.html.dom.svg/svgtextcontentelement/textlength/) 주어진 요소의 ‘textLength’ 속성에 해당합니다. |
...
| [getComputedTextLength](../../com.aspose.html.dom.svg/svgtextcontentelement/getcomputedtextlength/)() | 이 요소 내의 모든 문자들을 렌더링하면서 발생하는 전체 전진값의 합계이며, 글리프(수평 또는 수직)의 전진값, ‘kerning’, ‘letter-spacing’, ‘word-spacing’ 속성의 효과, 그리고 ‘tspan’ 요소의 ‘dx’ 및 ‘dy’ 속성에 의한 조정이 포함됩니다. 렌더링이 이루어지지 않는 환경에서는 사용자 에이전트가 글리프 메트릭에 대해 합리적인 가정을 해야 합니다. |
...
| [getNumberOfChars](../../com.aspose.html.dom.svg/svgtextcontentelement/getnumberofchars/)() | 현재 요소 내에서 렌더링 가능한 전체 문자 수를 반환합니다. 여기에는 렌더링 여부와 관계없이 ‘tref’ 참조에서 가져온 문자도 포함됩니다. 실제로 이는 DOM Level 3 Core([DOM3], 섹션 1.4)의 Node::textContent 속성 길이와 동일하며, 해당 속성이 ‘tref’ 요소까지 확장된 경우에도 적용됩니다. |
...
| 이름 | 설명 |
| --- | --- |
| const [LENGTHADJUST_SPACING](../../com.aspose.html.dom.svg/svgtextcontentelement/lengthadjust_spacing/) | 값 'spacing'에 해당합니다. |
| const [LENGTHADJUST_SPACINGANDGLYPHS](../../com.aspose.html.dom.svg/svgtextcontentelement/lengthadjust_spacingandglyphs/) | 값 'spacingAndGlyphs'에 해당합니다. |
| const [LENGTHADJUST_UNKNOWN](../../com.aspose.html.dom.svg/svgtextcontentelement/lengthadjust_unknown/) | 열거형이 미리 정의된 유형 중 하나가 아닌 값으로 설정되었습니다. 이 유형의 새 값을 정의하거나 기존 값을 이 유형으로 전환하려는 시도는 유효하지 않습니다. |
