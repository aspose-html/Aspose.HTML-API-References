...
description: "com.aspose.html.dom.svg.SVGTextContentElement 클래스. SVGTextContentElement는 SVGTextElement, SVGTSpanElement, SVGTRefElement, SVGAltGlyphElement 및 SVGTextPathElement와 같은 다양한 텍스트 관련 인터페이스가 상속합니다. 이 인터페이스의 메서드가 문자 인덱스 또는 문자 수를 참조할 경우, 해당 참조는 각각 UTF-16 코드 단위의 인덱스 또는 UTF-16 코드 단위의 수로 해석됩니다. 이는 DOM Level 2 Core와 일관성을 유지하기 위한 것으로, CharacterData 인터페이스의 메서드가 문자 데이터 내에서 인덱스와 개수로 UTF-16 코드 단위를 사용하기 때문입니다. 예를 들어 텍스트 요소의 텍스트 내용이 U10000과 같은 단일 비BMP 문자일 경우, 해당 요소에서 getNumberOfChars를 호출하면 UTF-16 코드 단위가 두 개(대리 쌍)이므로 2를 반환합니다."
type: docs
...
SVGTextContentElement는 SVGTextElement, SVGTSpanElement, SVGTRefElement, SVGAltGlyphElement 및 SVGTextPathElement와 같은 다양한 텍스트 관련 인터페이스가 상속합니다. 이 인터페이스의 메서드 중 문자 인덱스나 문자 수를 참조하는 경우, 이러한 참조는 각각 UTF-16 코드 단위의 인덱스 또는 UTF-16 코드 단위의 수로 해석되어야 합니다. 이는 CharacterData 인터페이스의 메서드가 문자 데이터 내에서 인덱스와 개수로 UTF-16 코드 단위를 사용하는 DOM Level 2 Core와의 일관성을 위해서입니다. 예를 들어, ‘text’ 요소의 텍스트 내용이 U+10000과 같은 단일 비BMP 문자일 경우, 해당 요소에서 getNumberOfChars를 호출하면 그 문자를 나타내는 두 개의 UTF-16 코드 단위(서러게이트 쌍) 때문에 2가 반환됩니다.
