...
description: "com.aspose.html.dom.svg.SVGDocument 클래스. SVGDocument는 SVG 계층 구조의 루트이며 전체 내용을 보유합니다. 계층 구조에 대한 접근을 제공할 뿐만 아니라 문서에서 특정 정보 집합에 접근하기 위한 편리한 메서드도 제공합니다. SVG 요소가 다른 패키지의 문서 구성 요소로 인라인 삽입될 때(예: XHTML 문서 내에 SVG 요소가 인라인 삽입되는 경우) SVGDocument 객체는 존재하지 않고, 문서 객체 계층의 루트 객체는 HTMLDocument와 같은 다른 유형의 Document 객체가 됩니다. 그러나 XML 문서 계층의 루트 요소가 SVG 요소인 경우(예: MIME 타입이 image/svg+xml인 독립 SVG 파일을 보는 경우) SVGDocument 객체가 실제로 존재하며, 이 경우 SVGDocument 객체가 문서 객체 모델 계층의 루트 객체가 됩니다."
type: docs
...
`SVGDocument`는 SVG 계층 구조의 루트이며 전체 내용을 보유합니다. 계층 구조에 대한 접근을 제공하는 것 외에도 문서에서 특정 정보 집합에 접근하기 위한 편리한 메서드도 제공합니다. 다른 패키지의 문서 구성 요소로서 ‘svg’ 요소가 인라인으로 포함될 때, 예를 들어 ‘svg’ 요소가 XHTML 문서 [XHTML] 내에 인라인으로 포함될 경우, `SVGDocument` 객체는 존재하지 않으며, 대신 문서 객체 계층의 루트 객체는 HTMLDocument와 같은 다른 유형의 Document 객체가 됩니다. 그러나 XML 문서 계층 구조의 루트 요소가 ‘svg’ 요소인 경우, 예를 들어 독립 실행형 SVG 파일(즉, MIME 타입 "image/svg+xml"인 파일)을 볼 때는 `SVGDocument` 객체가 실제로 존재합니다. 이 경우 `SVGDocument` 객체는 문서 객체 모델 계층 구조의 루트 객체가 됩니다.
...
| [getRootElement](../../com.aspose.html.dom.svg/svgdocument/rootelement/) 문서 계층 구조의 루트 ‘svg’ 요소입니다. |
...
| [getOverrideStyle](../../com.aspose.html.dom.svg/svgdocument/getoverridestyle/)(Element, String) | 이 메서드는 지정된 요소와 지정된 의사 요소에 대한 오버라이드 스타일 선언을 검색하는 데 사용됩니다. |
...
| [normalize](../../com.aspose.html.dom/node/normalize/)() | `[`Text`](../../com.aspose.html.dom/text/)` 노드들을 이 Node 아래의 서브트리 전체 깊이(속성 노드 포함)로 이동시켜, 구조만이 (예: [`elements`](../../com.aspose.html.dom/element/), [`comments`](../../com.aspose.html.dom/comment/), [`processing instructions`](../../com.aspose.html.dom/processinginstruction/), [`CDATA sections`](../../com.aspose.html.dom/cdatasection/), [`entity references`](../../com.aspose.html.dom/entityreference/)) [`Text`](../../com.aspose.html.dom/text/) 노드를 구분하도록 \"정규\" 형태로 만든다. 즉, 인접한 Text 노드나 빈 Text 노드가 존재하지 않는다. 이는 문서의 DOM 뷰가 저장 후 다시 로드된 것과 동일하도록 보장하는 데 사용할 수 있으며, 특정 문서 트리 구조에 의존하는 작업(예: XPointer [XPointer] 조회)에도 유용하다. [`Node.ownerDocument`](../../com.aspose.html.dom/node/ownerdocument/)에 연결된 [`DOMConfiguration`](../../com.aspose.html/configuration/) 객체의 \"normalize-characters\" 매개변수가 true이면, 이 메서드는 Text 노드의 문자도 완전히 정규화한다. |
...
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save)(ResourceHandler) | [`ResourceHandler`](../../com.aspose.html.saving.resourcehandlers/resourcehandler/)를 사용하여 문서 내용과 리소스를 저장합니다. |
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save_6)(String) | `path`로 지정된 로컬 파일에 문서를 저장합니다. 이 문서에서 사용된 모든 리소스는 인접 폴더에 저장되며, 폴더 이름은 output_file_name + "_files" 형태로 구성됩니다. |
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save_3)(Url) | `url`로 지정된 로컬 파일에 문서를 저장합니다. 이 문서에서 사용된 모든 리소스는 인접 폴더에 저장되며, 폴더 이름은 output_file_name + "_files" 형태로 구성됩니다. |
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save_1)(ResourceHandler, SVGSaveFormat) | [`ResourceHandler`](../../com.aspose.html.saving.resourcehandlers/resourcehandler/)를 사용하여 문서 내용과 리소스를 저장합니다. |
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save_2)(ResourceHandler, SVGSaveOptions) | [`ResourceHandler`](../../com.aspose.html.saving.resourcehandlers/resourcehandler/)를 사용하여 문서 내용과 리소스를 저장합니다. |
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save_7)(String, SVGSaveFormat) | `path`로 지정된 로컬 파일에 문서를 저장합니다. 이 문서에서 사용된 모든 리소스는 인접 폴더에 저장되며, 폴더 이름은 output_file_name + "_files" 형태로 구성됩니다. |
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save_8)(String, SVGSaveOptions) | `path`로 지정된 로컬 파일에 문서를 저장합니다. 이 문서에서 사용된 모든 리소스는 인접 폴더에 저장되며, 폴더 이름은 output_file_name + "_files" 형태로 구성됩니다. |
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save_4)(Url, SVGSaveFormat) | `url`로 지정된 로컬 파일에 문서를 저장합니다. 이 문서에서 사용된 모든 리소스는 인접 폴더에 저장되며, 폴더 이름은 output_file_name + "_files" 형태로 구성됩니다. |
| [save](../../com.aspose.html.dom.svg/svgdocument/save/#save_5)(Url, SVGSaveOptions) | `url`로 지정된 로컬 파일에 문서를 저장합니다. 이 문서에서 사용된 모든 리소스는 인접 폴더에 저장되며, 폴더 이름은 output_file_name + "_files" 형태로 구성됩니다. |
