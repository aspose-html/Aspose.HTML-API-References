...
| [getRowIndex](../../com.aspose.html/htmltablerowelement/rowindex/) 이것은 논리적 순서이며 문서 순서가 아닙니다. `rowIndex`는 표 내의 섹션(`THEAD`, `TFOOT`, 또는 `TBODY`)을 고려하여 `THEAD` 행을 인덱스의 처음에 배치하고, 그 다음에 `TBODY` 행, 그 다음에 `TFOOT` 행을 배치합니다. @version DOM Level 2 |
| [getSectionRowIndex](../../com.aspose.html/htmltablerowelement/sectionrowindex/) 현재 섹션(`THEAD`, `TFOOT`, 또는 `TBODY`)에 상대적인 이 행의 인덱스로, 0부터 시작합니다. @version DOM Level 2 |
...
| [insertCell](../../com.aspose.html/htmltablerowelement/insertcell/)(int) | 이 행에 빈 `TD` 셀을 삽입합니다. `index`가 -1이거나 셀 수와 같으면 새 셀이 뒤에 추가됩니다. |
