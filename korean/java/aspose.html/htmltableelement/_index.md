...
description: "com.aspose.html.HTMLTableElement 클래스. 테이블에 대한 create 및 delete 메서드는 작성자가 테이블을 만들고 수정할 수 있도록 합니다. HTML 4.01은 테이블에 CAPTION, THEAD 및 TFOOT 요소가 각각 하나만 존재할 수 있다고 지정합니다. 따라서 해당 요소가 이미 존재하고 createTHead 또는 createTFoot 메서드를 호출하면 메서드는 기존의 THead 또는 TFoot 요소를 반환합니다. HTML 4.01의 TABLE 요소 정의를 참조하십시오."
type: docs
...
테이블의 create* 및 delete* 메서드는 작성자가 테이블을 구성하고 수정할 수 있게 합니다. [[HTML 4.01](http://www.w3.org/TR/1999/REC-html401-19991224)]는 테이블에 `CAPTION`, `THEAD`, `TFOOT` 요소 각각이 하나만 존재할 수 있다고 명시합니다. 따라서 해당 요소가 이미 존재하고 createTHead() 또는 createTFoot() 메서드가 호출되면, 메서드는 기존의 THead 또는 TFoot 요소를 반환합니다. HTML 4.01의 TABLE 요소 정의를 참조하십시오.
...
| [getCaption](../../com.aspose.html/htmltableelement/caption/) 테이블의 `CAPTION`을 반환하며, 존재하지 않으면 void를 반환합니다. @version DOM Level 2 |
[getCellPadding]
...
| [getRows](../../com.aspose.html/htmltableelement/rows/) 테이블의 `TFOOT`을 반환하며, 존재하지 않으면 `null`을 반환합니다. |
[getRules]
...
| [getTBodies](../../com.aspose.html/htmltableelement/tbodies/) 테이블 본문(암시적인 것 포함)의 컬렉션을 반환합니다. |
...
| [getTFoot](../../com.aspose.html/htmltableelement/tfoot/) 테이블의 `THEAD`을 반환하며, 존재하지 않으면 `null`을 반환합니다. |
| [getTHead](../../com.aspose.html/htmltableelement/thead/) 테이블의 `CAPTION`을 반환하며, 존재하지 않으면 void를 반환합니다. |
[getTitle]
...
| [createCaption](../../com.aspose.html/htmltableelement/createcaption/)() | 새 테이블 캡션 객체를 생성하거나 기존 객체를 반환합니다. |
...
| [insertRow](../../com.aspose.html/htmltableelement/insertrow/)(int) | 테이블에 새로운 빈 행을 삽입합니다. 새로운 행은 테이블에서 현재 `index`번째 행 바로 앞, 동일한 섹션에 삽입됩니다. `index`가 -1이거나 행 수와 같으면 새로운 행이 뒤에 추가됩니다. 또한 테이블이 비어 있을 경우 행은 생성되어 테이블에 삽입되는 `TBODY`에 삽입됩니다. 테이블 행은 [[HTML 4.01](http://www.w3.org/TR/1999/REC-html401-19991224)]에 따라 비어 있을 수 없습니다. |
