...
HTMLDocument 생성자는 브라우저에 로드된 웹 페이지이며 페이지 내용에 대한 진입점 역할을 하는 새로운 HTML Document 객체를 생성합니다.
...
문서 객체가 생성되면 이후에 HTML 요소들로 채울 수 있습니다. 다음 코드 조각은 기본 HTMLDocument() 생성자를 사용하여 빈 HTML 문서를 만들고 파일에 저장하는 방법을 보여줍니다.
...
참고: 현재 접근할 수 없는 잘못된 URL을 전달하면, 라이브러리는 선택된 리소스를 찾을 수 없음을 알리기 위해 특수 코드 ‘NetworkError’를 가진 [`DOMException`](../../../com.aspose.html.dom/domexception/)을 throw합니다.
...
참고: 현재 접근할 수 없는 잘못된 URL을 전달하면, 라이브러리는 선택된 리소스를 찾을 수 없음을 알리기 위해 특수 코드 ‘NetworkError’를 가진 [DOMException](T:com.aspose.html.dom.DOMException)을 throw합니다.
...
지정된 base-uri와 환경 구성 설정을 사용하여 문자열 콘텐츠에서 HTML 문서를 생성합니다.
...
| Parameter | Type | 설명 |
| --- | --- | --- |
| content | Stream | 문서를 로드할 [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream) 콘텐츠. |
| baseUri | String | 문서의 기본 URI. |
...
지정된 base-uri와 환경 구성 설정을 사용하여 [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream) 콘텐츠에서 HTML 문서를 생성합니다.
...
HTML 문서를 [`RequestMessage`](../../../com.aspose.html.net/requestmessage/) 객체에서 생성합니다.
...
| Parameter | Type | 설명 |
| --- | --- | --- |
| request | RequestMessage | 문서 내용을 포함하는 [`body`](../../../com.aspose.html.net/requestmessage/content/)를 가진 요청 메시지입니다. |

## 비고

정의에 따르면, 메시지 핸들러는 웹 요청을 받아 웹 응답을 반환하는 클래스입니다. 다시 말해, 메시지 핸들러는 입력 중 웹 서비스 요청을 처리하거나 출력 중 응답을 처리하는 데 사용됩니다.

이 생성자를 사용하는 방법에 대한 더 많은 시나리오를 보려면 [docs site](https://docs.aspose.com/html/net/message-handlers/)를 방문해 주세요.
...
| Parameter | Type | 설명 |
| --- | --- | --- |
| request | RequestMessage | 문서 내용을 포함하는 [body](P:com.aspose.html.net.RequestMessage.Content)를 가진 요청 메시지입니다. |
| 구성 | 구성 | 스크립트 정책, 사용자 정의 스타일시트 등과 같은 환경 구성 |
