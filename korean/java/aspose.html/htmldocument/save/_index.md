...
description: "HTMLDocument 메서드. 문서를 지정된 URL의 로컬 파일에 저장합니다. 이 문서에서 사용된 모든 리소스는 output_file_name_files와 같이 이름이 구성된 인접 폴더에 저장됩니다."
type: docs
...
문서를 url로 지정된 로컬 파일에 저장합니다. 이 문서에서 사용된 모든 리소스는 인접 폴더에 저장되며, 폴더 이름은 output_file_name + "_files" 형태로 생성됩니다.
...
대부분의 작업은 문서를 저장해야 합니다. 기존 파일을 로드하거나 처음부터 HTML 문서를 만든 후에는 HTMLDocument.Save() 메서드 중 하나를 사용해 변경 사항을 저장할 수 있습니다. 이 메서드들은 경로, URL 또는 출력 저장소로 지정된 로컬 파일에 HTML을 저장하도록 허용합니다. 저장에 대해 자세히 알아보려면 [documentation](https://docs.aspose.com/html/net/working-with-documents/saving-a-document/)을 참조하십시오.

Save(Url) 메서드

HTML 문서를 저장하려면 전체 Url 경로인 'outputFilePath'를 지정해야 합니다. `Url(url)` 생성자는 지정된 url로 [`Url`](../../url/) 클래스의 인스턴스를 생성합니다. 그런 다음 해당 인스턴스를 Save(Url) 메서드에 전달해야 합니다. 문서는 지정된 url의 로컬 파일에 저장됩니다. 이 문서에서 사용된 모든 리소스는 output_file_name + "_files"와 같이 이름이 구성된 인접 폴더에 저장됩니다.
...
문서를 경로로 지정된 로컬 파일에 저장합니다. 이 문서에서 사용된 모든 리소스는 인접 폴더에 저장되며, 폴더 이름은 output_file_name + "_files" 형태로 생성됩니다.
...
Save(String) 메서드는 매개변수로 출력 파일에 대한 로컬 파일 시스템 경로를 받아 해당 경로로 지정된 로컬 파일에 HTML 문서를 저장합니다. 문서에서 사용된 모든 리소스는 인접 폴더에 저장됩니다.
...
문서를 path로 지정된 로컬 파일에 저장합니다. 이 문서에서 사용된 모든 리소스는 인접 폴더에 저장되며, 폴더 이름은 output_file_name + "_files" 형태로 생성됩니다.
...
Save(String, HTMLSaveFormat) 메서드는 출력 파일에 대한 로컬 파일 시스템 경로와 saveFormat을 매개변수로 받습니다. [`HTMLSaveFormat`](../../../com.aspose.html.saving/htmlsaveformat/) 열거형은 문서가 저장되는 형식을 지정하며, HTML, MHTML 및 MD 형식이 될 수 있습니다. 이 메서드는 지정된 형식으로 HTML 문서를 경로로 지정된 로컬 파일에 저장합니다. 문서에 사용된 모든 리소스는 인접 폴더에 저장됩니다.
...
&lt;div class="custom-class"&gt;CSS 클래스 컨테이너에 의해 맞춤화됨&lt;/div&gt;

&lt;div&gt;

&lt;p class="pStyle"&gt;첫 번째 스타일은 pStyle 클래스 단락에 의해 적용됩니다&lt;/p&gt;

&lt;p class="pStyle"&gt;두 번째 스타일은 pStyle 클래스 단락에 의해 적용됩니다&lt;/p&gt;

&lt;p class="pStyle"&gt;세 번째 스타일은 pStyle 클래스 단락에 의해 적용됩니다&lt;/p&gt;

&lt;span class="pStyle"&gt;pStyle에 의해 스타일링된 Span&lt;/span&gt;
...
HTML 문서를 저장하려면 전체 Url 경로를 지정해야 합니다. Url(url) 생성자는 지정된 url을 사용하여 [`Url`](../../url/) 클래스의 인스턴스를 생성합니다. HTMLSaveOptions() 생성자는 리소스 처리 구성을 위해 사용되는 ResourceHandlingOptions 속성을 가진 [`HTMLSaveOptions`](../../../com.aspose.html.saving/htmlsaveoptions/) 클래스의 인스턴스를 생성합니다. Save(url, saveOptions) 메서드는 매개변수를 받아 url이 지정한 로컬 파일에 리소스가 포함된 HTML 문서를 저장합니다.
...
문서를 저장하려면 출력 파일에 대한 로컬 파일 시스템 경로를 지정해야 합니다. MarkdownSaveOptions() 생성자는 [`MarkdownSaveOptions`](../../../com.aspose.html.saving/markdownsaveoptions/) 클래스의 인스턴스를 생성하며, 여러 속성을 제공합니다. 예를 들어, 마크다운 서식 스타일을 설정하고, 사전 정의된 GitLab Flavored Markdown 호환 옵션을 사용하며, 리소스 처리를 구성할 수 있습니다. Save(path, saveOptions) 메서드는 출력 파일의 로컬 파일 시스템 경로와 옵션 인스턴스를 매개변수로 받아, HTML을 리소스가 포함된 마크다운 문서로 변환하여 지정된 경로의 로컬 파일에 저장합니다.
...
| --- | --- | --- |
| url | Url | 출력 파일에 대한 로컬 [`URL`](../../url/) |
| saveOptions | MarkdownSaveOptions | [`MarkdownSaveOptions`](../../../com.aspose.html.saving/markdownsaveoptions/) 객체 사용을 통해 렌더링 프로세스를 조정할 수 있습니다. 자세한 내용은 [documentation](https://docs.aspose.com/html/net/converting-between-formats/html-to-markdown/#save-options) 을 참조하십시오. |
...
문서를 저장하려면 전체 Url 경로를 지정해야 합니다. Url(url) 생성자는 지정된 url을 사용하여 [`Url`](../../url/) 클래스의 인스턴스를 생성합니다. MarkdownSaveOptions() 생성자는 여러 속성을 가진 [`MarkdownSaveOptions`](../../../com.aspose.html.saving/markdownsaveoptions/) 클래스의 인스턴스를 생성합니다. 예를 들어, Markdown 서식 스타일을 설정하고, 사전 정의된 GitLab Flavored Markdown 호환 옵션을 사용하며, 리소스 처리를 구성할 수 있습니다. Save(url, saveOptions) 메서드는 url과 저장 옵션 인스턴스를 매개변수로 받아, 해당 url이 지정한 로컬 파일에 리소스가 포함된 문서를 저장합니다.
...
| --- | --- | --- |
| 경로 | String | 출력 파일에 대한 로컬 경로. |
| saveOptions | MHTMLSaveOptions | [`MHTMLSaveOptions`](../../../com.aspose.html.saving/mhtmlsaveoptions/) 객체 사용을 통해 렌더링 프로세스를 조정할 수 있습니다. 자세한 내용은 [documentation](https://docs.aspose.com/html/net/converting-between-formats/html-to-mhtml/#save-options) 을 참조하십시오. |
