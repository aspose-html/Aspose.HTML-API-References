...
description: "com.aspose.html.dom.ProcessingInstruction 클래스. ProcessingInstruction은 XML에서 프로세서별 정보를 문서 텍스트에 보관하기 위한 처리 지시자를 나타냅니다"
type: docs
...
ProcessingInstruction은 "processing instruction"을 나타내며, XML에서 문서 텍스트에 프로세서별 정보를 유지하는 방법으로 사용됩니다.
...
| [getNodeName](../../com.aspose.html.dom/processinginstruction/nodename/) 이 노드의 유형에 따라 이름을 반환합니다. |
| [getNodeType](../../com.aspose.html.dom/processinginstruction/nodetype/) 기본 객체의 유형을 나타내는 코드를 반환합니다. |
| [nodeValue](../../com.aspose.html.dom/processinginstruction/nodevalue/) { get; set; } | 이 노드의 유형에 따라 값을 반환합니다. |
...
| [getTarget](../../com.aspose.html.dom/processinginstruction/target/) 이 처리 명령의 대상입니다. |
| [textContent](../../com.aspose.html.dom/processinginstruction/textcontent/) { get; set; } | 이 속성은 이 노드와 그 하위 노드들의 텍스트 내용을 반환합니다. null로 정의된 경우, 설정해도 효과가 없습니다. 설정 시, 이 노드가 가질 수 있는 모든 자식이 제거되고, 새 문자열이 비어 있지 않거나 null이 아니면 해당 문자열을 포함하는 단일 Text 노드로 교체됩니다. |
...
| [normalize](../../com.aspose.html.dom/node/normalize/)() | 이 노드 아래의 하위 트리 전체 깊이에 있는 모든 [`Text`](../text/) 노드(속성 노드 포함)를 "정규" 형태로 변환합니다. 이 형태에서는 구조(예: [`elements`](../element/), [`comments`](../comment/), `processing instructions`, [`CDATA sections`](../cdatasection/), 및 [`entity references`](../entityreference/))만이 [`Text`](../text/) 노드를 구분하며, 인접한 Text 노드나 빈 Text 노드가 존재하지 않습니다. 이는 문서의 DOM 뷰가 저장 후 다시 로드된 것과 동일하도록 보장하는 데 사용할 수 있으며, 특정 문서 트리 구조에 의존하는 작업(예: XPointer [XPointer] 조회)에서 유용합니다. [`DOMConfiguration`](../../com.aspose.html/configuration/) 객체에 연결된 [`Node.ownerDocument`](../node/ownerdocument/)의 "normalize-characters" 매개변수가 true이면 이 메서드는 Text 노드의 문자를 완전히 정규화합니다. |
