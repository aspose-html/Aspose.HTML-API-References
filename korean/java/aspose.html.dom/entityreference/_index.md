---
title: "EntityReference 클래스"
second_title: "Java용 Aspose.HTML API 참조"
description: "com.aspose.html.dom.EntityReference 클래스. EntityReference 노드는 트리에서 엔터티 참조를 나타내는 데 사용할 수 있습니다."
type: docs

url: /ko/java/com.aspose.html.dom/entityreference/
---
## EntityReference class

EntityReference 노드는 트리에서 엔터티 참조를 나타내는 데 사용될 수 있습니다.
...
| [getNodeName](../../com.aspose.html.dom/entityreference/nodename/) 이 노드의 이름이며, 유형에 따라 달라집니다. |
| [getNodeType](../../com.aspose.html.dom/entityreference/nodetype/) 기본 객체의 유형을 나타내는 코드입니다. |
...
| [normalize](../../com.aspose.html.dom/node/normalize/)() | `[`Text`](../text/)` 노드를 이 Node 아래의 하위 트리 전체 깊이(속성 노드 포함)로 이동시켜 구조만 남는 "정규" 형태로 만든다(예: [`elements`](../element/), [`comments`](../comment/), [`processing instructions`](../processinginstruction/), [`CDATA sections`](../cdatasection/), 및 `entity references`가 [`Text`](../text/) 노드를 구분하도록 함). 즉 인접한 Text 노드나 빈 Text 노드가 없게 된다. 이는 문서의 DOM 뷰가 저장 후 다시 로드된 것과 동일하도록 보장하는 데 사용할 수 있으며, 특정 문서 트리 구조에 의존하는 작업(예: XPointer [XPointer] 조회)에도 유용하다. [`Node.ownerDocument`](../node/ownerdocument/)에 연결된 [`DOMConfiguration`](../../com.aspose.html/configuration/) 객체의 "normalize-characters" 매개변수가 true이면, 이 메서드는 Text 노드의 문자도 완전히 정규화한다. |
