...
DocumentFragment는 "lightweight" 또는 "minimal" 문서 객체입니다. 문서 트리의 일부를 추출하거나 새로운 문서 조각을 만들고자 하는 경우가 매우 흔합니다.
...
| [textContent](../../com.aspose.html.dom/documentfragment/textcontent/) { get; set; } | 이 속성은 이 노드와 그 하위 노드들의 텍스트 내용을 반환합니다. null로 정의된 경우, 설정해도 효과가 없습니다. 설정 시, 이 노드가 가질 수 있는 모든 자식이 제거되고, 새 문자열이 비어 있지 않거나 null이 아니면 해당 문자열을 포함하는 단일 Text 노드로 교체됩니다. |
...
| [querySelector](../../com.aspose.html.dom/documentfragment/queryselector/)(String) | 문서에서 선택자와 일치하는 첫 번째 요소를 반환합니다. |
| [querySelectorAll](../../com.aspose.html.dom/documentfragment/queryselectorall/)(String) | 문서에서 선택자와 일치하는 모든 요소의 NodeList를 반환합니다. |
...
| [replaceChild](../../com.aspose.html.dom/node/replacechild/)(Node, Node) | 자식 노드 oldChild를 newChild로 교체하고, 교체된 oldChild 노드를 반환합니다. newChild가 `DocumentFragment` 객체인 경우, oldChild는 `DocumentFragment`의 모든 자식으로 교체되며, 이들은 동일한 순서로 삽입됩니다. newChild가 이미 트리에 존재하면 먼저 트리에서 제거됩니다. |
