...
| [getIsElementContentWhitespace](../../com.aspose.html.dom/text/iselementcontentwhitespace/) 이 텍스트 노드가 요소 내용 공백(종종 "무시 가능한 공백"이라고 부름)을 포함하고 있는지 여부를 반환합니다. |
...
| [getLength](../../com.aspose.html.dom/characterdata/length/) data와 아래의 subStringData 메서드를 통해 사용할 수 있는 16비트 단위의 수입니다. 이 값은 0일 수 있으며, 즉 CharacterData 노드가 비어 있을 수 있습니다. |
| [getLocalName](../../com.aspose.html.dom/node/localname/) 이 노드의 정규화된 이름 중 로컬 부분을 반환합니다. [`ELEMENT_NODE`](../node/element_node/) 및 [`ATTRIBUTE_NODE`](../node/attribute_node/)이 아닌 모든 유형의 노드와 DOM Level 1 메서드(예: [`Document.createElement()`](../document/createelement/))로 생성된 노드에 대해서는 항상 null입니다. |
...
| [nodeValue](../../com.aspose.html.dom/text/nodevalue/) { get; set; } | 이 노드의 유형에 따라 값을 반환합니다. |
...
| [textContent](../../com.aspose.html.dom/text/textcontent/) { get; set; } | 이 속성은 이 노드와 그 하위 노드들의 텍스트 내용을 반환합니다. null로 정의된 경우, 설정해도 효과가 없습니다. 설정 시, 이 노드가 가질 수 있는 모든 자식이 제거되고, 새 문자열이 비어 있지 않거나 null이 아니면 해당 문자열을 포함하는 단일 Text 노드로 교체됩니다. |
...
| [appendData](../../com.aspose.html.dom/characterdata/appenddata/)(String) | 문자열을 노드의 문자 데이터 끝에 추가합니다. |
...
| [normalize](../../com.aspose.html.dom/node/normalize/)() | 이 노드 아래의 서브 트리 전체 깊이에 있는 모든 [`Text`](../text/) 노드(속성 노드 포함)를 "정규" 형태로 변환합니다. 이 형태에서는 구조(예: [`elements`](../element/), [`comments`](../comment/), [`processing instructions`](../processinginstruction/), `CDATA sections`, 및 [`entity references`](../entityreference/))만이 [`Text`](../text/) 노드를 구분하며, 인접한 Text 노드나 빈 Text 노드가 존재하지 않게 됩니다. 이는 문서의 DOM 뷰가 저장 후 다시 로드한 것과 동일하도록 보장하는 데 사용할 수 있으며, 특정 문서 트리 구조에 의존하는 작업(예: XPointer [XPointer] 조회) 시 유용합니다. `[`DOMConfiguration`](../../com.aspose.html/configuration/)` 객체에 연결된 [`Node.ownerDocument`](../node/ownerdocument/)의 "normalize-characters" 매개변수가 true인 경우, 이 메서드는 Text 노드의 문자도 완전히 정규화합니다. |
...
| [replaceWholeText](../../com.aspose.html.dom/text/replacewholetext/)(String) | 현재 노드와 논리적으로 인접한 모든 텍스트 노드의 텍스트를 지정된 텍스트로 교체합니다. 교체 텍스트를 받은 경우가 아니라면 현재 노드를 포함한 모든 논리적으로 인접한 텍스트 노드가 제거됩니다. |
