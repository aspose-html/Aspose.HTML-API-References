...
description: "Document 메서드. 지정된 패키지에 속하는 주어진 태그 이름을 가진 요소 목록을 반환합니다. 루트 노드를 포함한 전체 문서를 검색합니다."
type: docs
...
주어진 패키지에 속한 지정된 태그 이름을 가진 요소들의 목록을 반환합니다. 루트 노드를 포함한 전체 문서를 검색합니다.
...
| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| packageURI | String | 찾을 요소들의 패키지 URI. |
| localName | String | 찾고자 하는 요소의 로컬 이름이거나, 모든 요소와 일치하는 특수값 * 중 하나입니다. |

### 반환 값

트리에서 나타나는 순서대로 찾아진 요소들의 실시간 [`NodeList`](../../../com.aspose.html.collections/nodelist/) (아래 주석을 참고하세요).

## 비고

공식 [spec](https://dom.spec.whatwg.org/#dom-document-getelementsbytagnamens)을 참조하십시오.

[w3schools](https://www.w3schools.com/xml/met_document_getelementsbytagnamens.asp)에서 웹 개발 실습 콘텐츠를 찾을 수 있습니다.
