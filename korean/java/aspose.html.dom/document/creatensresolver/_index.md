...
description: "Document 메서드. DOM 노드를 조정하여 패키지를 해결하고, XPath 식을 문서 내에서 해당 노드가 나타난 컨텍스트를 기준으로 쉽게 평가할 수 있도록 합니다. 이 어댑터는 lookupNamespaceURI 메서드(DOM Level 3)와 같이 작동하여, 호출 시점에 노드 계층 구조에 존재하는 현재 정보를 사용해 주어진 접두사로부터 packageURI를 해결하고, 암시적인 xml 접두사도 올바르게 해결합니다."
type: docs
