...
description: "Node 메서드. Node 인터페이스의 insertBefore 메서드는 지정된 부모 노드의 자식으로서 기준 노드 앞에 노드를 삽입합니다."
type: docs
...
주어진 노드가 이미 문서에 존재한다면, insertBefore()는 해당 노드를 현재 위치에서 새로운 위치로 이동시킵니다. (즉, 지정된 새로운 부모에 추가되기 전에 기존 부모로부터 자동으로 제거됩니다.)
...
| --- | --- | --- |
| 노드 | Node | 삽입될 노드. |
| 자식 | Node | newNode가 삽입되는 기준이 되는 노드. 이것이 null이면 newNode는 해당 노드의 자식 노드 끝에 삽입됩니다. |
