...
참고: 제거된 자식에 대한 참조가 유지되는 한, 해당 객체는 메모리에 남아 있지만 더 이상 DOM의 일부가 아닙니다. 코딩 중에 나중에 다시 사용할 수 있습니다. removeChild()의 반환값을 저장하지 않고 다른 참조도 유지되지 않으면, 짧은 시간 후에 메모리에서 자동으로 삭제됩니다.
