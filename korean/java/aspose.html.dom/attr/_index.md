...
| [getLocalName](../../com.aspose.html.dom/attr/localname/) 이 노드의 정규화된 이름 중 로컬 부분을 반환합니다. ELEMENT_NODE와 ATTRIBUTE_NODE가 아닌 모든 유형의 노드 및 Document.createElement()와 같은 DOM Level 1 메서드로 생성된 노드의 경우, 항상 null을 반환합니다. |
...
| [getNodeType](../../com.aspose.html.dom/attr/nodetype/) 기본 객체 유형을 나타내는 코드를 반환합니다. |
| [nodeValue](../../com.aspose.html.dom/attr/nodevalue/) { get; set; } | 이 노드의 유형에 따라 값을 반환합니다. |
| [getOwnerDocument](../../com.aspose.html.dom/node/ownerdocument/) Node 인터페이스의 읽기 전용 ownerDocument 속성은 해당 노드의 최상위 문서 객체를 반환합니다. |
| [getOwnerElement](../../com.aspose.html.dom/attr/ownerelement/) 이 속성이 연결된 Element 노드를 반환하며, 사용 중이 아니면 null을 반환합니다. |
...
| [getSpecified](../../com.aspose.html.dom/attr/specified/) 인스턴스 문서에서 이 속성에 명시적으로 값이 지정된 경우 true, 그렇지 않으면 false를 반환합니다. |
| [textContent](../../com.aspose.html.dom/attr/textcontent/) { get; set; } | 이 속성은 이 노드와 그 하위 노드들의 텍스트 내용을 반환합니다. null로 정의된 경우, 설정해도 효과가 없습니다. 설정 시, 이 노드가 가질 수 있는 모든 자식이 제거되고, 새 문자열이 비어 있지 않거나 null이 아니면 해당 문자열을 포함하는 단일 Text 노드로 교체됩니다. |
[getValue]
