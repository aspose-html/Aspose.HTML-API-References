...
normal - 이 값은 사용자 에이전트가 공백 시퀀스를 축소하고, 라인 박스를 채우기 위해 필요에 따라 줄을 나누도록 지시합니다. 생성된 콘텐츠에서 \"\\A\"가 나타날 경우 추가 줄 바꿈이 생성될 수 있습니다(예: HTML의 BR 요소).pre - 이 값은 사용자 에이전트가 공백 시퀀스를 축소하지 않도록 방지합니다. 줄은 소스의 새 줄 문자나 생성된 콘텐츠에서 \"\\A\"가 나타날 때만 나뉩니다.nowrap - 이 값은 'normal'과 같이 공백을 축소하지만, 텍스트 내에서 \"\\A\"에 의해 생성된 경우를 제외하고 줄 바꿈을 억제합니다(예: HTML의 BR 요소).
