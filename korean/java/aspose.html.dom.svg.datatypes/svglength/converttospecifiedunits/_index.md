...
description: "SVGLength 메서드. 동일한 기본 저장 값을 유지하면서 저장된 단위 식별자를 지정된 unitType으로 재설정합니다. 이 메서드의 결과로 객체 속성인 unitType, valueInSpecifiedUnits 및 valueAsString이 변경될 수 있습니다. 예를 들어 원래 값이 0.5cm였고 이 메서드를 호출하여 밀리미터로 변환하면 unitType은 SVG_LENGTHTYPE_MM으로 변경되고, valueInSpecifiedUnits는 숫자 값 5로 변경되며, valueAsString은 5mm로 변경됩니다."
type: docs
...
기존에 저장된 기본 값을 유지하면서, 저장된 단위 식별자를 지정된 unitType으로 재설정합니다. 이 메서드의 결과로 객체 속성 unitType, valueInSpecifiedUnits 및 valueAsString이 변경될 수 있습니다. 예를 들어 원래 값이 "0.5cm"였고 메서드를 호출하여 밀리미터로 변환하면, unitType은 SVG_LENGTHTYPE_MM으로 변경되고, valueInSpecifiedUnits는 숫자 값 5로 변경되며, valueAsString은 "5mm"로 변경됩니다.
