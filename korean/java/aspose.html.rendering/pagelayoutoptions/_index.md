...
다른 PageSetup 옵션과 함께 페이지의 크기와 레이아웃을 결정하는 플래그를 지정합니다. 이러한 플래그는 설명에 따라 함께 결합할 수 있습니다.
...
| UseWidestPage | `2` | FitToContentWidth와 결합될 때 모든 페이지의 너비가 동일하며 모든 페이지 중 가장 넓은 콘텐츠 크기와 같게 됨을 나타냅니다. |
...
| ScaleToPageWidth | `100` | 이 플래그는 사용 가능한 페이지 너비와 겹치는 콘텐츠 간 차이가 가장 큰 페이지에 맞게 문서의 콘텐츠가 스케일링됨을 나타냅니다. FitToContentWidth 플래그와 충돌하며 두 플래그가 모두 지정된 경우에는 ScaleToPageWidth만 적용됩니다. |
| ScaleToPageHeight | `1000` | 이 플래그는 문서의 콘텐츠가 첫 페이지 높이에 맞게 스케일링됨을 나타냅니다. FitToContentHeight 플래그와 충돌하며 두 플래그가 모두 지정된 경우에는 ScaleToPageHeight만 적용됩니다. 모든 문서 콘텐츠는 단일 페이지에만 배치됩니다. |
