...
| 이름 | 값 | 설명 |
| --- | --- | --- |
| InlineHTML | `1` | 이 플래그는 HTML 요소 인라인을 활성화합니다. 이 플래그가 설정되면 `div`와 같은 블록 수준 요소 중 `markdown` 속성 값이 `inline`인 요소가 결과 마크다운에 삽입됩니다. |
...
| CodeBlock | `20` | 이 플래그는 코드 블록의 변환을 활성화합니다. 코드 블록은 `pre`와 `code` 두 요소로 구성되며, 이러한 구조의 내용은 그대로 "as is" 처리됩니다. |
| HorizontalRule | `40` | 이 플래그는 `horizontal rules`의 변환을 활성화합니다. |
...
| TaskList | `4000` | 이 플래그는 작업 목록의 변환을 활성화합니다. 작업 목록은 `input` 요소로 구성되며, 이는 `list` 요소의 첫 번째 자식이어야 하고 `type` 속성 값이 `checkbox`와 일치해야 합니다. |
