---
title: "MarkdownSyntaxNodeFilter.AcceptNode"
second_title: "Aspose.HTML for Java API 참조"
description: "MarkdownSyntaxNodeFilter 메서드. 노드를 수락하기 위한 인터페이스를 정의합니다"
type: docs

url: /ko/java/com.aspose.html.toolkit.markdown.syntax/markdownsyntaxnodefilter/acceptnode/
---
## MarkdownSyntaxNodeFilter.AcceptNode method

노드를 수락하기 위한 인터페이스를 정의합니다.

```java
public abstract short AcceptNode(MarkdownSyntaxNode n)
```

| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| n | MarkdownSyntaxNode | MarkdownSyntaxNode. |

### 반환 값

짧은 값.

### 또 보기

* class [MarkdownSyntaxNode](../../markdownsyntaxnode/)
* class [MarkdownSyntaxNodeFilter](../)
* package [com.aspose.html.toolkit.markdown.syntax](../../../com.aspose.html.toolkit.markdown.syntax/)
* package [Aspose.HTML](../../../)
