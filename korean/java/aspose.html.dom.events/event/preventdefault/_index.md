...
이벤트 흐름의 어느 단계에서든 `PreventDefault` 메서드가 호출되면 이벤트가 취소됩니다. 이벤트와 연관된 모든 기본 동작은 발생하지 않습니다. 취소 불가능한 이벤트에 대해 이 메서드를 호출해도 효과가 없습니다. `PreventDefault`가 호출된 후에는 이벤트 전파가 남은 동안 계속 적용됩니다. 이 메서드는 이벤트 흐름의 어느 단계에서든 사용할 수 있습니다.
