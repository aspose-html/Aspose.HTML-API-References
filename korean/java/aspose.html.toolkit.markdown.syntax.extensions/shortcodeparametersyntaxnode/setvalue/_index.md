---
title: "ShortCodeParameterSyntaxNode.SetValue"
second_title: "Java용 Aspose.HTML API 참조"
description: "ShortCodeParameterSyntaxNode 메서드. 값을 설정하기 위한 인터페이스를 정의합니다"
type: docs

url: /ko/java/com.aspose.html.toolkit.markdown.syntax.extensions/shortcodeparametersyntaxnode/setvalue/
---
## ShortCodeParameterSyntaxNode.SetValue method

값 설정을 위한 인터페이스를 정의합니다.

```java
public void SetValue(String value)
```

| Parameter | Type | 설명 |
| --- | --- | --- |
| 값 | String | 값. |

### 또 보기

* class [ShortCodeParameterSyntaxNode](../)
* package [com.aspose.html.toolkit.markdown.syntax.extensions](../../../com.aspose.html.toolkit.markdown.syntax.extensions/)
* package [Aspose.HTML](../../../)
