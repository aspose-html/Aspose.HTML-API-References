---
title: IDocumentView
second_title: Aspose.HTML for .NET API 参考
description: DocumentView 接口由支持 DOM 视图的 DOM 实现中的 Document 对象实现它提供了一个属性来检索文档的默认视图
type: docs
weight: 2660
url: /zh/net/aspose.html.dom.views/idocumentview/
---
## IDocumentView interface

DocumentView 接口由支持 DOM 视图的 DOM 实现中的 Document 对象实现。它提供了一个属性来检索文档的默认视图。

```csharp
public interface IDocumentView : IAbstractView
```

## 特性

| 姓名 | 描述 |
| --- | --- |
| [DefaultView](../../aspose.html.dom.views/idocumentview/defaultview) { get; } | 此文档的默认 AbstractView，如果没有可用则为 null。 |

### 也可以看看

* interface [IAbstractView](../iabstractview)
* 命名空间 [Aspose.Html.Dom.Views](../../aspose.html.dom.views)
* 部件 [Aspose.HTML](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.HTML.dll -->
