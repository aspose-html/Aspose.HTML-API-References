---
title: ImageRenderingOptions
second_title: Aspose.HTML for .NET API 参考
description: 初始化ImageRenderingOptionsaspose.html.rendering.image/imagerenderingoptions类的新实例Png将用作默认图像格式
type: docs
weight: 10
url: /zh/net/aspose.html.rendering.image/imagerenderingoptions/imagerenderingoptions/
---
## ImageRenderingOptions() {#constructor}

初始化[`ImageRenderingOptions`](../../imagerenderingoptions)类的新实例；Png将用作默认图像格式。

```csharp
public ImageRenderingOptions()
```

### 也可以看看

* class [ImageRenderingOptions](../../imagerenderingoptions)
* 命名空间 [Aspose.Html.Rendering.Image](../../imagerenderingoptions)
* 部件 [Aspose.HTML](../../../)

---

## ImageRenderingOptions(ImageFormat) {#constructor_1}

用指定的图像格式初始化[`ImageRenderingOptions`](../../imagerenderingoptions)类的新实例。

```csharp
public ImageRenderingOptions(ImageFormat format)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| format | ImageFormat | 输出图像格式。 |

### 也可以看看

* enum [ImageFormat](../../imageformat)
* class [ImageRenderingOptions](../../imagerenderingoptions)
* 命名空间 [Aspose.Html.Rendering.Image](../../imagerenderingoptions)
* 部件 [Aspose.HTML](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.HTML.dll -->
