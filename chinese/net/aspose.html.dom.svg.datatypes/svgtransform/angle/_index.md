---
title: Angle
second_title: Aspose.HTML for .NET API 参考
description: SVG_TRANSFORM_ROTATESVG_TRANSFORM_SKEWX 和 SVG_TRANSFORM_SKEWY 的便利属性它保存指定的角度 对于 SVG_TRANSFORM_MATRIXSVG_TRANSFORM_TRANSLATE 和 SVG_TRANSFORM_SCALE角度将为零
type: docs
weight: 10
url: /zh/net/aspose.html.dom.svg.datatypes/svgtransform/angle/
---
## SVGTransform.Angle property

SVG_TRANSFORM_ROTATE、SVG_TRANSFORM_SKEWX 和 SVG_TRANSFORM_SKEWY 的便利属性。它保存指定的角度。 对于 SVG_TRANSFORM_MATRIX、SVG_TRANSFORM_TRANSLATE 和 SVG_TRANSFORM_SCALE，角度将为零。

```csharp
public float Angle { get; }
```

### 适当的价值

指定的角度。

### 也可以看看

* class [SVGTransform](../../svgtransform)
* 命名空间 [Aspose.Html.Dom.Svg.DataTypes](../../svgtransform)
* 部件 [Aspose.HTML](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.HTML.dll -->
