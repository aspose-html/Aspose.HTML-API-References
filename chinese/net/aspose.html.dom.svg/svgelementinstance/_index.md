...
description: 每个使用元素阴影树的根对象都实现了 SVGUseElementShadowRoot 接口此接口当前没有为 ShadowRoot 接口和 DocumentOrShadowRoot mixin 定义的属性和方法定义任何扩展但是从作者脚本的角度来看以该节点为根的树完全是只读的
type: docs
weight: 2140
...
每个使用元素阴影树的根对象都实现了 SVGUseElementShadowRoot 接口。此接口当前没有为 ShadowRoot 接口和 DocumentOrShadowRoot mixin 定义的属性和方法定义任何扩展。但是，从作者脚本的角度来看，以该节点为根的树完全是只读的。
...
| [Mode](../../aspose.html.dom/shadowroot/mode) { get; } | 此 ShadowRoot 运行的模式。 |
