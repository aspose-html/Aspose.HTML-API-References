---
title: SVGPathElement.CreateSVGPathSegArcAbs
second_title: Aspose.HTML for .NET API 参考
description: SVGPathElement 方法. 返回一个独立的无父项的 SVGPathSegArcAbs 对象
type: docs
weight: 30
url: /zh/net/aspose.html.dom.svg/svgpathelement/createsvgpathsegarcabs/
---
## SVGPathElement.CreateSVGPathSegArcAbs method

返回一个独立的、无父项的 SVGPathSegArcAbs 对象。

```csharp
public SVGPathSegArcAbs CreateSVGPathSegArcAbs(float x, float y, float r1, float r2, float angle, 
    bool largeArcFlag, bool sweepFlag)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| x | Single | 此路径段终点的绝对 X 坐标。 |
| y | Single | 此路径段终点的绝对 Y 坐标。 |
| r1 | Single | 椭圆的 x 轴半径（即 r1）。 |
| r2 | Single | 椭圆的 y 轴半径（即 r2）。 |
| angle | Single | 椭圆 x 轴相对于用户坐标系 x 轴的旋转角度（以度为单位）。 |
| largeArcFlag | Boolean | large-arc-flag 参数的值。 |
| sweepFlag | Boolean | large-arc-flag 参数的值。 |

### 返回值

一个独立的、无父项的 SVGPathSegArcAbs 对象。

### 也可以看看

* class [SVGPathSegArcAbs](../../../aspose.html.dom.svg.paths/svgpathsegarcabs/)
* class [SVGPathElement](../)
* 命名空间 [Aspose.Html.Dom.Svg](../../svgpathelement/)
* 部件 [Aspose.HTML](../../../)


