...
description: 定义将多个 EPubStreams 渲染为特定IDevice 一旦没有任何用于加载资源活动计时器动画任务或指定超时的网络操作将执行渲染
type: docs
weight: 20
...
定义将多个 EPubStreams 渲染为特定IDevice。 一旦没有任何用于加载资源、活动计时器、动画任务或指定超时的网络操作，将执行渲染。
...
将 EPub 文档渲染到指定的[`IDevice`](../../idevice)中。
...
将 EPub 文档渲染到指定的[`IDevice`](../../idevice)中。 一旦没有任何用于加载资源、活动计时器、动画任务或指定超时的网络操作，将执行渲染。
...
将多个 EPub 文档渲染到指定的[`IDevice`](../../idevice)中。
...
将多个 EPub 文档渲染到指定的[`IDevice`](../../idevice)中。 一旦没有任何用于加载资源、活动计时器、动画任务或指定超时的网络操作，将执行渲染。
