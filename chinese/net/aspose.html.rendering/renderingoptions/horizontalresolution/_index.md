---
title: RenderingOptions.HorizontalResolution
second_title: Aspose.HTML for .NET API 参考
description: RenderingOptions 财产. 设置或获取内部在过滤器处理期间使用图像的水平分辨率以像素/英寸为单位默认情况下此属性为 300 dpi.
type: docs
weight: 40
url: /zh/net/aspose.html.rendering/renderingoptions/horizontalresolution/
---
## RenderingOptions.HorizontalResolution property

设置或获取内部（在过滤器处理期间使用）图像的水平分辨率，以像素/英寸为单位。默认情况下，此属性为 300 dpi.

```csharp
public virtual Resolution HorizontalResolution { get; set; }
```

### 也可以看看

* class [Resolution](../../../aspose.html.drawing/resolution/)
* class [RenderingOptions](../)
* 命名空间 [Aspose.Html.Rendering](../../renderingoptions/)
* 部件 [Aspose.HTML](../../../)


