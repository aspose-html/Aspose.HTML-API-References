...
description: Aspose.Html.HTMLElement 班级. 所有 HTML 元素接口都派生自此类仅 公开 HTML 核心属性的元素由基本 表示HTML元素界面这些元素如下 special SUB SUP SPAN BDOfont TT I B U S STRIKE BIG SMALL phrase EM STRONG DFN CODE SAMP KBD VAR CITE ACRONYM ABBRlist DD DTNOFRAMES NOSCRIPTADDRESS CENTERThe风格 HTML 元素的属性 可通过 访问元素CSSInlineStyle在 CSS 模块中定义的接口 DOM Level 2 样式表和 CSS .
type: docs
weight: 3200
...
所有 HTML 元素接口都派生自此类。仅 公开 HTML 核心属性的元素由基本 表示`HTML元素`界面。这些元素如下： special: SUB, SUP, SPAN, BDOfont: TT, I, B, U, S, STRIKE, BIG, SMALL phrase: EM, STRONG, DFN, CODE, SAMP, KBD, VAR, CITE, ACRONYM, ABBRlist: DD, DTNOFRAMES, NOSCRIPTADDRESS, CENTERThe`风格` HTML 元素的属性 可通过 访问`元素CSSInlineStyle`在 CSS 模块中定义的接口 [[DOM Level 2 样式表和 CSS](http://www.w3.org/TR/2000/REC-DOM-Level-2-Style-20001113) ].
