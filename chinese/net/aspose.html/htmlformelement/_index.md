...
description: Aspose.Html.HTMLFormElement 班级. 的形式元素包含类似于集合 和元素的行为它提供对包含的表单控件 以及表单元素属性的直接访问请参阅 HTML 4.01. 中的 FORM 元素 定义
type: docs
weight: 3230
...
的`形式`元素包含类似于集合 和元素的行为。它提供对包含的表单控件 以及表单元素属性的直接访问。请参阅 HTML 4.01. 中的 FORM 元素 定义
...
| [Enctype](../../aspose.html/htmlformelement/enctype/) { get; set; } | 提交表单的内容类型，一般为 "application/x-www-form-urlencoded"。请参阅 HTML 4.01 中的 enctype 属性 定义。调用此方法时，不能保证 触发 onsubmit 偶数处理程序。由于历史原因，该行为 不一致，作者不应依赖 特定行为。 |
...
| [Length](../../aspose.html/htmlformelement/length/) { get; } | 窗体中的窗体控件个数。 |
...
| [Method](../../aspose.html/htmlformelement/method/) { get; set; } | HTTP 方法 [[IETF RFC 2616](http://www.ietf.org/rfc/rfc2616.txt) 用于提交表单。参见 HTML 4.01. 中的方法属性定义 |
...
| [Target](../../aspose.html/htmlformelement/target/) { get; set; } | 在其中呈现资源的框架。请参阅 HTML 4.01. 中的目标属性定义 |
