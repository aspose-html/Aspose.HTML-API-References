...
description: Aspose.Html.HTMLInputElement 班级. 表单控件根据页面被 查看的环境文件上传输入 类型的值属性可能是只读的对于密码输入类型返回的实际值可能被 屏蔽以防止未经授权的使用请参阅 中的 INPUT 元素定义HTML 4.01.
type: docs
weight: 3320
...
表单控件。根据页面被 查看的环境，文件上传输入 类型的值属性可能是只读的。对于“密码”输入类型，返回的实际值可能被 屏蔽以防止未经授权的使用。请参阅 [中的 INPUT 元素定义[HTML 4.01](http://www.w3.org/TR/1999/REC-html401-19991224)].
...
| 姓名 | 描述 |
| --- | --- |
| [Accept](../../aspose.html/htmlinputelement/accept/) { get; set; } | 处理此 表单的服务器将正确处理的以逗号分隔的内容类型列表。请参阅 HTML 4.01. 中的接受属性定义 |
| [AccessKey](../../aspose.html/htmlinputelement/accesskey/) { get; set; } | 用于访问表单控件的单个字符访问键。请参阅 HTML 4.01. 中的 accesskey 属性定义 |
| [Align](../../aspose.html/htmlinputelement/align/) { get; set; } | 相对于其 周围文本对齐此对象（垂直或水平）。请参阅 HTML 4.01 中的对齐属性定义。 此属性在 HTML 4.01 中已弃用。 |
...
| [Checked](../../aspose.html/htmlinputelement/checked/) { get; set; } | 当`类型`元素的属性具有值 “radio”或“checkbox”，这表示在交互式用户代理中表单 控件的当前状态。更改此属性 会更改表单控件的状态，但不会更改 INPUT 元素的 HTML checked 属性的值。在处理具有 值“radio”或“checkbox”，某些实现可能会在 文档中调度事件之前更改此属性的 值。如果取消事件的默认操作，则属性的值 可能会更改回其原始值。这意味着 在处理单击事件期间此属性的值 取决于实现。 |
...
| [DefaultChecked](../../aspose.html/htmlinputelement/defaultchecked/) { get; set; } | 什么时候`类型`具有值“radio”或“checkbox”，这个 代表元素的HTML checked属性。如果相应表单 控件的状态在交互式用户代理中发生更改，则 此属性的值不会更改。请参阅 HTML 4.01. 中已检查的 属性定义 |
| [DefaultValue](../../aspose.html/htmlinputelement/defaultvalue/) { get; set; } | 当`类型`元素的属性具有值 “文本”、“文件”或“密码”，这表示元素的 HTML 值 属性。如果相应表单控件的内容在交互式 用户代理中发生更改，则此属性的值不会更改 。请参阅 HTML 4.01. 中的值属性定义 |
| [Dir](../../aspose.html/htmlelement/dir/) { get; set; } | 指定方向性中性文本的基本方向和表格的 方向性。请参阅 HTML 4.01. 中的 dir 属性定义 |
| [Disabled](../../aspose.html/htmlinputelement/disabled/) { get; set; } | 控件在此上下文中不可用。请参阅 HTML 4.01. 中的禁用属性 定义 |
| [Files](../../aspose.html/htmlinputelement/files/) { get; } | files IDL 属性允许脚本访问元素的选定文件。 获取时，如果 IDL 属性适用，它必须返回一个代表当前选定文件的 FileList 对象。 必须返回相同的对象，直到所选文件的列表发生变化。如果 IDL 属性不适用，则它必须返回 null。 [文件 API] |
...
| [Form](../../aspose.html/htmlinputelement/form/) { get; set; } | 返回`形式`包含此控件的元素。返回 `无效的`如果此控件不在 窗体的上下文中。 |
...
| [MaxLength](../../aspose.html/htmlinputelement/maxlength/) { get; set; } | 文本字段的最大字符数，当`类型` 的值为“文本”或“密码”。请参阅 HTML 4.01. 中的 maxlength 属性 定义 |
| [Name](../../aspose.html/htmlinputelement/name/) { get; set; } | 与表单一起提交时的表单控件或对象名称。请参阅 HTML 4.01. 中的名称 属性定义 |
...
| [ReadOnly](../../aspose.html/htmlinputelement/readonly/) { get; set; } | 此控件是只读的。只有当`类型`具有 值“文本”或“密码”。请参阅 HTML 4.01. 中的只读属性定义 |
...
| [Size](../../aspose.html/htmlinputelement/size/) { get; set; } | 大小信息。确切含义特定于每种类型的 字段。请参阅 HTML 4.01. 中的大小属性定义 @version DOM Level 2 |
| [Src](../../aspose.html/htmlinputelement/src/) { get; set; } | 当`类型`属性的值为“image”，此 属性指定用于装饰 图形提交按钮的图像的位置。请参阅 HTML 4.01. 中的 src 属性定义 |
| [Style](../../aspose.html/htmlelement/style/) { get; } | 表示样式属性，允许作者将样式信息直接应用于特定元素。 |
| [TabIndex](../../aspose.html/htmlinputelement/tabindex/) { get; set; } | 表示元素在 Tab 键顺序中的位置的索引。请参阅 HTML 4.01. 中的 tabindex 属性定义 |
...
| [Type](../../aspose.html/htmlinputelement/type/) { get; set; } | 创建的控件类型（全部小写）。请参阅 HTML 4.01 中的类型属性定义。 @version DOM 级别 2 |
| [UseMap](../../aspose.html/htmlinputelement/usemap/) { get; set; } | 使用客户端图像映射。请参阅 HTML 4.01. 中的 usemap 属性定义 |
| [Value](../../aspose.html/htmlinputelement/value/) { get; set; } | 当`类型`元素的属性具有值 “文本”、“文件”或“密码”，这表示在交互式用户代理中 相应表单控件的当前内容。 更改此属性会更改表单控件的内容，但 不会更改该元素的 HTML value 属性的值。 当`类型`元素的属性具有值 "button"、"hidden"、"submit"、"reset"、"image"、"checkbox" 或 "radio"，这表示元素的 HTML 值属性。请参阅 HTML 4.01. 中的值属性定义 |
