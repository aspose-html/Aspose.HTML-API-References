...
description: 表单控件根据查看页面的环境 文件上传输入的 value 属性可能是只读的 类型对于密码输入类型返回的实际值可能是 被屏蔽以防止未经授权的使用请参阅 HTML 4.01http//www.w3.org/TR/1999/REC-html401-19991224  中的 INPUT 元素定义
type: docs
weight: 3400
url: /zh/net/aspose.html/htmlinputelement/
---
## HTMLInputElement class

表单控件。根据查看页面的环境 文件上传输入的 value 属性可能是只读的 类型。对于“密码”输入类型，返回的实际值可能是 被屏蔽以防止未经授权的使用。请参阅 [[HTML 4.01](http://www.w3.org/TR/1999/REC-html401-19991224) ] 中的 INPUT 元素定义。
...
| 姓名 | 描述 |
| --- | --- |
| [Accept](../../aspose.html/htmlinputelement/accept) { get; set; } | 处理此 表单的服务器将正确处理的内容类型的逗号分隔列表。请参阅 HTML 4.01 中的接受属性定义。 |
| [AccessKey](../../aspose.html/htmlinputelement/accesskey) { get; set; } | 用于访问表单控件的单字符访问键。请参阅 HTML 4.01 中的 accesskey 属性定义。 |
| [Align](../../aspose.html/htmlinputelement/align) { get; set; } | 相对于其 周围文本对齐此对象（垂直或水平）。请参阅 HTML 4.01 中的 align 属性定义。 此属性在 HTML 4.01 中已弃用。 |
| [Alt](../../aspose.html/htmlinputelement/alt) { get; set; } | 用户代理的替代文本不呈现此 元素的正常内容。请参阅 HTML 4.01 中的 alt 属性定义。 |
...
| [Checked](../../aspose.html/htmlinputelement/checked) { get; set; } | 当元素的` 类型` 属性值为 "radio " 或 "复选框"，这表示表单 控件在交互式用户代理中的当前状态。更改此属性 会更改表单控件的状态，但不会更改 INPUT 元素的 HTML 选中属性的值。在处理:::47 期间:输入元素上的单击事件的 值为 "radio" 或 "checkbox" 的 type 属性，某些实现可能会在之前更改此属性的 值该事件正在 文档中发送。如果事件的默认动作被取消，则属性的值 可能会变回其原始值。这意味着 在处理点击事件期间该属性的值 取决于实现。 |
...
| [ClassName](../../aspose.html/htmlelement/classname) { get; set; } | 元素的类属性。由于 与许多语言公开的“类”关键字冲突，此属性已被重命名。请参阅 HTML 4.01 中的类属性定义。 |
| [DefaultChecked](../../aspose.html/htmlinputelement/defaultchecked) { get; set; } | 当` type` 的值为“radio”或“checkbox”时，这 表示元素的 HTML 检查属性。如果在交互式用户代理中对应的表单 控件的状态发生更改，则 的值不会更改。请参阅 HTML 4.01 中选中的 属性定义。 |
| [DefaultValue](../../aspose.html/htmlinputelement/defaultvalue) { get; set; } | 当元素的` 类型` 属性的值为 "text ", "file" 或 "password"，这表示元素的 HTML 值 属性。如果在交互式 用户代理中相应表单控件的内容发生更改，则此属性的值不会更改 。请参阅 HTML 4.01 中的 value 属性定义。 |
| [Dir](../../aspose.html/htmlelement/dir) { get; set; } | 指定方向性中性文本的基本方向和表格的 方向性。请参阅 HTML 4.01 中的 dir 属性定义。 |
| [Disabled](../../aspose.html/htmlinputelement/disabled) { get; set; } | 控件在此上下文中不可用。请参阅 HTML 4.01 中的 disabled 属性 定义。 |
| [Files](../../aspose.html/htmlinputelement/files) { get; } | files IDL 属性允许脚本访问元素的选定文件。 在获取时，如果 IDL 属性适用，它必须返回一个 FileList 对象，该对象代表当前选定的文件。 必须返回相同的对象，直到所选文件列表发生更改。如果 IDL 属性不适用，则它必须改为返回 null。 [文件API] |
...
| [Form](../../aspose.html/htmlinputelement/form) { get; set; } | 返回包含此控件的` FORM` 元素。如果此控件不在 表单的上下文中，则返回 ` null` 。 |
| [Id](../../aspose.html/htmlelement/id) { get; set; } | 元素的标识符。请参阅 HTML 4.01 中的 id 属性定义。 |
| [InnerHTML](../../aspose.html.dom/element/innerhtml) { get; set; } | 返回代表元素内容的 HTML 或 XML 片段。 可以设置，用从给定字符串解析的节点替换元素的内容。 |
| [Lang](../../aspose.html/htmlelement/lang) { get; set; } | RFC 1766 中定义的语言代码。参见 HTML 4.01 中的 lang 属性定义。 |
...
| [List](../../aspose.html/htmlinputelement/list) { get; set; } | list 属性用于标识列出向用户建议的预定义选项的元素。 如果存在，其值必须是同一文档中数据列表元素的 ID。 |
...
| [MaxLength](../../aspose.html/htmlinputelement/maxlength) { get; set; } | 文本字段的最大字符数，当` 类型` 具有值“文本”或“密码”。请参阅 HTML 4.01 中的 maxlength 属性 定义。 |
| [Name](../../aspose.html/htmlinputelement/name) { get; set; } | 与表单一起提交时的表单控件或对象名称。请参阅 HTML 4.01 中的 name 属性定义。 |
...
| [ReadOnly](../../aspose.html/htmlinputelement/readonly) { get; set; } | 此控件是只读的。仅当` 类型` 具有 值“文本”或“密码”时才相关。请参阅 HTML 4.01 中的只读属性定义。 |
...
| [Size](../../aspose.html/htmlinputelement/size) { get; set; } | 大小信息。确切的含义特定于每种类型的 字段。请参阅 HTML 4.01 中的 size 属性定义。 @version DOM 级别 2 |
| [Src](../../aspose.html/htmlinputelement/src) { get; set; } | 当` 类型` 属性具有值“图像”时，此 属性指定用于装饰 图形提交按钮的图像的位置。请参阅 HTML 4.01 中的 src 属性定义。 |
| [Style](../../aspose.html/htmlelement/style) { get; } | 表示允许作者直接将样式信息应用于特定元素的样式属性。 |
| [TabIndex](../../aspose.html/htmlinputelement/tabindex) { get; set; } | 表示元素在跳位顺序中的位置的索引。请参阅 HTML 4.01 中的 tabindex 属性定义。 |
...
| [Title](../../aspose.html/htmlelement/title) { get; set; } | 元素的建议标题。请参阅 HTML 4.01 中的 title 属性定义。 |
| [Type](../../aspose.html/htmlinputelement/type) { get; set; } | 创建的控件类型（全部小写）。请参阅 HTML 4.01 中的 type 属性定义。 @version DOM 级别 2 |
| [UseMap](../../aspose.html/htmlinputelement/usemap) { get; set; } | 使用客户端图像映射。请参阅 HTML 4.01 中的 usemap 属性定义。 |
| [Value](../../aspose.html/htmlinputelement/value) { get; set; } | 当元素的` 类型` 属性的值为 "text ", "file" 或 "password", 这表示 相应表单控件的当前内容，在交互式用户代理中。 更改此属性会更改表单控件的内容，但 不会更改元素的 HTML value 属性的值。 当元素的` 类型` 属性值为 "button", "hidden", "submit "、"reset"、"image"、"checkbox" 或 "radio"，这表示元素的 HTML 值属性。请参阅 HTML 4.01 中的 value 属性定义。 |
