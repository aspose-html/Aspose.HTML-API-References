...
description: Aspose.Html.Dom.Events.InputEvent 班级. 每当更新 DOM 时输入事件都会作为通知发送
type: docs
weight: 820
url: /zh/net/aspose.html.dom.events/inputevent/
---
## InputEvent class
...
| [Data](../../aspose.html.dom.events/inputevent/data/) { get; } | 该数据保存输入法生成的字符值。这可以是单个 Unicode 字符或 Unicode 字符的非空序列 [Unicode]。字符应该按照 [UAX15] 中定义的 Unicode 规范化形式 NFC 的定义进行规范化。该属性可以包含空字符串。 |
...
| [IsComposing](../../aspose.html.dom.events/inputevent/iscomposing/) { get; } | 如果输入事件作为组合会话的一部分发生，即在组合开始事件之后和相应的组合结束事件之前，则为真。此属性的未初始化值必须为 false. |
