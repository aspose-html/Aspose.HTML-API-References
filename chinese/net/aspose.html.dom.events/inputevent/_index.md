...
description: 每当更新 DOM 时输入事件都会作为通知发送
type: docs
weight: 930
...
| 姓名 | 描述 |
| --- | --- |
| [InputEvent](inputevent#constructor)(string) | 初始化[`InputEvent`](../inputevent)类的新实例。 |
| [InputEvent](inputevent#constructor_1)(string, IDictionary&lt;string, object&gt;) | 初始化[`InputEvent`](../inputevent)类的新实例。 |
...
| [Data](../../aspose.html.dom.events/inputevent/data) { get; } | 数据保存输入法生成的字符的值。这可能是单个 Unicode 字符或非空的 Unicode 字符序列 [Unicode]。字符应该按照 [UAX15] 中定义的 Unicode 规范化形式 NFC 的定义进行规范化。该属性可以包含空字符串。 |
...
| [IsComposing](../../aspose.html.dom.events/inputevent/iscomposing) { get; } | 如果输入事件作为合成会话的一部分发生，即在合成开始事件之后和相应的合成结束事件之前，则为真。该属性的未初始化值必须为假。 |
