---
title: Page
second_title: Aspose.HTML for .NET API 参考
description: 表示一个页面对象用于配置输出页面 页面尺寸的空白值为 A4210x297mm
type: docs
weight: 2890
url: /zh/net/aspose.html.drawing/page/
---
## Page class

表示一个页面对象，用于配置输出页面。 页面尺寸的空白值为 A4(210x297mm)

```csharp
public class Page
```

## 构造函数

| 姓名 | 描述 |
| --- | --- |
| [Page](page#constructor)() | 初始化[`Page`](../page)类的新实例。 |
| [Page](page#constructor_1)(Margin) | 初始化[`Page`](../page)类的新实例。 |
| [Page](page#constructor_2)(Size) | 初始化[`Page`](../page)类的新实例。 |
| [Page](page#constructor_3)(Size, Margin) | 初始化[`Page`](../page)类的新实例。 |

## 特性

| 姓名 | 描述 |
| --- | --- |
| [Margin](../../aspose.html.drawing/page/margin) { get; set; } | 获取或设置页边距。 |
| [Size](../../aspose.html.drawing/page/size) { get; set; } | 获取或设置页面大小。 |

### 也可以看看

* 命名空间 [Aspose.Html.Drawing](../../aspose.html.drawing)
* 部件 [Aspose.HTML](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.HTML.dll -->
