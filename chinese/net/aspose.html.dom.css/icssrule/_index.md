...
description: CSSRule 接口是任何类型的 CSS 语句的抽象基接口这包括规则集和规则实现应保留 CSS 样式表中指定的所有规则即使解析器无法识别该规则无法识别的规则使用ICSSUnknownRule接口表示
type: docs
weight: 580
...
CSSRule 接口是任何类型的 CSS 语句的抽象基接口。这包括规则集和规则。实现应保留 CSS 样式表中指定的所有规则，即使解析器无法识别该规则。无法识别的规则使用!:ICSSUnknownRule接口表示。
...
| [Type](../../aspose.html.dom.css/icssrule/type) { get; } | 规则的类型，如上定义。期望是特定于绑定的转换方法可用于从 CSSRule 接口的实例向下转换为类型所隐含的特定派生接口。 |
