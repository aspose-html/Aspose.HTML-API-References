...
description: 过滤器是知道如何过滤掉节点的对象如果 NodeIterator 或 TreeWalker 被赋予 NodeFilter它会在返回下一个 节点之前应用过滤器如果过滤器说接受该节点则遍历逻辑返回 it否则遍历查找下一个节点并假装被拒绝的 节点不存在
type: docs
weight: 2600
...
| 姓名 | 描述 |
| --- | --- |
| [AcceptNode](../../aspose.html.dom.traversal/inodefilter/acceptnode)(Node) | 测试指定节点是否在 TreeWalker 或 NodeIterator 的逻辑视图中可见。这个函数 将被 TreeWalker 和 NodeIterator 的实现调用；它通常不直接从 用户代码中调用。 （尽管如果您想使用相同的 过滤器来指导您自己的应用程序逻辑，您可以这样做。） |
