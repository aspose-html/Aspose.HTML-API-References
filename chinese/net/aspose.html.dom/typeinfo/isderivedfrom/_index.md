...
description: 如果在引用类型定义即调用该方法的 TypeInfo与另一个类型定义即通过的类型定义之间存在派生则此方法返回作为参数
type: docs
weight: 30
...
如果在引用类型定义（即调用该方法的 TypeInfo）与另一个类型定义（即通过的类型定义）之间存在派生，则此方法返回作为参数。
...
| derivationMethod | UInt64 | 两种类型之间应用的派生类型和条件，如此接口中提供的常量列表中所述。 |
