---
title: SVGOptimizationOptions.RemoveUselessStrokeAndFill
second_title: Aspose.HTML for .NET API 参考
description: SVGOptimizationOptions 财产. Gets of 设置删除未使用的描边和填充属性的选项默认情况下为真
type: docs
weight: 140
url: /zh/net/aspose.html.toolkit.optimizers/svgoptimizationoptions/removeuselessstrokeandfill/
---
## SVGOptimizationOptions.RemoveUselessStrokeAndFill property

Gets of 设置删除未使用的描边和填充属性的选项。默认情况下为“真”。

```csharp
public bool RemoveUselessStrokeAndFill { get; set; }
```

### 也可以看看

* class [SVGOptimizationOptions](../)
* 命名空间 [Aspose.Html.Toolkit.Optimizers](../../svgoptimizationoptions/)
* 部件 [Aspose.HTML](../../../)


