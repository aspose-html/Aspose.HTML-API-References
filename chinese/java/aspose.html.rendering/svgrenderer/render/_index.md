...
description: "SvgRenderer 方法。 定义用于将多个 SVGDocument 渲染到特定 IDevice 的方法。 渲染将在没有任何网络资源加载操作、活动计时器、动画任务，或在指定超时已到期时执行。"
type: docs
...
定义用于将多个[`SVGDocument`](../../../com.aspose.html.dom.svg/svgdocument/)渲染到特定[`IDevice`](../../idevice/)的方法。 渲染将在没有任何网络资源加载操作、活动计时器、动画任务，或在指定超时已到期时执行。
...
定义一种方法，将多个[`SVGDocument`](../../../com.aspose.html.dom.svg/svgdocument/)渲染到特定[`IDevice`](../../idevice/)，并使用取消令牌来请求取消该操作。
...
| --- | --- | --- |
| 设备 | IDevice | 输出设备。 |
| cancellationToken | CancellationToken | 在等待任务完成期间需要观察的取消令牌。 |
