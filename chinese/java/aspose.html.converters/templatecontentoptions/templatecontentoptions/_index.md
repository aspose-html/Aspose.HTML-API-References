---
title: "TemplateContentOptions"
second_title: "Aspose.HTML for Java API 参考"
description: "TemplateContentOptions 构造函数。 基于内联 String 的初始化"
type: docs

url: /zh/java/com.aspose.html.converters/templatecontentoptions/templatecontentoptions/
---
## TemplateContentOptions constructor

基于内联（String）的初始化

```java
public TemplateContentOptions(String dataContent, TemplateContent contentType)
```

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| dataContent | String | 内联（String）内容。 |
| contentType | TemplateContent | 内容类型。 |

### 另请参阅

* enum [TemplateContent](../../templatecontent/)
* class [TemplateContentOptions](../)
* package [com.aspose.html.converters](../../../com.aspose.html.converters/)
* package [Aspose.HTML](../../../)
