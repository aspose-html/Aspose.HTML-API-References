...
| outputPath | String | 完整的 html 文件路径作为输出转换结果。 |

## 备注

模板合并器

模板合并的理念是基于 HTML 模板创建 HTML 文档，并从数据源填充内容。Aspose.HTML 提供内联表达式语法以处理模板和各种数据源类型，如 XML 和 JSON。请参阅 [article](https://docs.aspose.com/html/net/converting-between-formats/html-template/) 以获取有关模板合并和使用 ConvertTemplate() 方法的更多信息。
...
合并由 [`URL`](../../../com.aspose.html/url/) 提供的模板 HTML 源与模板数据（XML，JSON）。结果是由输出文件路径生成的 HTML 文件。
...
将通过完整文件路径呈现的模板 HTML 源与模板数据（XML、JSON）合并。结果是通过输出文件路径形成的 html 文件。
...
将通过内联内容呈现的模板 HTML 源与模板数据（XML、JSON）合并。结果是通过输出文件路径形成的 html 文件。
