---
title: "MarkdownSyntaxVisitor.VisitEscapedCharacter"
second_title: "Aspose.HTML for Java API 参考"
description: "MarkdownSyntaxVisitor 方法。定义用于访问转义字符的接口"
type: docs

url: /zh/java/com.aspose.html.toolkit.markdown.syntax/markdownsyntaxvisitor/visitescapedcharacter/
---
## MarkdownSyntaxVisitor.VisitEscapedCharacter method

定义访问转义字符的接口。

```java
public void VisitEscapedCharacter(EscapedCharacterSyntaxNode character)
```

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| character | EscapedCharacterSyntaxNode | 转义字符。 |

### 另请参阅

* class [EscapedCharacterSyntaxNode](../../escapedcharactersyntaxnode/)
* class [MarkdownSyntaxVisitor](../)
* package [com.aspose.html.toolkit.markdown.syntax](../../../com.aspose.html.toolkit.markdown.syntax/)
* package [Aspose.HTML](../../../)
