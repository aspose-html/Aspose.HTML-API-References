...
description: "INodeFilter 方法。测试指定节点在 TreeWalker 或 NodeIterator 的逻辑视图中是否可见。此函数将由 TreeWalker 和 NodeIterator 的实现调用，通常不会直接从用户代码调用。不过，如果您想使用相同的过滤器来指导自己的应用逻辑，也可以这样做。"
type: docs
