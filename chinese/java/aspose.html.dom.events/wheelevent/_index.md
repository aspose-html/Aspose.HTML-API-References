...
| [getButton](../../com.aspose.html.dom.events/mouseevent/button/) 在因按下或释放鼠标按钮而产生的鼠标事件期间，必须使用 button 来指示哪个指针设备按钮的状态发生了变化。 |
| [getButtons](../../com.aspose.html.dom.events/mouseevent/buttons/) 在任何鼠标事件期间，必须使用 buttons 来指示当前被按下的鼠标按钮组合，以位掩码形式表示。 |
...
| [getDeltaX](../../com.aspose.html.dom.events/wheelevent/deltax/) 在默认操作为滚动的用户代理中，如果事件未被取消，则该值必须是沿 x 轴的滚动量（以像素、行或页为单位）。否则，它是滚轮设备围绕 x 轴移动的实现特定测量（以像素、行或页为单位）。 |
| [getDeltaY](../../com.aspose.html.dom.events/wheelevent/deltay/) 在默认操作为滚动的用户代理中，如果事件未被取消，则该值必须是沿 y 轴的滚动量（以像素、行或页为单位）。否则，它是滚轮设备围绕 y 轴移动的实现特定测量（以像素、行或页为单位）。 |
| [getDeltaZ](../../com.aspose.html.dom.events/wheelevent/deltaz/) 在默认操作为滚动的用户代理中，如果事件未被取消，则该值必须是沿 z 轴的滚动量（以像素、行或页为单位）。否则，它是滚轮设备围绕 z 轴移动的实现特定测量（以像素、行或页为单位）。 |
...
| [getRelatedTarget](../../com.aspose.html.dom.events/mouseevent/relatedtarget/) 用于识别与 UI 事件相关的次要 EventTarget，取决于事件类型。 |
...
| 名称 | 描述 |
| --- | --- |
| const [DOM_DELTA_LINE](../../com.aspose.html.dom.events/wheelevent/dom_delta_line/) | delta 的计量单位必须是单独的文本行。这在许多表单控件中适用。 |
| const [DOM_DELTA_PAGE](../../com.aspose.html.dom.events/wheelevent/dom_delta_page/) | delta 的计量单位必须是页面，可以定义为单个屏幕或划分的页面。 |
