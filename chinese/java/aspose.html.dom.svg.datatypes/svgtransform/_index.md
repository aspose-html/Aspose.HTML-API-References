...
description: "com.aspose.html.dom.svg.datatypes.SVGTransform 类。SVGTransform 是 SVGTransformList 中组件变换之一的接口，因此 SVGTransform 对象对应于单个组件，例如在 transform 属性规范中的 scale（缩放）或 matrix（矩阵）。"
type: docs
...
SVGTransform 是 SVGTransformList 中的一个组件变换的接口；因此，SVGTransform 对象对应于 ‘transform’ 属性规范中的单个组件（例如 'scale(…)' 或 'matrix(…)')。
...
| [setMatrix](../../com.aspose.html.dom.svg.datatypes/svgtransform/setmatrix/)(SVGMatrix) | 将变换类型设置为 SVG_TRANSFORM_MATRIX，参数 matrix 定义新的变换。参数矩阵的值会被复制，matrix 参数不会替换 SVGTransform::matrix。 |
