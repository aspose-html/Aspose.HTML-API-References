---
title: "SVGOptimizationOptions.RemoveEmptyAttributes"
second_title: "Aspose.HTML for Java API 参考"
description: "SVGOptimizationOptions 属性。获取或设置删除值为空的属性的选项。默认值为 true。"
type: docs

url: /zh/java/com.aspose.html.toolkit.optimizers/svgoptimizationoptions/removeemptyattributes/
---
## SVGOptimizationOptions.RemoveEmptyAttributes property

获取或设置删除值为空的属性的选项。默认值为 'true'。

```java
public bool RemoveEmptyAttributes { get; set; }
```

### 另请参见

* class [SVGOptimizationOptions](../)
* package [com.aspose.html.toolkit.optimizers](../../../com.aspose.html.toolkit.optimizers/)
* package [Aspose.HTML](../../../)
