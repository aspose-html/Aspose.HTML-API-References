...
description: "com.aspose.html.dom.canvas.Path2D 类。Path2D 接口是 Canvas 2D API 用于声明路径的接口，随后这些路径会在 CanvasRenderingContext2D 对象上使用。CanvasRenderingContext2D 接口的路径方法也在此接口中提供，允许您创建可以在画布上按需保留和重放的路径。"
type: docs
...
Canvas 2D API 的 Path2D 接口用于声明路径，这些路径随后在 CanvasRenderingContext2D 对象上使用。CanvasRenderingContext2D 接口的路径方法也在此接口中提供，允许您创建可以在画布上根据需要保留和重放的路径。
...
| [Path2D](path2d/#constructor_1)(Path2D) | 返回一个新实例化的 Path2D 对象，参数为另一个路径（创建副本） |
| [Path2D](path2d/#constructor_2)(String) | 返回一个新实例化的 Path2D 对象，参数为包含 SVG 路径数据的字符串。 |
...
| [bezierCurveTo](../../com.aspose.html.dom.canvas/path2d/beziercurveto/)(double, double, double, double, double, double) | 向路径添加三次 Bézier 曲线。它需要三个点。前两个点是控制点，第三个点是终点。起始点是当前路径中的最后一点，可在创建 Bézier 曲线之前使用 moveTo() 更改。 |
| [closePath](../../com.aspose.html.dom.canvas/path2d/closepath/)() | 使笔尖移动回当前子路径的起点。它尝试从当前点绘制一条直线到起点。如果形状已经闭合或只有一个点，此函数不执行任何操作。 |
...
| [rect](../../com.aspose.html.dom.canvas/path2d/rect/)(double, double, double, double) | 在位置 (x, y) 创建一个矩形路径，其大小由 width 和 height 决定。 |
