...
description: "ICanvasRenderingContext2D 方法。绘制指定的图像"
type: docs

url: /zh/java/com.aspose.html.dom.canvas/icanvasrenderingcontext2d/drawimage/
---
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 图像 | HTMLImageElement | 用于绘制到上下文中的 HTMLImageElement。 |
| dx | Double | 在目标画布中放置源图像左上角的 X 坐标。 |
| dy | Double | 在目标画布中放置源图像左上角的 Y 坐标。 |
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 图像 | HTMLCanvasElement | 用于绘制到上下文中的 HTMLCanvasElement。 |
| dx | Double | 在目标画布中放置源图像左上角的 X 坐标。 |
| dy | Double | 在目标画布中放置源图像左上角的 Y 坐标。 |
...
| dx | Double | 在目标画布中放置源图像左上角的 X 坐标。 |
| dy | Double | 在目标画布中放置源图像左上角的 Y 坐标。 |
| dw | Double | 在目标画布中绘制图像的宽度。这允许对绘制的图像进行缩放。如果未指定，绘制时图像的宽度不会被缩放。 |
| dh | Double | 在目标画布中绘制图像的高度。这允许对绘制的图像进行缩放。如果未指定，绘制时图像的高度不会被缩放。 |
...
| --- | --- | --- |
| 图像 | HTMLImageElement | 用于绘制到上下文中的 HTMLImageElement。 |
| sx | Double | 在目标上下文中绘制的源图像子矩形左上角的 X 坐标。 |
| sy | Double | 在目标上下文中绘制的源图像子矩形左上角的 Y 坐标。 |
| sw | Double | 在目标上下文中绘制的源图像子矩形的宽度。如果未指定，则使用从 sx 和 sy 指定的坐标到图像右下角的整个矩形。 |
| sh | Double | 在目标上下文中绘制的源图像子矩形的高度。 |
| dx | Double | 在目标画布中放置源图像左上角的 X 坐标。 |
| dy | Double | 在目标画布中放置源图像左上角的 Y 坐标。 |
| dw | Double | 在目标画布中绘制图像的宽度。这允许对绘制的图像进行缩放。如果未指定，绘制时图像的宽度不会被缩放。 |
| dh | Double | 在目标画布中绘制图像的高度。这允许对绘制的图像进行缩放。如果未指定，绘制时图像的高度不会被缩放。 |
