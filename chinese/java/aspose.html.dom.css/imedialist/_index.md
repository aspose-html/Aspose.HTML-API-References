...
MediaList 接口提供媒体有序集合的抽象，而不定义或限制该集合的实现方式。空列表等同于包含媒体 "all" 的列表。
...
| 名称 | 描述 |
| --- | --- |
| [getItem](../../com.aspose.html.dom.css/imedialist/item/) item(index) 方法必须返回由索引指定的媒体查询集合的序列化表示，如果索引大于或等于媒体查询集合的数量，则返回 null。 |
| [getLength](../../com.aspose.html.dom.css/imedialist/length/) length 属性必须返回媒体查询集合中的媒体查询数量。有效媒体的范围为 0 到 length-1（含）。 |
| [getMediaText](../../com.aspose.html.dom.css/imedialist/mediatext/) 一个 Stringifier，返回表示 MediaList 为文本的 DOMString，并且允许设置新的 MediaList。 |
...
注意：MediaList 是实时列表；使用下列属性或方法更新列表会立即影响文档的行为。
