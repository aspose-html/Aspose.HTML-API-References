...
description: "ICSSRule 属性。如果此规则位于另一个规则内部，例如位于媒体块中的样式规则，则它是包含的规则。如果此规则未嵌套在任何其他规则中，则返回 null。"
type: docs
...
如果此规则位于另一个规则内部（例如位于 @media 块中的样式规则），则它是包含的规则。如果此规则未嵌套在任何其他规则中，则返回 null。
