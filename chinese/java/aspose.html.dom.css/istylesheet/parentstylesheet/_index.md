...
description: "IStyleSheet 属性。对于支持样式表包含概念的样式表语言，此属性表示包含的样式表（如果存在）。如果样式表是顶层样式表，或该样式表语言不支持包含，则此属性的值为 null。"
type: docs
...
对于支持样式表包含概念的样式表语言，此属性表示包含的样式表（如果存在）。如果样式表是顶层样式表，或该语言不支持包含，则此属性的值为 null。
