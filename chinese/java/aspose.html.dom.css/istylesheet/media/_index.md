...
description: "IStyleSheet 属性。StyleSheet 接口的 media 属性指定样式信息的目标媒体。它是只读的类数组 MediaList 对象，可通过 deleteMedium 移除，或通过 appendMedium 添加。"
type: docs
...
[`StyleSheet`](../) 接口的 media 属性指定样式信息的目标媒体。它是只读的、类数组的 [`MediaList`](../../imedialist/) 对象，可通过 deleteMedium() 移除，或通过 appendMedium() 添加。
