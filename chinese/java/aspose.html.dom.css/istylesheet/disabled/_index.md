...
description: "IStyleSheet 属性。StyleSheet 接口的 disabled 属性决定是否阻止样式表应用于文档。"
type: docs
...
可以通过手动将此属性设为 true，或当它是非活动的替代样式表时来禁用样式表。请注意，disabled == false 并不保证样式表已应用（例如，它可能已从文档中移除）。

修改此属性可能导致文档的样式重新解析。只有在存在适当的媒体定义且 disabled 属性为 false 时，样式表才会应用。因此，如果媒体不适用于当前用户代理，disabled 属性将被忽略。
...
disabled 属性在获取时，如果 disabled 标志已设置则必须返回 true，否则返回 false。设置时，如果新值为 true，disabled 属性必须设置 disabled 标志，否则必须取消设置该标志。
