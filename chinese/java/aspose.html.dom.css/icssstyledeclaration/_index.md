...
可以通过三种不同的 API 来访问 CSSStyleDeclaration 对象：

通过 HTMLElement.style，处理单个元素的内联样式。通过 [`CSSStyleSheet`](../icssstylesheet/) API。例如，document.styleSheets[0].cssRules[0].style 返回文档第一个样式表中第一个 CSS 规则的 `CSSStyleDeclaration` 对象。通过 Window.getComputedStyle()，它以只读接口公开 `CSSStyleDeclaration` 对象。
