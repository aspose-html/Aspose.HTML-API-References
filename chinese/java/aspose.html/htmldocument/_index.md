...
description: "com.aspose.html.HTMLDocument 类。表示一个 HTML 文档。所有顶层 HTML 对象都会添加到此对象中。此类将 HTML 页面表示为我们在浏览器中看到的页面。所有表单、表格、脚本等都通过此类的接口添加到 HTML 页面。HTMLDocument 是最通用的 Document 接口的 HTML 实现，两者都是 DOM（文档对象模型）的核心或根节点。这些概念完全符合官方的网页开发基础或标准。就网页开发而言，通常可以将 HTMLDocument 视为基于 Document 的别名。"
type: docs
...
表示一个 HTML 文档。所有顶层 HTML 对象都会添加到此对象中。此类将 HTML 页面表示为我们在浏览器中看到的页面。所有表单、表格、脚本等都通过此类的接口添加到 HTML 页面中。[HTMLDocument](https://dom.spec.whatwg.org/#ref-for-dom-domimplementation-createhtmldocument) 是最通用的 [Document](https://dom.spec.whatwg.org/#document) 接口的 HTML 实现，两者都是 [DOM](https://dom.spec.whatwg.org/)（文档对象模型）的核心或根节点。这些概念完全符合官方的网页开发基础或标准。就网页开发而言，通常可以将 HTMLDocument 视为 Document 的别名，HTMLDocument 基于 Document。
...
| [HTMLDocument](htmldocument/#constructor_8)(Stream, String) | 从具有指定 base-uri 的 [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream) 内容创建 HTML 文档，该 base-uri 用于解析相对资源的路径。 |
| [HTMLDocument](htmldocument/#constructor_6)(Stream, Url) | 从具有指定 base-uri 的 [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream) 内容创建 HTML 文档，该 base-uri 用于解析相对资源的路径。 |
| [HTMLDocument](htmldocument/#constructor_11)(String, Configuration) | 使用指定的环境配置设置，从地址加载 HTML 文档。 |
...
| [HTMLDocument](htmldocument/#constructor_5)(Url, Configuration) | 使用指定的环境配置设置，从 URL 加载 HTML 文档。 |
| [HTMLDocument](htmldocument/#constructor_9)(Stream, String, Configuration) | 从具有指定 base-uri 和环境配置设置的 [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream) 内容创建 HTML 文档。 |
| [HTMLDocument](htmldocument/#constructor_7)(Stream, Url, Configuration) | 从具有指定 base-uri 和环境配置设置的 [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream) 内容创建 HTML 文档。 |
...
| 名称 | 描述 |
| --- | --- |
| [getAnchors](../../com.aspose.html/htmldocument/anchors/) 文档中所有具有 `name` 属性值的锚点（`A`）元素的集合。出于向后兼容性的考虑，返回的锚点集合仅包含使用 `name` 属性创建的锚点，而不包括使用 `id` 属性创建的锚点。请注意，在 [[XHTML 1.0](http://www.w3.org/TR/2002/REC-xhtml1-20020801)] 中，`name` 属性（参见第 4.10 节）没有语义，仅为旧版用户代理保留：应使用 `id` 属性。用户应优先使用 [[DOM Level 2 Traversal](http://www.w3.org/TR/2000/REC-DOM-Level-2-Traversal-Range-20001113)] 提供的迭代器机制。 |
| [getApplets](../../com.aspose.html/htmldocument/applets/) 文档中包含小程序和已弃用的 `APPLET` 元素的所有 `OBJECT` 元素的集合。 |
...
| [getImages](../../com.aspose.html/htmldocument/images/) 文档中所有 `IMG` 元素的集合。出于向后兼容性的考虑，仅限于 `IMG` 元素。正如 [[HTML 4.01](http://www.w3.org/TR/1999/REC-html401-19991224)] 所建议的，为了包含图像，作者可以使用 `OBJECT` 元素或 `IMG` 元素。因此，建议不要使用此属性来查找文档中的图像，而是使用 HTML 4.01 的 `getElementsByTagName` 或 XHTML 1.0 的 `getElementsByTagNameNS`。 |
...
| [getReferrer](../../com.aspose.html/htmldocument/referrer/) 返回链接到此页面的页面的 URI [[IETF RFC 2396](http://www.ietf.org/rfc/rfc2396.txt)]。如果用户直接导航到该页面（不是通过链接，而是例如通过书签），则该值为空字符串。 |
...
| [save](../../com.aspose.html/htmldocument/save/#save_10)(String) | 将文档保存到由路径指定的本地文件。此文档使用的所有资源将保存到相邻的文件夹中，文件夹名称将构建为：output_file_name + \"_files\"。 |
...
| [save](../../com.aspose.html/htmldocument/save/#save_11)(String, HTMLSaveFormat) | 将文档保存到由 path 指定的本地文件。此文档使用的所有资源将保存到相邻的文件夹中，文件夹名称将构建为 output_file_name + "_files"。 |
| [save](../../com.aspose.html/htmldocument/save/#save_12)(String, HTMLSaveOptions) | 将文档保存到由路径指定的本地文件。此文档使用的所有资源将保存到相邻的文件夹中，文件夹名称将构建为：output_file_name + \"_files\"。 |
| [save](../../com.aspose.html/htmldocument/save/#save_13)(String, MarkdownSaveOptions) | 将文档保存到由路径指定的本地文件。此文档使用的所有资源将保存到相邻的文件夹中，文件夹名称将构建为：output_file_name + \"_files\"。 |
| [save](../../com.aspose.html/htmldocument/save/#save_14)(String, MHTMLSaveOptions) | 将文档保存到由路径指定的本地文件。此文档使用的所有资源将保存到相邻的文件夹中，文件夹名称将构建为：output_file_name + \"_files\"。 |
...
有关 HTMLDocument、Document 和 DOM 的更多信息可在流行的 Web 开发资源中获取：
