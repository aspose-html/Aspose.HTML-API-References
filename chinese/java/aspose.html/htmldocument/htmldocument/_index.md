...
文档对象创建后，可以随后填充 HTML 元素。下面的代码片段展示了使用默认的 HTMLDocument() 构造函数创建空的 HTML 文档并将其保存到文件的用法。
...
注意：如果传入的 URL 错误且当前无法访问，库会抛出 [`DOMException`](../../../com.aspose.html.dom/domexception/) 并带有专用代码 ‘NetworkError’，以告知未找到所选资源。
...
使用指定的环境配置设置从 URL 加载 HTML 文档。

注意：如果传入的 URL 错误且当前无法访问，库会抛出 [DOMException](T:com.aspose.html.dom.DOMException) 并带有专用代码 ‘NetworkError’，以告知未找到所选资源。
...
从指定 base-uri 的 [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream) 内容创建 HTML 文档，该 base-uri 用于解析相对资源的路径。
...
import (var sw = new StreamWriter(mem))
{
	...
	// 重要的是将位置设置为开头，因为 HTMLDocument 从流中当前的位置开始读取。
	sw.Flush();
...
使用指定的 base-uri 和环境配置设置，从 [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream) 内容创建 HTML 文档。
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| request | RequestMessage | 包含有文档内容的 [`body`](../../../com.aspose.html.net/requestmessage/content/) 的请求消息。 |
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| request | RequestMessage | 包含有文档内容的 [body](P:com.aspose.html.net.RequestMessage.Content) 的请求消息。 |
| 配置 | 配置 | 环境配置，例如脚本策略、自定义用户样式表等。 |
