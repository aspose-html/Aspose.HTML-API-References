...
description: "com.aspose.html.HTMLTableElement 类。表格上的创建和删除方法允许作者构建和修改表格。HTML 4.01 规定表格中每种 CAPTION、THEAD 和 TFOOT 元素只能出现一次。因此如果已存在且调用 createTHead 或 createTFoot 方法，方法将返回已有的 THead 或 TFoot 元素。请参阅 HTML 4.01 中的 TABLE 元素定义。"
type: docs
...
表格上的 create* 和 delete* 方法允许作者构建和修改表格。[[HTML 4.01](http://www.w3.org/TR/1999/REC-html401-19991224)] 指定在表格中每种 `CAPTION`、`THEAD` 和 `TFOOT` 元素只能出现一次。因此，如果已经存在，并且调用 createTHead() 或 createTFoot() 方法，方法将返回已有的 THead 或 TFoot 元素。参见 HTML 4.01 中的 TABLE 元素定义。
...
| [createCaption](../../com.aspose.html/htmltableelement/createcaption/)() | 创建新的表格标题对象，或返回已有的对象。 |
