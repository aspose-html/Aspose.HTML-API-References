...
description: "HTMLTableSectionElement 方法。向此节插入一行。新行会立即插入到本节当前第 index 行之前。如果 index 为 -1 或等于本节的行数，则在末尾追加新行。"
type: docs
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| index | Int32 | 要插入新行的行号。此索引从 0 开始，仅相对于本节内包含的行，而不是表中的所有行。 |
