...
表单控件。根据页面查看的环境，文件上传输入类型的 value 属性可能是只读的。对于 \"password\" 输入类型，返回的实际值可能会被掩码以防止未授权使用。参见 [[HTML 4.01](http://www.w3.org/TR/1999/REC-html401-19991224)] 中的 INPUT 元素定义。另请参见 [Document object Model (DOM) Level 2 HTML Specification](http://www.w3.org/TR/2003/REC-DOM-Level-2-HTML-20030109)。
...
| [getFiles](../../com.aspose.html/htmlinputelement/files/) files IDL 属性允许脚本访问元素的已选文件。获取时，如果该 IDL 属性适用，则必须返回表示当前已选文件的 FileList 对象。该对象应保持返回，直至已选文件列表更改。如果该 IDL 属性不适用，则必须返回 null。[FILEAPI] |
