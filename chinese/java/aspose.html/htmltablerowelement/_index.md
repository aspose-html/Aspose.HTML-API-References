...
| [getRowIndex](../../com.aspose.html/htmltablerowelement/rowindex/) 这是逻辑顺序，而非文档顺序。`rowIndex` 会考虑表格中的各个部分（`THEAD`、`TFOOT` 或 `TBODY`），在索引中先放置 `THEAD` 行，其次是 `TBODY` 行，最后是 `TFOOT` 行。 @version DOM Level 2 |
| [getSectionRowIndex](../../com.aspose.html/htmltablerowelement/sectionrowindex/) 此行的索引，相对于当前章节（`THEAD`、`TFOOT` 或 `TBODY`），从 0 开始。 @version DOM Level 2 |
