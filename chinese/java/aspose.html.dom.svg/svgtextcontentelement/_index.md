...
description: "com.aspose.html.dom.svg.SVGTextContentElement 类。SVGTextContentElement 被多个与文本相关的接口继承，如 SVGTextElement、SVGTSpanElement、SVGTRefElement、SVGAltGlyphElement 和 SVGTextPathElement。对于此接口中引用字符索引或字符数量的方法，这些引用应解释为 UTF-16 代码单元的索引或 UTF-16 代码单元的数量。这是为了与 DOM Level 2 Core 保持一致，在该规范中 CharacterData 接口的方法使用 UTF-16 代码单元作为索引和计数。因此，例如，如果文本元素的文本内容是单个非 BMP 字符，如 U+10000，则对该元素调用 getNumberOfChars 将返回 2，因为用于表示该字符的代理对包含两个 UTF-16 代码单元。"
type: docs
