---
title: "Document.CreateAttributeNS"
second_title: "Aspose.HTML for Java API 参考"
description: "Document method. Document.createAttribute 方法创建一个新的属性节点并返回它。该对象创建了一个实现 Attr 接口的节点。DOM 不强制规定可以以此方式添加到特定元素的属性类型。"
type: docs

url: /zh/java/com.aspose.html.dom/document/createattributens/
---
## Document.CreateAttributeNS method

Document.createAttribute() 方法创建一个新的属性节点并返回它。该对象创建了一个实现了 [Attr](T:com.aspose.html.dom.Attr) 接口的节点。DOM 不强制规定可以以这种方式添加到特定元素的属性类型。

```java
public Attr CreateAttributeNS(String packageURI, String qualifiedName)
```

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| packageURI | String | 该属性的包 URI。 |
| qualifiedName | String | name 是一个包含属性名称的字符串。 |

### 返回值

该 [`Attr`](../../attr/)。

### 另请参见

* class [Attr](../../attr/)
* class [Document](../)
* package [com.aspose.html.dom](../../../com.aspose.html.dom/)
* package [Aspose.HTML](../../../)
