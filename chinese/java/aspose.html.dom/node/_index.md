...
description: "com.aspose.html.dom.Node 类。Node 接口是整个文档对象模型的主要数据类型。它表示文档树中的单个节点。虽然所有实现 Node 接口的对象都公开处理子节点的方法，但并非所有实现 Node 接口的对象都可能拥有子节点。例如，Text 节点可能没有子节点，向此类节点添加子节点会导致抛出 DOMException。"
type: docs
...
Node 接口是整个文档对象模型的主要数据类型。它表示文档树中的单个节点。虽然实现 Node 接口的所有对象都公开处理子节点的方法，但并非所有实现 Node 接口的对象都有子节点。例如，[`Text`](../text/) 节点可能没有子节点，向此类节点添加子节点会导致抛出 [`DOMException`](../domexception/)。

属性 [`nodeName`](./nodename/)、[`nodeValue`](./nodevalue/) 以及属性被包含作为一种在不向下转型到特定派生接口的情况下获取节点信息的机制。如果对于特定的 [`nodeType`](./nodetype/) 没有明显的映射（例如，`Element` 的 nodeValue 或 `Comment` 的 attributes），则返回 null。请注意，专门的接口可能包含额外且更方便的机制来获取和设置相关信息。
...
| [getFirstChild](../../com.aspose.html.dom/node/firstchild/) `Node` 接口的只读 firstChild 属性返回树中节点的第一个子节点，如果节点没有子节点则返回 null。 |
| [getLastChild](../../com.aspose.html.dom/node/lastchild/) `Node` 接口的只读 lastChild 属性返回节点的最后一个子节点。如果其父节点是元素，则子节点通常是元素节点、文本节点或注释节点。如果没有子元素则返回 null。 |
| [getLocalName](../../com.aspose.html.dom/node/localname/) 返回此节点限定名称的本地部分。对于除 [`ELEMENT_NODE`](./element_node/) 和 [`ATTRIBUTE_NODE`](./attribute_node/) 之外的任何类型的节点，以及使用 DOM Level 1 方法（如 [`Document.createElement()`](../document/createelement/)）创建的节点，此属性始终为 null。 |
| [getNamespaceURI](../../com.aspose.html.dom/node/packageuri/) Element.packageURI 只读属性返回元素的包 URI；如果元素不在任何包中，则为 null。 |
| [getNextSibling](../../com.aspose.html.dom/node/nextsibling/) `Node` 接口的只读 nextSibling 属性返回其父节点的 [`childNodes`](./childnodes/) 中紧随指定节点之后的节点，如果指定节点是父元素中的最后一个子节点则返回 null。 |
| abstract [getNodeName](../../com.aspose.html.dom/node/nodename/) Node 的只读 nodeName 属性返回当前节点的名称，类型为 String。 |
...
| [getParentElement](../../com.aspose.html.dom/node/parentelement/) `Node` 接口的只读 parentElement 属性返回 DOM 节点的父级 [`Element`](../element/)，如果节点没有父节点或其父节点不是 DOM 元素，则返回 null。 |
...
| [getPreviousSibling](../../com.aspose.html.dom/node/previoussibling/) `Node` 接口的只读 previousSibling 属性返回其父节点的 [`childNodes`](./firstchild/) 列表中紧邻指定节点之前的节点，如果指定节点是列表中的第一个则返回 null。 |
...
| [cloneNode](../../com.aspose.html.dom/node/clonenode/#clonenode)() | cloneNode() 方法属于 Node 接口，返回调用该方法的节点的副本。其参数决定是否同时克隆节点中包含的子树。 |
| [cloneNode](../../com.aspose.html.dom/node/clonenode/#clonenode_1)(bool) | cloneNode() 方法属于 Node 接口，返回调用该方法的节点的副本。其参数决定是否同时克隆节点中包含的子树。 |
...
| [hasChildNodes](../../com.aspose.html.dom/node/haschildnodes/)() | Node 接口的 hasChildNodes() 方法返回一个布尔值，指示给定的 `Node` 是否具有子节点。 |
...
| [isEqualNode](../../com.aspose.html.dom/node/isequalnode/)(Node) | `Node` 接口的 isEqualNode() 方法测试两个节点是否相等。当两个节点具有相同的类型、定义特征（对于元素来说，这包括它们的 ID、子节点数量等）、属性匹配等时即视为相等。必须匹配的数据点集合取决于节点的类型。 |
...
| [normalize](../../com.aspose.html.dom/node/normalize/)() | 将此 Node 下子树的全部深度中的所有 [`Text`](../text/) 节点（包括属性节点）转换为“正常”形式，使得只有结构（例如 [`elements`](../element/)、[`comments`](../comment/)、[`processing instructions`](../processinginstruction/)、[`CDATA sections`](../cdatasection/)、[`entity references`](../entityreference/)）分隔 [`Text`](../text/) 节点，即不存在相邻的 Text 节点或空的 Text 节点。这可用于确保文档的 DOM 视图与保存后重新加载时相同，并在需要依赖特定文档树结构的操作（如 XPointer [XPointer] 查找）时非常有用。如果附加到 [`Node.ownerDocument`](./ownerdocument/) 的 [`DOMConfiguration`](../../com.aspose.html/configuration/) 对象的参数 "normalize-characters" 为 true，则此方法还会完全规范化 Text 节点的字符。 |
...
| 名称 | 描述 |
| --- | --- |
| const [ATTRIBUTE_NODE](../../com.aspose.html.dom/node/attribute_node/) | 一个 [`Attribute`](../attr/) 属于 [`Element`](../element/)。 |
...
| const [TEXT_NODE](../../com.aspose.html.dom/node/text_node/) | 实际位于 [`Element`](../element/) 或 [`Attr`](../attr/) 内的 [`Text`](../text/)。 |
