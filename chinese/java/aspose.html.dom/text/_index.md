...
description: "com.aspose.html.dom.Text 类。Text 接口继承自 CharacterData，表示 Element 或 Attr 在 XML 中的文本内容（称为字符数据）。"
type: docs
...
Text 接口继承自 CharacterData，表示 Element 或 Attr 的文本内容（在 XML 中称为字符数据）。
...
| [getNodeName](../../com.aspose.html.dom/text/nodename/) 根据其类型，此节点的名称。 |
...
| [normalize](../../com.aspose.html.dom/node/normalize/)() | 将此节点下子树的全部深度中的所有 `Text` 节点（包括属性节点）转换为"正常"形式，即仅有结构（例如 [`elements`](../element/)、[`comments`](../comment/)、[`processing instructions`](../processinginstruction/)、[`CDATA sections`](../cdatasection/)、以及 [`entity references`](../entityreference/)）将 `Text` 节点分隔开，确保不存在相邻的 Text 节点或空的 Text 节点。此操作可用于确保文档的 DOM 视图与保存后重新加载时相同，并在需要依赖特定文档树结构的操作（如 XPointer [XPointer] 查找）时非常有用。如果附加到 [`Node.ownerDocument`](../node/ownerdocument/) 的 [`DOMConfiguration`](../../com.aspose.html/configuration/) 对象的参数 "normalize-characters" 为 true，则此方法还会完全规范化 Text 节点的字符。 |
