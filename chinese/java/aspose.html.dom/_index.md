...
---
该 **com.aspose.html.dom (Document Object Model)** 包提供用于表示和交互任何 HTML、XML 或 SVG 文档的 API。DOM 是在浏览器中加载的文档模型，以节点树的形式表示文档，其中每个节点代表文档的一部分（例如元素、文本字符串或注释）。
...
| [DocumentFragment](./documentfragment/) | DocumentFragment 是一种“轻量级”或“最小化”的 Document 对象。通常需要能够提取文档树的一部分或创建文档的新片段。 |
| [DocumentType](./documenttype/) | DocumentType 提供了一个接口，用于访问文档中定义的实体列表。 |
| [DOMException](./domexception/) | DOMException 接口表示因调用 Web API 的方法或访问其属性而产生的异常事件（称为异常）。这基本上是 Web API 中描述错误情况的方式。 |
...
| [QualifiedName](./qualifiedname/) | 表示 HTML 的限定名称。 |
...
| 接口 | 描述 |
| --- | --- |
| [IBrowsingContext](./ibrowsingcontext/) | 浏览上下文是呈现给用户的 [`Document`](../com.aspose.html.dom/document/) 对象的环境。 |
| [IChildNode](./ichildnode/) | 定义了应由能够拥有父节点的 [`Node`](../com.aspose.html.dom/node/) 实现的 [`IChildNode`](../com.aspose.html.dom/ichildnode/) 接口。 |
| [IDOMImplementation](./idomimplementation/) | DOMImplementation 接口提供了多种方法，用于执行独立于文档对象模型特定实例的操作。 |
| [IGlobalEventHandlers](./iglobaleventhandlers/) | 表示必须被所有支持系统事件处理的元素继承的接口。 |
| [INonDocumentTypeChildNode](./inondocumenttypechildnode/) | 定义了非 [`DOCUMENT_TYPE_NODE`](../com.aspose.html.dom/node/document_type_node/) 的 [`IChildNode`](../com.aspose.html.dom/ichildnode/)。 |
| [INonElementParentNode](./inonelementparentnode/) | 定义了非 Element 类型的 [`IParentNode`](../com.aspose.html.dom/iparentnode/)。 |
...
| 枚举 | 描述 |
| --- | --- |
| [ShadowRootMode](./shadowrootmode/) | ShadowRoot 可运行的模式。 |
