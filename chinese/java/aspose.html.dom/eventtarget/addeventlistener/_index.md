...
description: "EventTarget 方法。EventTarget 接口的 addEventListener 方法设置一个函数，该函数将在指定事件传递到目标时被调用。"
type: docs
...
addEventListener() 方法属于 [EventTarget ](T:com.aspose.html.dom.EventTarget) 接口，用于设置一个函数，该函数将在指定事件被发送到目标时被调用。

它通过向调用所在的 EventTarget 上指定事件类型的事件监听器列表中添加一个函数或实现了 [EventListener](T:com.aspose.html.dom.events.IEventListener) 的对象来工作。如果该函数或对象已经在此目标的事件监听器列表中，则不会再次添加。
...
| 处理程序 | DOMEventHandler | 接受一个在事件发生时被调用的 |
...
它通过向调用所在的 EventTarget 上指定事件类型的事件监听器列表中添加一个函数或实现了 [`EventListener`](../../../com.aspose.html.dom.events/ieventlistener/) 的对象来工作。如果该函数或对象已经在此目标的事件监听器列表中，则不会再次添加。
