...
description: "com.aspose.html.saving.MarkdownFeatures 枚举。MarkdownFeatures 标志集是以下一个或多个标志的集合，用于选择要转换为 markdown 的元素。"
type: docs
...
`MarkdownFeatures` 标志集是以下一个或多个标志的集合，用于选择要转换为 markdown 的元素。
...
| 名称 | 值 | 描述 |
| --- | --- | --- |
| InlineHTML | `1` | 此标志启用 HTML 元素的内联。如果设置此标志，则块级元素（例如 `div`）其 `markdown` 属性值等于 `inline` 将被插入到生成的 markdown 中。 |
...
| CodeBlock | `20` | 此标志启用对代码块的转换。代码块由两个元素 `pre` 和 `code` 组成，此类结构的内容 "按原样"。 |
