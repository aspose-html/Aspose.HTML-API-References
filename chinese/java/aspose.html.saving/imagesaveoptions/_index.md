...
description: "com.aspose.html.saving.ImageSaveOptions 类。特定的选项数据类。它提供属性来管理图像结果的分辨率、平滑、质量、格式以及页面设置等。更多信息可在文档文章中获取。"
type: docs
...
| 名称 | 描述 |
| --- | --- |
| [ImageSaveOptions](imagesaveoptions/#constructor)() | 初始化 `ImageSaveOptions` 类的新实例；Png 将用作默认图像格式。 |
| [ImageSaveOptions](imagesaveoptions/#constructor_1)(ImageFormat) | 图像格式 [`ImageFormat`](../../com.aspose.html.rendering.image/imageformat/) 基于初始化 |
...
| [horizontalResolution](../../com.aspose.html.rendering.image/imagerenderingoptions/horizontalresolution/) { get; set; } | 设置或获取输出和内部（在过滤器处理期间使用）图像的水平分辨率，单位为每英寸像素。默认情况下，此属性为 300 dpi。 |
...
| [verticalResolution](../../com.aspose.html.rendering.image/imagerenderingoptions/verticalresolution/) { get; set; } | 设置或获取输出和内部（在过滤器处理期间使用）图像的垂直分辨率，单位为每英寸像素。默认情况下，此属性为 300 dpi。 |
