...
description: "IXPathResult 方法。返回快照集合中第 index 项。如果 index 大于或等于列表中的节点数，则此方法返回 null。与迭代器结果不同，快照不会失效，但如果文档被修改，快照可能不再对应当前文档。"
type: docs
...
`NodeList` 中第 `index` 位置的节点，如果该索引无效，则为 `null`。
