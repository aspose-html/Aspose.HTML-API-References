---
title: FocusEvent.FocusEvent
second_title: Aspose.HTML voor .NET API-referentie
description: FocusEvent constructeur. Initialiseert een nieuw exemplaar van hetFocusEvent klasse.
type: docs
weight: 10
url: /nl/net/aspose.html.dom.events/focusevent/focusevent/
---
## FocusEvent(string) {#constructor}

Initialiseert een nieuw exemplaar van het[`FocusEvent`](../) klasse.

```csharp
public FocusEvent(string type)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| type | String | Het gebeurtenistype. |

### Zie ook

* class [FocusEvent](../)
* naamruimte [Aspose.Html.Dom.Events](../../focusevent/)
* montage [Aspose.HTML](../../../)

---

## FocusEvent(string, IDictionary&lt;string, object&gt;) {#constructor_1}

Initialiseert een nieuw exemplaar van het[`FocusEvent`](../) klasse.

```csharp
public FocusEvent(string type, IDictionary<string, object> eventInitDict)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| type | String | Het gebeurtenistype. |
| eventInitDict | IDictionary`2 | De gebeurtenis initialiseert woordenboek. |

### Zie ook

* class [FocusEvent](../)
* naamruimte [Aspose.Html.Dom.Events](../../focusevent/)
* montage [Aspose.HTML](../../../)


