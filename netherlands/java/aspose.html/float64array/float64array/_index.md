---
title: "Float64Array"
second_title: "Aspose.HTML voor Java API-referentie"
description: "Float64Array constructor. Initialiseert een nieuw exemplaar van de Float64Array-klasse"
type: docs

url: /nl/java/com.aspose.html/float64array/float64array/
---
## Float64Array(int) {#constructor_4}

Initialiseert een nieuw exemplaar van de [`Float64Array`](../) klasse.

```java
public Float64Array(int length)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| lengte | Int32 | De lengte (aantal elementen). |

### Zie ook

* class [Float64Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Float64Array(double[]) {#constructor_3}

Initialiseert een nieuw exemplaar van de [`Float64Array`](../) klasse.

```java
public Float64Array(double[] data)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| gegevens | Double[] | De array‑gegevens. |

### Zie ook

* class [Float64Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Float64Array(ArrayBuffer) {#constructor}

Initialiseert een nieuw exemplaar van de [`Float64Array`](../) klasse.

```java
public Float64Array(ArrayBuffer buffer)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| buffer | ArrayBuffer | De gegevensbuffer. |

### Zie ook

* class [ArrayBuffer](../../arraybuffer/)
* class [Float64Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Float64Array(ArrayBuffer, int) {#constructor_1}

Initialiseert een nieuw exemplaar van de [`Float64Array`](../) klasse.

```java
public Float64Array(ArrayBuffer buffer, int byteOffset)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| buffer | ArrayBuffer | De gegevensbuffer. |
| byteOffset | Int32 | De byte‑offset. |

### Zie ook

* class [ArrayBuffer](../../arraybuffer/)
* class [Float64Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Float64Array(ArrayBuffer, int, int) {#constructor_2}

Initialiseert een nieuw exemplaar van de [`Float64Array`](../) klasse.

```java
public Float64Array(ArrayBuffer buffer, int byteOffset, int length)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| buffer | ArrayBuffer | De gegevensbuffer. |
| byteOffset | Int32 | De byte‑offset. |
| lengte | Int32 | De lengte. |

### Zie ook

* class [ArrayBuffer](../../arraybuffer/)
* class [Float64Array](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)
