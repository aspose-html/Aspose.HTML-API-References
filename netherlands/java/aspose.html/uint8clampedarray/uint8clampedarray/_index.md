---
title: "Uint8ClampedArray"
second_title: "Aspose.HTML voor Java API-referentie"
description: "Uint8ClampedArray constructor. Initialiseert een nieuw exemplaar van de Uint8ClampedArray klasse"
type: docs

url: /nl/java/com.aspose.html/uint8clampedarray/uint8clampedarray/
---
## Uint8ClampedArray(int) {#constructor_4}

Initialiseert een nieuw exemplaar van de [`Uint8ClampedArray`](../) klasse.

```java
public Uint8ClampedArray(int length)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| lengte | Int32 | De lengte (aantal elementen). |

### Zie ook

* class [Uint8ClampedArray](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Uint8ClampedArray(byte[]) {#constructor_3}

Initialiseert een nieuw exemplaar van de [`Uint8ClampedArray`](../) klasse.

```java
public Uint8ClampedArray(byte[] data)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| gegevens | Byte[] | De array‑gegevens. |

### Zie ook

* class [Uint8ClampedArray](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Uint8ClampedArray(ArrayBuffer) {#constructor}

Initialiseert een nieuw exemplaar van de [`Uint8ClampedArray`](../) klasse.

```java
public Uint8ClampedArray(ArrayBuffer buffer)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| buffer | ArrayBuffer | De gegevensbuffer. |

### Zie ook

* class [ArrayBuffer](../../arraybuffer/)
* class [Uint8ClampedArray](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Uint8ClampedArray(ArrayBuffer, int) {#constructor_1}

Initialiseert een nieuw exemplaar van de [`Uint8ClampedArray`](../) klasse.

```java
public Uint8ClampedArray(ArrayBuffer buffer, int byteOffset)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| buffer | ArrayBuffer | De gegevensbuffer. |
| byteOffset | Int32 | De byte‑offset. |

### Zie ook

* class [ArrayBuffer](../../arraybuffer/)
* class [Uint8ClampedArray](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)

---

## Uint8ClampedArray(ArrayBuffer, int, int) {#constructor_2}

Initialiseert een nieuw exemplaar van de [`Uint8ClampedArray`](../) klasse.

```java
public Uint8ClampedArray(ArrayBuffer buffer, int byteOffset, int length)
```

| Parameter | Type | Beschrijving |
| --- | --- | --- |
| buffer | ArrayBuffer | De gegevensbuffer. |
| byteOffset | Int32 | De byte‑offset. |
| lengte | Int32 | De lengte. |

### Zie ook

* class [ArrayBuffer](../../arraybuffer/)
* class [Uint8ClampedArray](../)
* package [com.aspose.html](../../../com.aspose.html/)
* package [Aspose.HTML](../../../)
