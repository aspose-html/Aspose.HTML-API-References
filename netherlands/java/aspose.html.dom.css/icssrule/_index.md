...
De CSSRule‑interface is de abstracte basis‑interface voor elk type CSS‑statement. Dit omvat zowel regelsets als at‑rules. Van een implementatie wordt verwacht alle regels die in een CSS‑stijlblad zijn gespecificeerd te behouden, zelfs als de regel niet wordt herkend door de parser. Niet‑herkende regels worden weergegeven met behulp van de interface.
...
| [getType](../../com.aspose.html.dom.css/icssrule/type/) Het type van de regel, zoals gedefinieerd in [CSSOM # dom-cssrule-type](https://drafts.csswg.org/cssom/#dom-cssrule-type). De verwachting is dat bindings‑specifieke cast‑methoden kunnen worden gebruikt om van een instantie van de CSSRule‑interface naar de specifieke afgeleide interface te casten die door het type wordt geïmpliceerd. |
