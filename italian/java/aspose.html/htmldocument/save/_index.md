...
description: "HTMLDocument method. Salva il documento in un file locale specificato dall'url. Tutte le risorse utilizzate in questo documento saranno salvate in una cartella adiacente il cui nome sarà costruito come output_file_name _files."
type: docs
...
La maggior parte delle attività che devi eseguire richiede il salvataggio di un documento. Una volta caricato il file esistente o creato un documento HTML da zero, puoi salvare le modifiche utilizzando uno dei metodi HTMLDocument.Save(). I metodi consentono di salvare l'HTML in un file locale specificato da percorso, URL o archiviazione di output. Consulta la [documentazione](https://docs.aspose.com/html/net/working-with-documents/saving-a-document/) per saperne di più sul salvataggio.
...
Salva il contenuto del documento e le risorse usando il [`ResourceHandler`](../../../com.aspose.html.saving.resourcehandlers/resourcehandler/).
...
Il metodo Save(String, HTMLSaveFormat) accetta come parametri un percorso del file system locale per il file di output e saveFormat. L'enumerazione [`HTMLSaveFormat`](../../../com.aspose.html.saving/htmlsaveformat/) specifica il formato in cui il documento viene salvato; può essere HTML, MHTML e MD. Il metodo salva il documento HTML nel formato specificato nel file locale indicato dal percorso. Tutte le risorse utilizzate nel documento verranno salvate in una cartella adiacente.
...
import (var document = new HTMLDocument(inputHtmlPath))
{
	...
	var outputFilePath = Path.Combine(outputHtmlPath, "result.html");
	// Definisci un'istanza della classe di opzioni
...
Metodo Save(Url, HTMLSaveOptions)

È necessario specificare un percorso Url completo per il salvataggio del documento HTML. Il costruttore Url(url) crea un'istanza della classe [`Url`](../../url/) con l'url specificato. Il costruttore HTMLSaveOptions() crea un'istanza della classe [`HTMLSaveOptions`](../../../com.aspose.html.saving/htmlsaveoptions/) che possiede le proprietà ResourceHandlingOptions, utilizzate per la configurazione della gestione delle risorse. Il metodo Save(url, saveOptions) accetta i parametri e salva il documento HTML con le risorse nel file locale specificato dall'url.
...
| --- | --- | --- |
| percorso | String | Percorso locale del file di output. |
| saveOptions | MarkdownSaveOptions | L'utilizzo dell'oggetto [`MarkdownSaveOptions`](../../../com.aspose.html.saving/markdownsaveoptions/) consente di regolare il processo di rendering. Per ulteriori informazioni, vedere la [Documentazione Aspose](https://docs.aspose.com/html/net/converting-between-formats/html-to-markdown/#save-options). |
...
| --- | --- | --- |
| percorso | String | Percorso locale del file di output. |
| saveOptions | MHTMLSaveOptions | L'utilizzo dell'oggetto [`MHTMLSaveOptions`](../../../com.aspose.html.saving/mhtmlsaveoptions/) consente di regolare il processo di rendering. Per ulteriori informazioni, vedere la [documentazione](https://docs.aspose.com/html/net/converting-between-formats/html-to-mhtml/#save-options). |
