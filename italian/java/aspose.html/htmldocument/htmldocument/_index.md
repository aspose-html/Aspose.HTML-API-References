...
Nota: Il documento è creato con un valore predefinito per la proprietà base-url pari a 'about:blank'.
...
Una volta creato l'oggetto documento, può essere successivamente riempito con elementi HTML. Il frammento di codice seguente mostra l'uso del costruttore predefinito HTMLDocument() per creare un documento HTML vuoto e salvarlo in un file.
...
Nota: Nel caso in cui venga fornito un URL errato che non può essere raggiunto al momento, la libreria genera il [`DOMException`](../../../com.aspose.html.dom/domexception/) con il codice specializzato ‘NetworkError’ per informare che la risorsa selezionata non può essere trovata.
...
Nota: Nel caso in cui venga fornito un URL errato che non può essere raggiunto al momento, la libreria genera il [DOMException](T:com.aspose.html.dom.DOMException) con il codice specializzato ‘NetworkError’ per informare che la risorsa selezionata non può essere trovata.
...
Crea un documento HTML da un contenuto String con base-uri specificato.
...
| --- | --- |
| ArgumentNullException | Genera un'eccezione se il parametro base-uri è nullo. |
...
Crea un documento HTML da un contenuto [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream) con base-uri specificato, utilizzato per risolvere il percorso delle risorse relative.
...
Per definizione, un gestore di messaggi è una classe che riceve una richiesta Web e restituisce una risposta Web. In altre parole, un gestore di messaggi è usato per elaborare una richiesta di servizio Web durante l'input e/o per elaborare la risposta durante l'output.

Per favore, visita il nostro [sito della documentazione](https://docs.aspose.com/html/net/message-handlers/) per vedere più scenari su come utilizzare questo costruttore.
...
Crea un documento HTML da un oggetto [RequestMessage](T:com.aspose.html.net.RequestMessage).
