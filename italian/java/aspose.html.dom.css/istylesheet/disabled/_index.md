...
Un foglio di stile può essere disabilitato impostando manualmente questa proprietà a true o se è un foglio di stile alternativo inattivo. Nota che disabled == false non garantisce che il foglio di stile sia applicato (potrebbe essere rimosso dal documento, ad esempio).

Modificare questo attributo può causare una nuova risoluzione dello stile per il documento. Un foglio di stile si applica solo se è presente sia una definizione di media appropriata sia l'attributo disabled è false. Quindi, se il media non si applica all'agente utente corrente, l'attributo disabled viene ignorato.
