...
Viene utilizzato per fornire informazioni contestuali su un evento al gestore che elabora l'evento.
...
| [stopPropagation](../../com.aspose.html.dom.events/event/stoppropagation/)() | Il metodo [`StopPropagation`](./stoppropagation/) è usato per impedire ulteriori propagazioni di un evento durante il flusso degli eventi. |
...
| const [CapturingPhase](../../com.aspose.html.dom.events/event/capturingphase/) | L'evento è attualmente valutato al target [`IEventTarget`](../ieventtarget/). |
| const [NonePhase](../../com.aspose.html.dom.events/event/nonephase/) | Gli eventi non attualmente inviati sono in questa fase. |

## Osservazioni

Un oggetto che implementa l' è generalmente passato come primo parametro a un gestore di eventi. Informazioni contestuali più specifiche sono passate ai gestori di eventi derivando interfacce aggiuntive da cui contengono informazioni direttamente relative al tipo di evento a cui accompagnano. Queste interfacce derivate sono anche implementate dall'oggetto passato all'ascoltatore di eventi.
