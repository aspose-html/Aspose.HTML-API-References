...
description: "com.aspose.html.saving.MarkdownFeatures enum. Un set di flag MarkdownFeatures è un insieme di zero o più dei seguenti flag che vengono utilizzati per selezionare gli elementi convertiti in markdown"
type: docs
...
Un set di flag `MarkdownFeatures` è un insieme di zero o più dei seguenti flag, che vengono utilizzati per selezionare gli elementi convertiti in markdown.
...
| --- | --- | --- |
| InlineHTML | `1` | Questo flag abilita l'inserimento in linea degli elementi HTML. Se questo flag è impostato, gli elementi di livello blocco (come `div`) il cui valore dell'attributo `markdown` è `inline` verranno inseriti nel markdown risultante. |
| AutomaticParagraph | `2` | Questo flag abilita la conversione degli elementi `paragraph`. Il contenuto di tali elementi verrà posizionato su linee separate, così i gestori markdown lo avvolgeranno. |
...
| CodeBlock | `20` | Questo flag abilita la conversione dei blocchi di codice. Un blocco di codice è composto da 2 elementi `pre` e `code`, il contenuto di tale costruzione viene elaborato "così com'è". |
