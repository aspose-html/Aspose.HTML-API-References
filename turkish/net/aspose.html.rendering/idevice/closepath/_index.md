---
title: IDevice.ClosePath
second_title: Aspose.HTML for .NET API Referansı
description: IDevice yöntem. Mevcut noktadan alt yolun başlangıç noktasına düz bir çizgi parçası ekleyerek mevcut alt yolu kapatır. Mevcut alt yol zaten kapalıysa ClosePath hiçbir şey yapmaz. Bu işleç mevcut alt yolu sonlandırır. Geçerli yola başka bir parça eklemek yeni bir alt yol başlatır yeni bölüm ClosePath yöntemiyle ulaşılan bitiş noktasında başlasa bile .
type: docs
weight: 80
url: /tr/net/aspose.html.rendering/idevice/closepath/
---
## IDevice.ClosePath method

Mevcut noktadan alt yolun başlangıç noktasına düz bir çizgi parçası ekleyerek mevcut alt yolu kapatır. Mevcut alt yol zaten kapalıysa, "ClosePath" hiçbir şey yapmaz. Bu işleç mevcut alt yolu sonlandırır. Geçerli yola başka bir parça eklemek yeni bir alt yol başlatır, yeni bölüm "ClosePath" yöntemiyle ulaşılan bitiş noktasında başlasa bile .

```csharp
public void ClosePath()
```

### Ayrıca bakınız

* interface [IDevice](../)
* ad alanı [Aspose.Html.Rendering](../../idevice/)
* toplantı [Aspose.HTML](../../../)


