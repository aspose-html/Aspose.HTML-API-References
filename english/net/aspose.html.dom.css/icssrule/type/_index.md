---
title: Type
second_title: Aspose.HTML for .NET API Reference
description: The type of the rule as defined CSSOM  domcssruletypehttps//drafts.csswg.org/cssom/domcssruletype. The expectation is that bindingspecific casting methods can be used to cast down from an instance of the CSSRule interface to the specific derived interface implied by the type.
type: docs
weight: 40
url: /net/aspose.html.dom.css/icssrule/type/
---
## ICSSRule.Type property

The type of the rule, as defined [CSSOM # dom-cssrule-type](https://drafts.csswg.org/cssom/#dom-cssrule-type). The expectation is that binding-specific casting methods can be used to cast down from an instance of the CSSRule interface to the specific derived interface implied by the type.

```csharp
public short Type { get; }
```

### Property Value

A non-negative integer associated with a particular type of rule. This item is initialized when a rule is created and cannot change.

### See Also

* interface [ICSSRule](../)
* namespace [Aspose.Html.Dom.Css](../../icssrule/)
* assembly [Aspose.HTML](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.HTML.dll -->
