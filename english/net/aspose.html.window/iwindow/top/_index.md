---
title: Top
second_title: Aspose.HTML for .NET API Reference
description: The top IDL attribute on the Window object of a Document in a browsing context b must return the WindowProxy object of its toplevel browsing context which would be its own WindowProxy object if it was a toplevel browsing context itself if it has one or its own WindowProxy object otherwise e.g. if it was a detached nested browsing context.
type: docs
weight: 80
url: /net/aspose.html.window/iwindow/top/
---
## IWindow.Top property

The top IDL attribute on the Window object of a Document in a browsing context b must return the WindowProxy object of its top-level browsing context (which would be its own WindowProxy object if it was a top-level browsing context itself), if it has one, or its own WindowProxy object otherwise (e.g. if it was a detached nested browsing context).

```csharp
public IWindow Top { get; }
```

### See Also

* interface [IWindow](../)
* namespace [Aspose.Html.Window](../../iwindow/)
* assembly [Aspose.HTML](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.HTML.dll -->
