...
| [NextSibling](../../aspose.html.dom/node/nextsibling) { get; } | The read-only nextSibling property of the [`Node`](../node) interface returns the node immediately following the specified one in their parent's [`childNodes`](./childnodes), or returns null if the specified node is the last child in the parent element. |
...
| [PreviousSibling](../../aspose.html.dom/node/previoussibling) { get; } | The read-only previousSibling property of the [`Node`](../node) interface returns the node immediately preceding the specified one in its parent's [`childNodes`](./firstchild) list, or null if the specified node is the first in that list. |
