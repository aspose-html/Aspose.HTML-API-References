---
title: Cancelable
second_title: Référence de l'API Aspose.HTML pour .NET
description: Utilisé pour indiquer si un événement peut ou non voir son action par défaut empêchée. Si laction par défaut peut être empêchée la valeur est true sinon la valeur est false.
type: docs
weight: 30
url: /fr/net/aspose.html.dom.events/event/cancelable/
---
## Event.Cancelable property

Utilisé pour indiquer si un événement peut ou non voir son action par défaut empêchée. Si l'action par défaut peut être empêchée, la valeur est true, sinon la valeur est false.

```csharp
public bool Cancelable { get; }
```

### Voir également

* class [Event](../../event)
* espace de noms [Aspose.Html.Dom.Events](../../event)
* Assemblée [Aspose.HTML](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.HTML.dll -->
