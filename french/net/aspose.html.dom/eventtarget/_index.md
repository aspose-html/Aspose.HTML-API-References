...
description: LeEventTarget./eventtarget est implémentée par tous les nœuds dans une implémentation qui prend en charge le modèle dévénement DOM. Par conséquent cette interface peut être obtenue en utilisant des méthodes de conversion spécifiques à la liaison sur une instance de linterface de nœud. Linterface permet lenregistrement et la suppression des écouteurs dévénement sur unEventTarget./eventtarget et lenvoi dévénements à ceIEventTarget../aspose.html.dom.events/ieventtarget .
type: docs
weight: 730
...
Le[`EventTarget`](../eventtarget) est implémentée par tous les nœuds dans une implémentation qui prend en charge le modèle d'événement DOM. Par conséquent, cette interface peut être obtenue en utilisant des méthodes de conversion spécifiques à la liaison sur une instance de l'interface de nœud. L'interface permet l'enregistrement et la suppression des écouteurs d'événement sur un[`EventTarget`](../eventtarget) et l'envoi d'événements à ce[`IEventTarget`](../../aspose.html.dom.events/ieventtarget) .
...
| [RemoveEventListener](../../aspose.html.dom/eventtarget/removeeventlistener#removeeventlistener_1)(string, IEventListener) | Cette méthode permet de supprimer les écouteurs d'événement de la cible de l'événement. Si un[`IEventListener`](../../aspose.html.dom.events/ieventlistener) est retiré d'un[`EventTarget`](../eventtarget) pendant qu'il traite un événement, il ne sera pas déclenché par les actions en cours. Les écouteurs d'événement ne peuvent jamais être invoqués après avoir été supprimés. |
...
| [RemoveEventListener](../../aspose.html.dom/eventtarget/removeeventlistener#removeeventlistener_2)(string, IEventListener, bool) | Cette méthode permet de supprimer les écouteurs d'événement de la cible de l'événement. Si un[`IEventListener`](../../aspose.html.dom.events/ieventlistener) est retiré d'un[`EventTarget`](../eventtarget) pendant qu'il traite un événement, il ne sera pas déclenché par les actions en cours. Les écouteurs d'événement ne peuvent jamais être invoqués après avoir été supprimés. |
