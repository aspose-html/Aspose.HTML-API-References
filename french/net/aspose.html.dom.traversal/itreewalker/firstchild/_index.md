---
title: FirstChild
second_title: Référence de l'API Aspose.HTML pour .NET
description: Déplace le TreeWalker vers le premier enfant visible du nœud actuel et renvoie le nouveau nœud. Si le nœud actuel na pas denfants visibles  renvoie null et conserve le nœud current .
type: docs
weight: 20
url: /fr/net/aspose.html.dom.traversal/itreewalker/firstchild/
---
## ITreeWalker.FirstChild method

Déplace le TreeWalker vers le premier enfant visible du nœud actuel et renvoie le nouveau nœud. Si le nœud actuel n'a pas d'enfants visibles , renvoie null et conserve le nœud current .

```csharp
public Node FirstChild()
```

### Return_Value

Le nouveau nœud, ou null si le nœud actuel n'a pas d'enfants visibles dans la vue logique du TreeWalker.

### Voir également

* class [Node](../../../aspose.html.dom/node)
* interface [ITreeWalker](../../itreewalker)
* espace de noms [Aspose.Html.Dom.Traversal](../../itreewalker)
* Assemblée [Aspose.HTML](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.HTML.dll -->
