---
title: Referrer
second_title: Référence de l'API Aspose.HTML pour .NET
description: Renvoie lURI de la page liée à cette page. La valeur est une chaîne vide si lutilisateur a accédé directement à la page pas via un lien mais par exemple via un signet.
type: docs
weight: 30
url: /fr/net/aspose.html.dom.svg/svgdocument/referrer/
---
## SVGDocument.Referrer property

Renvoie l'URI de la page liée à cette page. La valeur est une chaîne vide si l'utilisateur a accédé directement à la page (pas via un lien, mais, par exemple, via un signet).

```csharp
public string Referrer { get; }
```

### Valeur de la propriété

Le référent.

### Voir également

* class [SVGDocument](../../svgdocument)
* espace de noms [Aspose.Html.Dom.Svg](../../svgdocument)
* Assemblée [Aspose.HTML](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.HTML.dll -->
