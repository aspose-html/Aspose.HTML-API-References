...
Remarque : Si vous fournissez une URL incorrecte qui ne peut pas être atteinte pour le moment, la bibliothèque lève la [`DOMException`](../../../com.aspose.html.dom/domexception/) avec le code spécialisé ‘NetworkError’ pour vous informer que la ressource sélectionnée est introuvable.
...
Crée un document HTML à partir d’un contenu String avec l’URI de base spécifié.
...
Crée un document HTML à partir d’un contenu [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream) avec l’URI de base spécifié, utilisé pour résoudre le chemin des ressources relatives.
...
import (var sw = new StreamWriter(mem))
{
	...
	// Il est important de définir la position au début puisque HTMLDocument commence la lecture exactement à partir de la position actuelle dans le flux
	sw.Flush();
...
Veuillez visiter notre [site de documentation](https://docs.aspose.com/html/net/message-handlers/) pour voir plus de scénarios sur la façon d'utiliser ce constructeur.
...
Crée un document HTML à partir de l’objet [RequestMessage](T:com.aspose.html.net.RequestMessage).
