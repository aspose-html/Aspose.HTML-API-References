...
description: "Interface com.aspose.html.dom.xpath.IXPathResult. L'interface XPathResult représente le résultat de l'évaluation d'une expression XPath 1.0 dans le contexte d'un nœud particulier. Comme l'évaluation d'une expression XPath peut produire différents types de résultats, cet objet permet de découvrir et de manipuler le type et la valeur du résultat"
type: docs
...
| [getInvalidIteratorState](../../com.aspose.html.dom.xpath/ixpathresult/invaliditeratorstate/) Indique que l'itérateur est devenu invalide. Vrai si le type `resultType` est `UnorderedNodeIterator` ou `OrderedNodeIterator` et que le document a été modifié depuis que ce résultat a été renvoyé. |
| [getNumberValue](../../com.aspose.html.dom.xpath/ixpathresult/numbervalue/) La valeur de ce résultat numérique. |
| [getResultType](../../com.aspose.html.dom.xpath/ixpathresult/resulttype/) Un code représentant le type de ce résultat, tel que défini par l'énumération http://www.w3.org/TR/DOM-Level-3-XPath/xpath.html#XPathResult[`XPathResultType`](../xpathresulttype/) . |
...
| [snapshotItem](../../com.aspose.html.dom.xpath/ixpathresult/snapshotitem/)(int) | Renvoie l'élément `index`‑ème de la collection d'instantanés. Si `index` est supérieur ou égal au nombre de nœuds dans la liste, cette méthode renvoie `null`. Contrairement au résultat de l'itérateur, l'instantané ne devient pas invalide, mais il peut ne plus correspondre au document actuel s'il est modifié. |
