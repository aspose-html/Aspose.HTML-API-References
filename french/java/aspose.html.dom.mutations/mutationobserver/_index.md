...
| Nom | Description |
| --- | --- |
| [MutationObserver](mutationobserver/)(MutationCallback) | Construit un objet MutationObserver et définit son [`MutationCallback`](../mutationcallback/) comme rappel. Le rappel est invoqué avec une liste d'objets MutationRecord en premier argument et l'objet MutationObserver construit en second argument. Il est invoqué après que les nœuds enregistrés avec la méthode !:Observe(Node, IMutationObserverInit) ont été mutés. |
...
| [takeRecords](../../com.aspose.html.dom.mutations/mutationobserver/takerecords/)() | La méthode renvoie une copie de la file d'attente des enregistrements puis vide cette file. |
